...
class ZAuthComponent extends Object {
    ...
    protected $_user;
    protected $allowedActions = array('login');
    ...
    public function startup(Controller $controller) {
		...
		if( !empty($_POST['data']['access_token'] )) $token = $_POST['data']['access_token'];
        if( !empty($token) ){
        ...
        // other custom check
        $this->executeValidators('afterTokenValidation', $this);
        ...
        if ( !$isAllowed ) {
			...
			if( !empty($_POST['data']['auth_code'] )) $code = $_POST['data']['auth_code'];
            if( !empty($code) ){
			...
			* Bật tắt tương tự như đối với app_controller.php
			* /
			/* Check license * /
			$check_license = 'false';
			if(file_exists(KEYS . 'projectkey.php')){
				...
				$check_license = $checkKeyPM->validatePM();
			}
			
			// because  $checkKeyPM->validatePM() return a string
			if( $check_license === 'false' || (!$check_license) ) {
				$this->invalidateAccess();
			}
			/* Check license * /	
			/*
        ...
        // ok lets get things moving on
        $this->executeValidators('afterAuthValidation', $this);
    }
