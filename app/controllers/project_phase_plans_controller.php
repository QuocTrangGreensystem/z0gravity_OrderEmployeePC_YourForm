 ...
 * @access public
 */
class ProjectPhasePlansController extends AppController {
    ...
    function phase_vision($project_id = null, $new_gantt = null) {
		...
		// $condPhaseProject['ProjectMilestone.milestone_date !='] = '0000-00-00';
		...
		$project_gantt = $this->ProjectPhasePlan->Project->find("first", array(
			'contain' => array('ProjectMilestone' => array('project_milestone', 'milestone_date', 'validated', 'part_id', 'order' => 'milestone_date ASC', 'conditions' => array('milestone_date !=' => '0000-00-00', 'milestone_date IS NOT NULL'))),
			"fields" => array('id', 'project_name', 'start_date', 'end_date', 'planed_end_date'), 
			'conditions' => $condPhaseProject
		));
        $this->set('projectName', $project_gantt);
        $options = array(
            'limit' => 2000,
            'contain' => array(
    ...
    function index($project_id = null) {
        ...
        $this->set(compact('projectName', 'projectPhases','projectPhases1', 'projectPhaseStatuses', 'projectPhasePlans', 'project_id', 'projectParts', 'countTaskOfPhases', 'profiles', 'activateProfile', 'phaseDefaults', 'manuallyAchievement', 'displayParst', 'loadFilter'));
    }
    ...
    public function time_reset_plan_real($project_id = null){
        ...
        foreach($time_reset as $phase){
                ...
                $this->ProjectPhasePlan->save();
				$_duration = 0;
				if(($phase['ProjectPhasePlan']['phase_planed_start_date'] != '0000-00-00') && ($phase['ProjectPhasePlan']['phase_planed_end_date'] != '0000-00-00')){
					$_duration = $this->getWorkingDays($phase['ProjectPhasePlan']['phase_planed_start_date'], $phase['ProjectPhasePlan']['phase_planed_end_date'], $_duration);
				}
                $this->ProjectTask->updateAll(array('task_start_date'=>$v1,'task_end_date'=>$v2,'duration'=>$_duration), array('ProjectTask.project_planed_phase_id'=>$phase['ProjectPhasePlan']['id']));
