...
class ProjectIssuesController extends AppController {
    ...
    function index($project_id = null) {
        if( !empty($this->employee_info['Color']['is_new_design']) && $this->employee_info['Color']['is_new_design']){
            $this->redirect(array('controller' => 'project_issues_preview','action' => 'index/'. $project_id));
        }
        ...
        $this->set(compact('company_id', 'employeeLoginId', 'avatarEmployeeLogin', 'employeeLoginName', 'companyName', 'avatarEmploys', 'logSystems', 'employee_id', 'listEmployee', '_issueEmployeeRefer'));
