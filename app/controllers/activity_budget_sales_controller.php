 ...
 */
class ActivityBudgetSalesController extends AppController {
    /**
     * 
     * LUU Y: KHI CHINH SUA CONTROLLER NAY THI NHO CHINH SUA CONTROLLER
     * ------------------------ project_budget_sales_controller --------------------------
    ...
    function index($activity_id = null) {
        ...
        $this->_parseParams();
        $this->set(compact('activityName', 'activity_id', 'budgetCustomers', 'budgetSales', 'billed', 'paid', 'billed_check', 'projectLinked', 'bg_currency'));
    }
        ...
        if (empty($this->data['Upload']) && !empty($_FILES)) {
            ...
            if(empty($this->data['Upload']['id_invoice'])){
                ...
                if ($projectBudgetSales) {
                    ...
                    $activity_id = $projectBudgetSales['ProjectBudgetSale']['activity_id'];
                }
                if($project_id != 0){
                    ...
                    if (!empty($attachment)) {
                        ...
                        if ($this->ProjectBudgetSale->save(array(
                            ...
                            if($this->MultiFileUpload->otherServer == true){
                                $this->MultiFileUpload->uploadFileToServerOther($path, $attachment, '/activity_budget_sales/index/' . $activity_id);
                            }
                            $result = true;
                        } else {
                ...
                if ($projectBudgetInvoices) {
                    ...
                    $activity_id = $projectBudgetInvoices['ProjectBudgetInvoice']['activity_id'];
                }
                if($project_id != 0){
                    $path = $this->_getPath($project_id);
                } else {
                ...
                if(!empty($this->data['Upload']['url'])){
                    ...
                    $this->data['Upload']['url'] = $this->LogSystem->cleanHttpString($this->data['Upload']['url']);
                    if ($this->ProjectBudgetInvoice->save(array(
                                'file_attachement' => $this->data['Upload']['url'],
                                'format' => 1
                            ))) {
                        $this->Session->setFlash(__('Saved', true), 'success');
    ...
    public function attachement($id = null, $checkTypes) {
        ...
        if(!empty($checkTypes)){
            if($checkTypes == 'sale'){
                ...
                $error = true;
                if ($last && $last['ProjectBudgetSale']['activity_id']) {
                    if($last['ProjectBudgetSale']['project_id'] != 0){
                        $path = trim($this->_getPath($last['ProjectBudgetSale']['project_id'])
                            . $last['ProjectBudgetSale']['file_attachement']);
                    } else {
                        $path = trim($this->_getPathActivity($last['ProjectBudgetSale']['activity_id'])
                            . $last['ProjectBudgetSale']['file_attachement']);
                    }  
                    ...
                    if ($type != 'download') {
                        ...
                        if($this->MultiFileUpload->otherServer == true){
                            ...
                            $this->MultiFileUpload->deleteFileToServerOther($path, $attachment, '/activity_budget_sales/index/' . $last['ProjectBudgetSale']['activity_id']);
                        }
                    }
                }
                if ($type != 'download') {
                    ...
                    $this->redirect(array('action' => 'index',
                        $last ? $last['ProjectBudgetSale']['activity_id'] : __('Unknown', true)));
                }
                ...
                $error = true;
                if ($last && $last['ProjectBudgetInvoice']['activity_id']) {
                    if($last['ProjectBudgetInvoice']['project_id'] != 0){
                        $path = trim($this->_getPath($last['ProjectBudgetInvoice']['project_id'])
                            . $last['ProjectBudgetInvoice']['file_attachement']);
                    } else {
                        $path = trim($this->_getPathActivity($last['ProjectBudgetInvoice']['activity_id'])
                            . $last['ProjectBudgetInvoice']['file_attachement']);
                    }
                    ...
                    if ($type != 'download') {
                        ...
                        if($this->MultiFileUpload->otherServer == true){
                            ...
                            $this->MultiFileUpload->deleteFileToServerOther($path, $attachment, '/activity_budget_sales/index/' . $last['ProjectBudgetInvoice']['activity_id']);
                        }
                    }
                }
                if ($type != 'download') {
                    ...
                    $this->Session->setFlash(__('File not found.', true), 'error');
                    $this->redirect(array('action' => 'index',
                        $last ? $last['ProjectBudgetInvoice']['activity_id'] : __('Unknown', true)));
                }
            } else {
                //do nothing
            }
    ...
    protected function _getPathActivity($activity_id) {
        ...
        $pcompany = ClassRegistry::init('Company')->find('first', array(
            'recursive' => -1, 'conditions' => array('Company.id' => $company['Company']['parent_id'])));
        $path = FILES . 'project_budgets' . DS . 'activity_sales' . DS;
        if ($pcompany) {
            $path .= strtolower(Inflector::slug(' ', '_', $pcompany['Company']['dir'])) . DS . $activity_id . DS;
        }
     ...
     * @access public
     */
    function delete($id = null, $activity_id = null, $checkTypes) {
        $this->loadModel('ProjectBudgetInvoice');
        $this->loadModel('ProjectBudgetSale');
        if (!$id) {
        ...
        if($checkTypes == 'sale'){
            ...
            if($invoices != 0){
				...
				$activity_id = @$last['ProjectBudgetSale']['activity_id'];			
				$datas = array(
					'project_id' => 0,
                ...
                if ($last && $this->ProjectBudgetSale->delete($id)) {
                    if($last['ProjectBudgetExternal']['project_id'] != 0){
                        ...
                        @unlink(trim($this->_getPathActivity($last['ProjectBudgetSale']['activity_id'])
                            . $last['ProjectBudgetSale']['file_attachement']));
                    }   
			...
			$activity_id = @$last['ProjectBudgetInvoice']['activity_id'];			
			$datas = array(
				'project_id' => 0,
				'activity_id' => $activity_id
			);
            ...
            if ($last && $this->ProjectBudgetInvoice->delete($id)) {
                if($last['ProjectBudgetInvoice']['project_id'] != 0){
                    ...
                    @unlink(trim($this->_getPathActivity($last['ProjectBudgetInvoice']['activity_id'])
                        . $last['ProjectBudgetInvoice']['file_attachement']));
                }
        ...
        $this->Session->setFlash(__('Activity Budget Sales  was not deleted', true), 'error');
        $this->redirect(array('action' => 'index', $activity_id));
    }
