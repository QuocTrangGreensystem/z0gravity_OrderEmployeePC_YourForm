 ...
 */
class ActivityTasksPreviewController extends AppController {
    /**
     * Controller name
     *
     * @var string
     * @access public
     */
    var $name = 'ActivityTasksPreview';
    var $uses = array('ActivityTasks');
    ...
    function beforeFilter() {
        ...
        $this->loadModels('ActivityRequest', 'ProjectTaskEmployeeRefer', 'ActivityTaskEmployeeRefer', 'ActivityTasks');
    ...
    function index($activity_id = null) {
        ...
        foreach ($datas as $data) {
            ...
            if (!isset($sumActivities[$dx['task_id']])) {
                $sumActivities[$dx['task_id']] = 0;
            }

            $sumActivities[$dx['task_id']] += $data;

            if (!isset($sumEmployees[$dx['task_id']][$dx['employee_id']])) {
                $sumEmployees[$dx['task_id']][$dx['employee_id']] = 0;
            }

            $sumEmployees[$dx['task_id']][$dx['employee_id']] += $data;
        ...
        $previous = $this->ActivityTask->find('all', array(
                        'recursive' => -1,
                        'conditions' => array('activity_id' => $activity_id),
                        'fields' => array('id', 'previous')
                    ));

        $previous =Set::combine($previous, '{n}.ActivityTask.previous', '{n}.ActivityTask');
        ...
        if($projectLinked != 0){
            ...
            $activityTasks = $this->ActivityTask->find('all', array(
                'fields' => array(
                    'SUM(ActivityTask.estimated) AS Total',
                    'SUM(ActivityTask.special_consumed) AS exConsumed'
                ),
                'recursive' => -1,
                'conditions' => array('activity_id' => $activity_id, 'special' => 1)
            ));
            $varE = !empty($activityTasks) && !empty($activityTasks[0][0]['Total']) ? $activityTasks[0][0]['Total'] : 0;
            $externalConsumeds = !empty($activityTasks) && !empty($activityTasks[0][0]['exConsumed']) ? $activityTasks[0][0]['exConsumed'] : 0;
        }
        // BudgetSetting
        ...
        $workloadInter = $getDataProjectTasks['workload'] - $varE-$varO['ActivityTask']['s_over'];
        $workloadExter = $varE;
        // kiem tra role
        ...
        $this->ActivityTask->virtualFields = array('consumedex' => 'SUM(ActivityTask.special_consumed)');
        $consumedExter = $this->ActivityTask->find('first', array(
                            'fields' => array('ActivityTask.consumedex'),
                            'recursive' => -1,
                            'conditions' => array('activity_id' => $activity_id,'special'=>1)
        ));
        if($projectLinked != 0){
        ...
        if(!empty($activityTasks)){
            ...
            foreach($activityTasks as $key => $activityTask){
                ...
                if($activityTask['ActivityTask']['previous'] == 1){
                    ...
                    $children[$key]['hasUsed'] = !empty($activityRequests[$activityTask['ActivityTask']['id']]['hasUsed']) ? $activityRequests[$activityTask['ActivityTask']['id']]['hasUsed'] : 0;
                    $children[$key]['wait'] = !empty($activityRequests[$activityTask['ActivityTask']['id']]['wait']) ? $activityRequests[$activityTask['ActivityTask']['id']]['wait'] : 0;
                    //estimated task
...
//                }
    ...
    private function _getConsumedActivity($activity_id = null){
     ...
     public function visions_staffing() {
		...
		/* Do có 2 popup cùng 1 màn hình nên phải đặt 2 element khác ID, Ở đây gộp lại */ 
        if(!empty($this->params['url']['aPC'])) $this->params['url']['aPC'] = array_filter($this->params['url']['aPC']);
		if(!empty($this->params['url']['aProfitCenter'])&& empty($this->params['url']['aPC'])){
			$this->params['url']['aPC'] = $this->params['url']['aProfitCenter'];
			$_GET['aPC'] = $_GET['aProfitCenter'];
		}
		unset( $this->params['url']['aProfitCenter']);
		unset( $_GET['aProfitCenter']);
		/* END Do có 2 popup cùng 1 màn hình nên phải đặt 2 element khác ID, Ở đây gộp lại */ 
		if(isset($this->params['url']['type']) && $this->params['url']['type'] == 5){
		...
		if($this->params['url']['selectPCAll'] == "true"){
			unset($this->params['url']['aPC']);
			unset($_GET['aPC']);
		}
		...
		$dateType =isset($arrGetUrl['aDateType']) ? $arrGetUrl['aDateType'] : '';
		if($dateType == 1)
		  $dateType = 'day';
		elseif($dateType == 2)
		  $dateType = 'week';
		...
		if($dateType == 'day' || $dateType == 'week'){
			$endDateCheck = strtotime('+6 months',strtotime($arrGetUrl['aStartDate'])); 
		...
		if($type != 2){ 
			...
			$args['conditions']['Project.category'] = 1;
	...
	//            );
				...
				switch ($showType) {
					case 0 : {
                            $is_ajax = !empty($arrGetUrl['ajax']) ? 1 : 0;
							$staffings = $this->_visionsStaffingActivity($activities, $filterEmployee, $startDateFilter, $endDateFilter, $onlyEmployee, $dateType, $is_ajax);
							break;
						}
					case 1 : {
			...
			// $activityType
			$this->loadModels('Project', 'TmpStaffingSystem', 'ProjectAmrProgram', 'ProjectAmrSubProgram', 'Employee', 'ProjectTask', 'ProjectTaskEmployeeRefer');
			// Get staffing for projects
			$project_conds = array(
				'company_id' => $this->employee_info['Company']['id'],
				'category' => 1,
			);
			if(!empty($arrGetUrl['ajax'])){
				$project_conds = array(
					'id' => $arrGetUrl['ItMe'],
					'category' => 1,
				);
			}
            if(!isset($_conditions['employee_id'])){
				$_conditions['employee_id'] = array();
			}
			if(!isset($_conditions['profit_center_id'])){
				$_conditions['profit_center_id'] = array();
			}
            $_conditions['employee_id'] = array_filter($_conditions['employee_id']);
            $_conditions['profit_center_id'] = array_filter($_conditions['profit_center_id']);
			if(!empty( $_conditions['employee_id']) || !empty( $_conditions['profit_center_id'])){
				$_datas = array();
                $isProfitCenter = $isEmployee = false;
                if(!empty($_conditions)){
                    $_getDatas = $setIds = $_setIds = array();
                ...
                if(!empty($_datas)){
                   $project_conds['activity_id'] = $_datas;
                }
			}
			$projects = $this->Project->find('all', array(
				'recursive' => -1,
				'conditions' => $project_conds,
				'fields' => array('id', 'project_name', 'project_amr_program_id', 'project_amr_sub_program_id', 'start_date', 'end_date', 'category')
			));
			$listIdProjects = Set::classicExtract($projects, '{n}.Project.id');
			
			$listIdProgram = Set::classicExtract($projects, '{n}.Project.project_amr_program_id');
			$listIdSubProgram = Set::classicExtract($projects, '{n}.Project.project_amr_sub_program_id');
			$project_program_refer = Set::combine($projects, '{n}.Project.id','{n}.Project.project_amr_program_id');
			$project_subprogram_refer = Set::combine($projects, '{n}.Project.id','{n}.Project.project_amr_sub_program_id');
			$project_name = Set::combine($projects, '{n}.Project.id','{n}.Project.project_name');
			$project_program = $this->ProjectAmrProgram->find('list', array(
				'recursive' => -1,
				'conditions' => array(
					'company_id' => $this->employee_info['Company']['id'],
					'id' => $listIdProgram,
			...
			$project_subprogram = $this->ProjectAmrSubProgram->find('list', array(
				'recursive' => -1,
				'conditions' => array(
					'id' => $listIdSubProgram,
				),
			...
			if(empty($arrGetUrl['ajax'])){
				$group_fields = array('project_id', 'date');
			}else{
				$group_fields = array('model_id', 'date');
			}
			$stf_conds = array(
			...
			if(!empty($_conditions['profit_center_id'])){
				...
				$stf_conds['model'] = 'profit_center';
				$stf_conds['model_id'] = $_conditions['profit_center_id'];
				$is_stf_pc = 1;
			}else{
                $stf_conds['model'] = 'profit_center';
				$is_stf_pc = 1;
			...
			if(!empty($_conditions['employee_id'])){
				// Filter by employee
				$stf_conds['model'] = 'employee';
				$stf_conds['model_id'] = $_conditions['employee_id'];
				if(!empty($_conditions['profit_center_id'])){
					// Filter by PC and employee
					$stf_conds['model_id'] = array_merge($_conditions['employee_id'], $_conditions['profit_center_id']);
				}
				$is_stf_pc = 0;
				$is_stf_employee = 1;
				$isCheck = 1;
			}
			$getDataNotAffected = array();
			if(!empty($arrGetUrl['ajax']) && !empty($is_stf_pc)){
				// Get affected data
				// Only filter by PC
				$getDataNotAffected = $this->TmpStaffingSystem->find('all', array(
					'recursive' => -1,
					'conditions' => $stf_conds,
					'fields' => array('project_id', 'SUM(consumed) as consumed', 'date', 'SUM(estimated) as estimated', 'model_id', 'model'),
					'group'=> $group_fields,
					'order' => array('date'),
				));
                $_list_pc_id = !empty($getDataNotAffected) ? array_unique(Set::classicExtract($getDataNotAffected, '{n}.TmpStaffingSystem.model_id')) : array();
				// Get employees in profit center
				$getEmployeesAjax = array();
				$_joins = array(
					array(
						'table' => 'employees',
				...
				$_getEmployeesAjax = ClassRegistry::init('ProjectEmployeeProfitFunctionRefer')->find('list', array(
						...
						'conditions' => array(
							...
							'ProjectEmployeeProfitFunctionRefer.profit_center_id' => $_list_pc_id,
						),
						'fields' => array('employee_id', 'employee_id'),
				));
				$getEmployeesAjax = array_values(array_unique($_getEmployeesAjax));

				$stf_conds['model'] = 'employee';
				$stf_conds['model_id'] = $getEmployeesAjax;
			}
			
			$getTmpData = $this->TmpStaffingSystem->find('all', array(
				'recursive' => -1,
				'conditions' => $stf_conds,
				'fields' => array('TmpStaffingSystem.project_id', 'SUM(consumed) as consumed', 'TmpStaffingSystem.date', 'SUM(estimated) as estimated', 'model_id', 'model', 'Project.project_name'),
				'group'=> $group_fields,
				'order' => array('date'),
				'joins' => array(
					array(
						...
						'alias' => 'Project',
						'conditions' => array(
							'Project.id = TmpStaffingSystem.project_id',
						)
					)
				)
			));
            ...
            $getDatas = array();
            if(!empty($getTmpData)){
                foreach($getTmpData as $_tmpVal){
                    $getDatas[] = $_tmpVal;
                }
            }
            if(!empty($getDataNotAffected)){
                foreach($getDataNotAffected as $_tmpValAffect){
                    $getDatas[] = $_tmpValAffect;
                }
            }
            if(empty($arrGetUrl['ajax'])){
                // Order project name khi hien thi stafing project
                // Khong can order project khi tinh ajax staffing employee
                $tmp_project_name =  !empty($getDatas) ? Set::classicExtract($getDatas, '{n}.Project.project_name') : array();
                array_multisort($tmp_project_name, SORT_ASC, $getDatas);
            }
			// $getTmpData = array_merge($getTmpData, $getDataNotAffected);
			if(!empty($arrGetUrl['ajax']) || !empty($is_stf_employee)){
				$stf_employee_id =  !empty($getDatas) ? Set::classicExtract($getDatas, '{n}.TmpStaffingSystem.model_id') : array();
				$stf_employee_id = array_unique($stf_employee_id);
				// debug($stf_employee_id);exit;
				$pc_refer = array();
				$stf_pc_name = array();
				$stf_employee = array();
				if(!empty($stf_employee_id)){
					$tmp_employee = $this->Employee->find('all', array(
						'recursive' => -1,
						'conditions' => array(
							'id' => $stf_employee_id,
							'company_id' => $this->employee_info['Company']['id'],
					...
					$stf_employee =  !empty($tmp_employee) ? Set::combine($tmp_employee, '{n}.Employee.id', '{n}.Employee.fullname') : array();
					if(!empty($tmp_employee)){
						foreach($tmp_employee as $index => $value){
							if(!empty($value['Employee']['profit_center_id'])){
								$pc_refer[$value['Employee']['id']] = $value['Employee']['profit_center_id'];
							}
						}
						if(!empty($pc_refer)){
							$stf_pc_name = $this->ProfitCenter->find('list', array(
								'recursive' => -1,
								'conditions' => array(
									'id' => $pc_refer,
									'company_id' => $this->employee_info['Company']['id'],
				...
				if(!empty($getDataNotAffected)){
					$stf_pc_id =  !empty($getDataNotAffected) ? Set::classicExtract($getDataNotAffected, '{n}.TmpStaffingSystem.model_id') : array();
					if(!empty($stf_pc_id)){
						$stf_pc_affected = $this->ProfitCenter->find('list', array(
							'recursive' => -1,
							'conditions' => array(
								'id' => array_unique($stf_pc_id),
								'company_id' => $this->employee_info['Company']['id'],
							),
							'fields' => array('id', 'name'),
						));
						$stf_pc_name = $stf_pc_name + $stf_pc_affected;
					}
				}
				
			}
			$staffings = array();
			$stf_project_refer = array();
			$stf_pc_refer = array();
			$stf_employee_affect = array();
			$i = 0;
			foreach($getDatas as $index => $tmpData){
				// Order projects in sub-program/ program
				$dx = $tmpData['TmpStaffingSystem'];
				$dy = $tmpData[0];
				$project_id = $dx['project_id'];
				$program_id = $project_program_refer[$project_id];
				$sub_program_id = $project_subprogram_refer[$project_id];
				$date = $dx['date'];
				$estimated = $dy['estimated'];
				$consumed = $dy['consumed'];
				$employee_id = $dx['model_id'];
				$model = $dx['model'];
				$staf_item = array(
					 'date' => $date,
                     'validated' => $estimated,
				...
				$key_refer = !empty($sub_program_id) ? 'sub-'.$sub_program_id : $program_id;
				
				if(!isset($stf_project_refer[$key_refer][$project_id])){
					$stf_project_refer[$key_refer][$project_id] = array(
						'id' => $project_id,
						'isActivity' => 1,
						'employee_id' => $employee_id,
						'isManager' => true,
						'family' => $project_program[$program_id],
						'subfamily' => !empty($project_subprogram[$sub_program_id]) ? $project_subprogram[$sub_program_id] : '',
						'family_id' => $program_id,
						'subfamily_id' => $sub_program_id,
						'name' => $project_name[$project_id],
						'data' => array(),
					);
					$stf_project_refer[$key_refer][$project_id]['data'][$date] = $staf_item;
				}else{
					if(isset($stf_project_refer[$key_refer][$project_id][$date])){
						$stf_project_refer[$key_refer][$project_id]['data'][$date]['validated'] += $estimated;
						$stf_project_refer[$key_refer][$project_id]['data'][$date]['consumed'] += $consumed;
					}else{
						$stf_project_refer[$key_refer][$project_id]['data'][$date] = $staf_item;
					}
				}
				if(!empty($arrGetUrl['ajax'])){
					$profit_center_id = $employee_id;
					$stf_employee_name = !empty($stf_employee[$employee_id]) ? $stf_employee[$employee_id] : __('Not affected to a resource', true);
					if($model == 'profit_center'){
						$employee_id = 999999999;
					}
					if(!isset($stf_employee_affect[$date])){
						$stf_employee_affect[$date] = 0;
					}
					if($model == 'profit_center'){
						$consumed = 0;
						$staf_item['consumed'] = 0;
					}elseif(!empty($pc_refer[$employee_id])){
						$profit_center_id = $pc_refer[$employee_id];
						$stf_employee_affect[$date] += $estimated;
					}
					if(!isset($stf_pc_refer[$profit_center_id][$employee_id])){
						$stf_pc_refer[$profit_center_id][$employee_id] = array(
							...
							'name' => $stf_employee_name,
							'data' => array()
						);
						$stf_pc_refer[$profit_center_id][$employee_id]['data'][$date] = $staf_item;
					}else{
						if(isset($stf_pc_refer[$profit_center_id][$employee_id]['data'][$date])){
							$stf_pc_refer[$profit_center_id][$employee_id]['data'][$date]['validated'] += $estimated;
							$stf_pc_refer[$profit_center_id][$employee_id]['data'][$date]['consumed'] += $consumed;
						}else{
							$stf_pc_refer[$profit_center_id][$employee_id]['data'][$date] = $staf_item;
						}
					}
					if($model == 'profit_center'){
						$stf_pc_refer[$profit_center_id][$employee_id]['data'][$date]['validated'] -= $stf_employee_affect[$date];
					}
				}
			}
			$flag_staf_project = array();
			$estiamtedAffected = array();
			$consumedAffected = array();
			...
			foreach($getDatas as $index => $tmpData){
				...
				if(empty($arrGetUrl['ajax'])){
					// Staffing project
					$sub_program_id = !empty($sub_program_id) ? $sub_program_id : '';
					if(!empty($is_stf_employee)){
						$employee_key='employee-'.$employee_id;
						$presenseResources[] = $employee_id;
						$program_key = 'family-'.$employee_id.'-'.$program_id;
						$sub_program_key = 'subfamily-'.$employee_id.'-'.$program_id.'-'.$sub_program_id;
					}else{
						$program_key='family-'.$program_id;
						$sub_program_key='subfamily-'.$program_id.'-'.$sub_program_id;
					}
					
					if(!empty($is_stf_employee)){
						$stf_employee_name = !empty($stf_employee[$employee_id]) ? $stf_employee[$employee_id] : __('No name', true);
						if(!isset($staffings[$employee_key])){
							$staffings[$employee_key] = array(
								'id' => $employee_key,
								'isEmployee' => 1,
								'isManager' => true,
								'employee_id' => $employee_id,
							...
							$staffings[$employee_key]['data'][$date] = $staffing_item;
						}else{
							if(isset($staffings[$employee_key]['data'][$date])){
								$staffings[$employee_key]['data'][$date]['validated'] += $estimated;
								$staffings[$employee_key]['data'][$date]['consumed'] += $consumed;
							}else{
								$staffings[$employee_key]['data'][$date] = $staffing_item;
							}
						}
					...
					// Staffing program
					if(!isset($staffings[$program_key])){
						$staffings[$program_key] = array(
							'id' => $program_key,
							'employee_id' => $employee_id,
							'isManager' => true,
							'isFamily' => 1,
							...
							'family_id' => -1,
							'subfamily_id' => -1,
							'name' => $project_program[$project_program_refer[$project_id]],
							'data' => array()
						);
						$staffings[$program_key]['data'][$date] = $staffing_item;
						// Staffing projects in Program
						$program_project_refer = !empty($stf_project_refer[$program_id]) ? $stf_project_refer[$program_id] : array();
						if(!empty($program_project_refer)){
							$staffings = array_merge($staffings, array_values($program_project_refer));
						}
					}else{
						if(isset($staffings[$program_key]['data'][$date])){
							$staffings[$program_key]['data'][$date]['validated'] += $estimated;
							$staffings[$program_key]['data'][$date]['consumed'] += $consumed;
						}else{
							$staffings[$program_key]['data'][$date] = $staffing_item;
						}
					}
					
					// Staffing sub program
					if(!isset($staffings[$sub_program_key])){
						$staffings[$sub_program_key] = array(
							'id' => $sub_program_key,
							'employee_id' => $employee_id,
							'isManager' => true,
							'isSubfamily' => 1,
							'subfamily' => false,
							'family' => $project_program[$project_program_refer[$project_id]],
							'family_id' => $program_id,
							'subfamily_id' => -1,
							'name' => !empty($project_subprogram[$project_subprogram_refer[$project_id]]) ? $project_subprogram[$project_subprogram_refer[$project_id]] : 'No name sub program',
							'data' => array()
						);
						unset($staffing_item['date']);
						$staffings[$sub_program_key]['data'][$date] = $staffing_item;
						// Staffing projects in SubProgram
						$subprogram_project_refer = !empty($stf_project_refer['sub-'.$sub_program_id]) ? $stf_project_refer['sub-'.$sub_program_id] : array();
						if(!empty($subprogram_project_refer)){
							$staffings = array_merge($staffings, array_values($subprogram_project_refer));
						}
					}else{
						if(isset($staffings[$sub_program_key]['data'][$date])){
							$staffings[$sub_program_key]['data'][$date]['validated'] += $estimated;
							$staffings[$sub_program_key]['data'][$date]['consumed'] += $consumed;
						}else{
							$staffings[$sub_program_key]['data'][$date] = $staffing_item;
						}
					}
					...
					// Staffing PC
                    // if(!isset($estiamtedAffected[$project_id][$date])) $estiamtedAffected[$project_id][$date] = 0;
					if($model == 'profit_center'){
						$profit_center_id = $employee_id;
                        // debug( $profit_center_id);
                        $consumed = 0;
                        $staffing_item['consumed'] = 0;
					}else{
						$profit_center_id = !empty($pc_refer[$employee_id]) ? $pc_refer[$employee_id] : $employee_id;
                        // Set estiamted of employee = 0
                        // in this funciton only get estimated of PC
                        $estimated = 0;
                        $staffing_item['estimated'] = 0;

                        // $estiamtedAffected[$project_id][$date] += $estimated;
					}
					$key_model = 'profit_center-';
                    if(!isset($staffings[$key_model . $profit_center_id])){
                        $pc_name = !empty($stf_pc_name[$profit_center_id]) ? 'PC / ' . $stf_pc_name[$profit_center_id] : __('Not affected to a resource', true);
                        $staffings[$key_model . $profit_center_id] = array(
                            'id' => $key_model . $profit_center_id,
                            'employee_id' => -1,
                            'isManager' => false,
                            'isEmployee' => false,
                            'family' => false,
                    ...
                    if(!isset($staffings[$key_model . $profit_center_id]['data'][$date])){
                        $staffings[$key_model . $profit_center_id]['data'][$date]['date'] = $date;
                        if(!empty($stf_pc_refer[$profit_center_id]) && $profit_center_id != '999999999'){
                            // Get data employee of this PC
                            $staffings = array_merge($staffings, array_values($stf_pc_refer[$profit_center_id]));
                            unset($stf_pc_refer[$profit_center_id]);
                        }
                    }  
                    if($model == 'profit_center'){
                        // Only get estiamted for PC
                        if(isset($staffings[$key_model . $profit_center_id]['data'][$date]['validated'])){
                            $staffings[$key_model . $profit_center_id]['data'][$date]['validated'] += $estimated;
                        }else{
                            $staffings[$key_model . $profit_center_id]['data'][$date]['validated']  = $estimated;
                        }
                    }else{
                        // consumed PC = sumary consumed employee
                        if(isset($staffings[$key_model . $profit_center_id]['data'][$date]['consumed'])){
                            $staffings[$key_model . $profit_center_id]['data'][$date]['consumed'] += $consumed;
                        }else{
                            $staffings[$key_model . $profit_center_id]['data'][$date]['consumed']  = $consumed;
                        }
                    }
                    // if($model == 'profit_center'){
					// 	$staffings[$key_model . $profit_center_id]['data'][$date]['validated'] -= $estiamtedAffected[$project_id][$date];
						// $staffings[$key_model . $profit_center_id]['data'][$date]['consumed'] -= $consumedAffected[$project_id][$date];
					// }
			...
			if( $this->settings['show_empty_resources'] && isset($_conditions['profit_center_id']) && $is_stf_employee == 1 ){
				//get list of resource of PC
				$conds = array(
					'profit_center_id' => $_conditions['profit_center_id'],
					'NOT' => array('id' => $presenseResources)
				);
				if( !empty($_conditions['employee_id']) ){
					$conds['id'] = $_conditions['employee_id'];
				}
		...
		if(isset($arrGetUrl['aFamily']))
			...
			$this->set(compact('activities', 'showGantt', 'showType', 'display', 'staffings', 'startDateFilter', 'endDateFilter','activityType','arrGetUrl','subFamilyList','activityFilterList','employeeRorProfitCenterList','listDays','dateType','listWeeks','filterByProfile', 'budgetTeam','projects'));
			//END
...
//		}
        ...
        if(!empty($activities)){
			...
			if(!empty($getDatas)){
				...
				foreach($getDatas as $key=>$getData){
					...
					//if($typeIs==1)
					$datas[$_itemIndex]['data']['model_id'] = !empty($dx['model_id']) ? $dx['model_id'] :0;
					$datas[$_itemIndex]['data']['name'] = $dx['name'];
					$datas[$_itemIndex]['data']['family_id'] = !empty($dx['family_id']) ? $dx['family_id'] : 0;
					$datas[$_itemIndex]['data']['subfamily_id'] = !empty($dx['subfamily_id']) ? $dx['subfamily_id'] : 0; 
				}
     ...
     private function _visionsStaffingActivity($activities = null, $conditions = null, $startDateFilter = null, $endDateFilter = null, $onlyEmployee = false, $dateType = null, $is_ajax = false){
		//ini_set('memory_limit', '-1');
		$this->loadModel('Activity');
		$this->loadModel('ActivityFamily');
        $this->loadModel('TmpStaffingSystem');
        ...
        if(!empty($activities)){
            ...
            if(!empty($conditions)){
                if(!empty($conditions['employee_id'])){
                    ...
                    $getDatas = array();
                    if(!empty($_getDatasManager)){
                        foreach($_getDatasManager as $_tmpValMana){
                            $getDatas[] = $_tmpValMana;
                        }
                    }
                    if(!empty($_getDatas)){
                        foreach($_getDatas as $_tmpVal){
                            $getDatas[] = $_tmpVal;
                        }
                    }
                    if(!empty($_getDatasNotAffected)){
                        foreach($_getDatasNotAffected as $_tmpValAffect){
                            $getDatas[] = $_tmpValAffect;
                        }
                    }
                } else {
            ...
            if(empty($is_ajax)){
                // Order activity name khi hien thi activity
                // Khong can order khi tinh toan ajax staffing cho employee
                $tmp_activity_name =  !empty($getDatas) ? Set::classicExtract($getDatas, '{n}.Activity.name') : array();
			    array_multisort($tmp_activity_name, SORT_ASC, $getDatas);
            }
			if($typeIs==1){
				$datas = array();
				...
				if(!empty($getDatas)){
					...
					foreach($getDatas as $key=>$getData){
						$dx = array_merge($getData['Activity'],$getData['TmpStaffingSystem'],$getData[0]);
                        $_itemIndex = $dx['model'] == 'profit_center' ? '999999999-'.$dx['activity_id'] : $dx['model_id'].'-'.$dx['activity_id'];
                        $staf_item = array(
                            'date' => $dx['date'],
                            'validated' => $dx['estimated'],
                            'consumed' => $dx['consumed'],
                       );
                        if(!isset($datas[$_itemIndex])){
                            $datas[$_itemIndex] = array(
                                'data' => array(
                                    'model_id' => $dx['model'] == 'profit_center' ? 999999999 : $dx['model_id'],
                                    'name' => $dx['name'],
                                    'family_id' => !empty($dx['family_id']) ? $dx['family_id'] : 0,
                                    'subfamily_id' => !empty($dx['subfamily_id']) ? $dx['subfamily_id'] : 0,
                                ),
                                'val' => array()
                            );
                        }
                        if(!isset($datas[$_itemIndex]['val'][$dx['date']])){
                            $datas[$_itemIndex]['val'][$dx['date']]['date'] = $dx['date'];
                            $datas[$_itemIndex]['val'][$dx['date']]['validated'] = $dx['estimated'];
                            $datas[$_itemIndex]['val'][$dx['date']]['consumed'] = $dx['consumed'];
                            ...
                            if(isset($datas[$_itemIndex]['val'][$dx['date']])){
                                ...
                                $datas[$_itemIndex]['val'][$dx['date']] = $staf_item;
                            }
                        ...
                        if($dx['model'] == 'profit_center'){
                            ...
                            $datas[$_itemIndex]['val'][$dx['date']]['validated'] -= $datasAffected[$dx['activity_id']][$dx['date']]['estimated'];
                        }else{
			...
			else
			{
				...
				if(!empty($getDatas)){
					...
					foreach($getDatas as $key=>$getData){
						...
						//if($typeIs==1)
						$datas[$dx['activity_id']]['data']['model_id'] = !empty($dx['model_id']) ? $dx['model_id'] : 0;
						$datas[$dx['activity_id']]['data']['name'] = $dx['name'];
						$datas[$dx['activity_id']]['data']['family_id'] = !empty($dx['family_id']) ? $dx['family_id'] : 0;
						$datas[$dx['activity_id']]['data']['subfamily_id'] = !empty($dx['subfamily_id']) ? $dx['subfamily_id'] : 0; 
		...
		//IF BY DAY
		$sharedEqually = false;
		if((isset($this->params['url']['aDateType']) && $this->params['url']['aDateType']==1 || $this->params['url']['aDateType']==2) || $showMonthMultiple)
		{
			/*---------------------------------------------------------------
			if filter by day-week-month multiple (get data from Period table)
			apply algorithm shared equally workload form month to day.
	 ...
	 private function _visionsStaffingProfitCenterPC($activities = null, $conditions = null, $startDateFilter = null, $endDateFilter = null){
        ...
        if(!empty($activities)){
			...
			$listChildPC = array_unique($listChildPC);
			$getAllPC = $this->ProfitCenter-> find('all', array(
				'recursive' => -1,
				'conditions' => array( 'id' => $listChildPC ),
				'fields' => array('id', 'name', 'parent_id')
			));
			$getAllPC = !empty( $getAllPC) ? Set::combine($getAllPC, '{n}.ProfitCenter.id', '{n}.ProfitCenter') : array();
			$getAllDatas = $this->TmpStaffingSystem->find('all', array(
				'recursive' => -1,
				'conditions' => array(
					'TmpStaffingSystem.activity_id' => $listIdActivitys, 
					'date BETWEEN ? AND ?' => array($startDateFilter, $endDateFilter),
					'TmpStaffingSystem.model_id' => $listChildPC,
					'model' => 'profit_center',
					'TmpStaffingSystem.company_id' => $employeeName['company_id']
				),
				'fields'=>$_fieldsSum,
				'group'=>array('TmpStaffingSystem.date', 'TmpStaffingSystem.model_id'),
			));
			$getAllDatas = !empty($getAllDatas) ? Set::combine($getAllDatas, '{n}.TmpStaffingSystem.date', '{n}.0', '{n}.TmpStaffingSystem.model_id') : array();
			$datas = array();
            if(!empty($getAllDatas) && !empty($listChildPC)){
                foreach($listChildPC as $_pc){
					foreach( $getAllChildrenOfPC[$_pc] as $_childPC){
						if( !empty($getAllDatas[$_childPC])) {
							foreach($getAllDatas[$_childPC] as $date => $val){
								if( empty($getAllDatas[$_pc][$date]['consumed'])) $getAllDatas[$_pc][$date]['consumed'] = 0;
								if( empty($getAllDatas[$_pc][$date]['estimated'])) $getAllDatas[$_pc][$date]['estimated'] = 0;
								$getAllDatas[$_pc][$date]['consumed'] += $val['consumed'];
								$getAllDatas[$_pc][$date]['estimated'] += $val['estimated'];
							}
						}
					}
					$_getData = isset( $getAllDatas[$_pc]) ? $getAllDatas[$_pc] : array();
					$datas[$_pc] = array();
					foreach($_getData as $_m=>$getData){
						$dx = $getData;
						$datas[$_pc]['val'][$_m]['date'] = $_m;
						$datas[$_pc]['val'][$_m]['validated'] = $dx['estimated'];
						$datas[$_pc]['val'][$_m]['consumed'] = $dx['consumed'];
						$datas[$_pc]['val'][$_m]['remains'] = round($dx['estimated'] - $dx['consumed'], 2);
					}
					$datas[$_pc]['data']['name'] 	= !empty( $getAllPC[$_pc]['name'])		? $getAllPC[$_pc]['name'] 		: '';
					$datas[$_pc]['data']['parent'] 	= !empty($getAllPC[$_pc]['parent_id'])	? $getAllPC[$_pc]['parent_id'] 	: '';
                }
            }
        }
		//GET LEVEL FOR PC
        ...
        if(!empty($datas)){
            foreach($datas as $id => $_data){
                ...
                $staffings[] = array(
					...
					'parent' => $parent,
                    'name' => !empty($_data['data']['name']) ? $_data['data']['name'] : __('Not Affected', true),
                    'data' => $data
                );
            }
        }
