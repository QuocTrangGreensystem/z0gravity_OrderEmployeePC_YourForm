...
class EmployeesController extends AppController {
     ...
     * @access public
     */
    var $uses = array('ProjectFunction', 'ProfitCenter', 'ProjectTeam', 'Employee', 'Company', 'CompanyEmployeeReference', 'City', 'Country', 'ProjectFunctionEmployeeRefer', 'ProjectEmployeeProfitFunctionRefer', 'Profile', 'ProfileProjectManagerDetail', 'External');
     ...
     * @access public
     */
    var $components = array('ControllerList', 'MultiFileUpload', 'PImage', 'LogSystem', 'SlickExporter');
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('recovery', 'token_sent', 'change_password',"login", "logout", "list_acos", "get_company_role", "otp_authentication");
		App::import('Vendor', 'TwoFactorAuth');
    ...
    function index() {
        ...
        if (isset($this->employee_info['Role']['id'])
            ...
            if ($this->is_sas != 1) {
                $references = $this->Employee->CompanyEmployeeReference->find('all', array(
                    'group' => 'employee_id',
                    'fields' => array( 'CompanyEmployeeReference.id', 'role_id', 'company_id', 'employee_id', 'see_budget' ),
                    'recursive' => 1,
                    'conditions' => array(
						'OR' => array(
				...
				$canAddMoreMax = $this->Employee->canAddMoreMax($this->employee_info['Company']['id']);
        ...
        $employees = $this->Employee->find('all', array(
            'fields' => array('id', 'first_name', 'last_name', 'email', 'work_phone', 'mobile_phone', 'identifiant', 'code_id', 'id3', 'id4', 'id5', 'id6', 'contract_type_id','update_your_form','create_a_project','delete_a_project','change_status_project','can_communication','can_see_forecast','actif','external','email_receive','activate_copy','auto_timesheet','auto_absence','auto_by_himself', 'update_budget','can_communication','can_see_forecast'),
            'contain' => array(
                'City' => array('name'),
                'Country' => array('name'),
		...
		$employees = !empty($employees) ?  Set::combine($employees, '{n}.Employee.id', '{n}') : array();
		...
		$checkControlResource = $this->CompanyEmployeeReference->find('first', array(
                    'fields' => array('employee_id','control_resource' ),
                    'recursive' => -1,
                    'conditions' => array('employee_id' => $this->employee_info['Employee']['id'])
                ));
        $this->loadModel('Menu');
		$company_id = $this->employee_info['Company']['id'];
		$menu_project_livrable = $this->Menu->find('first', array(
			'recursive' => -1,
			'conditions' => array(
				'company_id' => $company_id, 
				'model' => 'project', 
				'controllers' => array('project_livrables'),
				'functions' => array('index')
			),
			'fields' => array('controllers','functions','name_eng','name_fre')
		));
		$this->loadModel('Company');
		
		$licensesDate = 0;
		$show_alert_billing = 0;
		if(!empty($company_id)){
			$day_alert_billing = $this->Company->find('first', array(
				'recursive' => -1,
				'conditions' => array(
					'id' => $company_id, 
				),
				'fields' => array('day_alert_billing', 'day_licence','actif_max','no_add_more_max')
			));
			if(!empty($day_alert_billing['Company']['day_alert_billing'])){
				if($day_alert_billing['Company']['day_alert_billing'] < time()){
					$show_alert_billing = 1;
					$licensesDate = !empty( $day_alert_billing['Company']['day_licence']) ? date('d-m-Y', $day_alert_billing['Company']['day_licence']) : date('d-m-Y', $day_alert_billing['Company']['day_alert_billing']);
				}
			}
		}
		if(isset($day_alert_billing)){$this->set(compact('day_alert_billing'));}
        $this->set(compact('employee_profits', 'employees', 'roles', 'companies', 'references', 'reference_ids', 'profit_centers', 'profileName', 'employeeProfile','checkControlResource','menu_project_livrable', 'licensesDate', 'show_alert_billing', 'canAddMoreMax'));
    }
     ...
     */
    function add() {
		if (!empty($this->companyConfigs['new_edit_employee']) && empty($_POST)) {
    ...
    function edit($id = null, $company_id = null) {
		if (!empty($this->companyConfigs['new_edit_employee'])) {
            if (!((isset($this->params['url']['noredirect']) && $this->params['url']['noredirect']))) {
                $_controller = !empty($this->params['controller']) ? $this->params['controller'] : '';
                $_controller_preview = trim(str_replace('_preview', '', $_controller), ' \t\n\r\0\x0B_') . '_preview';
                $_action = !empty($this->params['action']) ? $this->params['action'] : 'index';
        ...
        $employee_date = $this->Employee->find('first', array(
            'fields' => array('start_date', 'end_date', 'company_id', 'hour', 'minutes'),
            'recursive' => -1,
            'conditions' => array('Employee.id' => $id)
        ));
        ...
        if (!empty($this->data)) {
            ...
            $this->data['Employee']['can_communication'] = $this->data['can_communication'];
            $this->data['Employee']['can_see_forecast'] = $this->data['can_see_forecast'];
        ...
        $this->set(compact('sumYearOfMutis', 'sumMonthOfMutis', 'cities', 'countries', 'employee_info', 'id', 'company_id', 'contract_types', 'employeeLogin', 'profiles', 'activateProfile', 'employee_multi', 'dates', 'count', 'employee_date', 'first', 'days', 'externals', 'profile_name', 'list_profile_name'));
    }
    /*------------------------------
    @author : ViN
    @date : 23/06/2015
    ...
    function delete($id = null) {
        $chuanbixoa = $this->Employee->find('first', array('conditions' => array('Employee.id' => $id)));
        $result = array(
            'result' => false,
            'message' => ''
        );
        if (!$chuanbixoa) {
            $result['message'] = __('Resource not found', true);
            die(json_encode($result));
        }
        $role = $chuanbixoa['CompanyEmployeeReference'][0]['role_id'];
        if( $this->is_pm && $role == 2 ){
            $result['message'] = __('You do not have permission to delete this resource', true);
            die(json_encode($result));
        }
        // security
		$this->loadModels('ProjectEmployeeManager', 'LogSystem', 'ProjectLivrableComment' , 'ProjectLivrableActor');
		$listProjectOfEmps = $this->ProjectEmployeeManager->find('list', array(
			'recursive' => -1,
			'fields' => array('project_id', 'project_id'),
			'conditions' => array('project_manager_id' => $id, 'is_profit_center' => 0)
		));
		// debug($listProjectOfEmps);
		$dataLog = $this->LogSystem->find('list', array(
			'recursive' => -1,
			'fields' => array('LogSystem.model_id', 'Project.project_name'),
		...
		$dataLivCmt = $this->ProjectLivrableComment->find('list', array(
			'recursive' => -1,
			'fields' => array("Project.id", "Project.project_name"),
			'conditions' => array('employee_id' => $id),
			'joins' => array(
				array(
                    'table' => 'project_livrables',
                    'alias' => 'ProjectLivrable',
                    'conditions' => array(
                        'ProjectLivrable.id = ProjectLivrableComment.project_livrable_id'
                    )
                ),
				array(
                    'table' => 'projects',
                    'alias' => 'Project',
                    'conditions' => array(
                        'Project.id = ProjectLivrable.project_id'
                    )
                )
			)
		));
		// debug($dataLivCmt);exit;
		$dataLivAct = $this->ProjectLivrableActor->find('list', array(
			'recursive' => -1,
			'fields' => array("Project.id", "Project.project_name"),
			'conditions' => array('employee_id' => $id),
			'joins' => array(
				array(
                    ...
                    'alias' => 'ProjectLivrable',
                    'conditions' => array(
                        'ProjectLivrable.id = ProjectLivrableActor.project_livrable_id'
                    )
                ),
				array(
                    'table' => 'projects',
		...
		// debug($dataLivAct);exit;
		
        $project_list = $this->Employee->Project->find('list', array(
			'fields' => array("Project.id", "Project.project_name"),
			'conditions' => array(
				'OR' =>array(
					array('Project.project_manager_id' => $id),
					array('Project.id' => $listProjectOfEmps)
				)
			)
		));
		// debug($project_list);exit;
        $name_chuanbixoa = $chuanbixoa['Employee']['fullname'];
        $checkCompany = $this->CompanyEmployeeReference->find('first',array('conditions'=>array('employee_id'=>$id),'fields'=>'company_id'));
        $companyParent = $this->CompanyEmployeeReference->find('list', array(
        ...
        // check task assign, activity request
        $this->loadModels('ProjectTask', 'ActivityTask', 'ActivityRequest');
        $this->ActivityRequest->virtualFields['month'] = 'FROM_UNIXTIME(`date`, "%m-%Y")';
        $this->ActivityRequest->virtualFields['sum_consume'] = 'SUM(IF(status = 2, `value`, 0))';
        $this->ActivityRequest->virtualFields['sum_wait'] = 'SUM(IF(status != 2, `value`, 0))';
        $requests = $this->ActivityRequest->find('all', array(
            'recursive' => -1,
            'conditions' => array(
                'employee_id' => $id
            ),
            'fields' => array('month', 'sum_consume', 'sum_wait'),
            'group' => array('month')
        ));
        $tasks = $this->ProjectTask->find('all', array(
            'fields' => array('ProjectTask.id', 'ProjectTask.task_title', 'ProjectTask.project_id'),
            'recursive' => -1,
            'conditions' => array(
                'Refer.reference_id' => $id,
                'Refer.is_profit_center' => 0
            ),
            'joins' => array(
                array(
                    'table' => 'project_task_employee_refers',
                    'alias' => 'Refer',
                    'conditions' => array(
                        'Refer.project_task_id = ProjectTask.id'
                    )
                )
            )
        ));
        $tasks2 = $this->ActivityTask->find('all', array(
            'fields' => array('ActivityTask.id', 'ActivityTask.name', 'ActivityTask.activity_id'),
            'recursive' => -1,
            'conditions' => array(
                'Refer.reference_id' => $id,
                'Refer.is_profit_center' => 0
            ...
            'joins' => array(
                array(
                    ...
                    'alias' => 'Refer',
                    'conditions' => array(
                        'Refer.activity_task_id = ActivityTask.id'
                    )
                )
            )
        ));
        $continue = true;
        if( $id == $this->employee_info['Employee']['id'] ){
            $result['message'] = __('Cannot delete your own account', true);
        } else {
            if( !empty($dataLog) ){
                $continue = false;
            ...
            if( !empty($dataLivCmt) ){
                $continue = false;
                $result['dataLivCmt'] = $dataLivCmt;
            }
            if( !empty($dataLivAct) ){
                $continue = false;
                $result['dataLivAct'] = $dataLivAct;
            }
            if( !empty($requests) ){
                $continue = false;
                $result['requests'] = $requests;
            }
            if( !empty($tasks) || !empty($tasks2) ){
                $continue = false;
                $result['tasks'] = $tasks;
                $result['tasks2'] = $tasks2;
            }
            ...
            if( $continue ){
                if($checkCompany['CompanyEmployeeReference']['company_id'] == $this->employee_info["Company"]["id"] || in_array($id, $companyParent)){
                    if ($this->Employee->delete($id)) {
                        $this->writeLog('', $this->employee_info, sprintf('Delete resource `%s %s`', $chuanbixoa["Employee"]["first_name"], $chuanbixoa["Employee"]["last_name"]), $checkCompany['CompanyEmployeeReference']['company_id']);
						// Delete data of tables when delete employee successfully
						$this->CompanyEmployeeReference->deleteAll(array('CompanyEmployeeReference.employee_id' => $id),false);
						$this->loadModels('UserDefaultView', 'UserLastUpdated', 'UserStatusView', 'UserView', 'UserStatusViewActivity', 'UserStatusViewSaleDeal', 'UserStatusViewSale', 'ProjectTaskTxtRefer', 'ProjectTaskAttachmentView', 'ProjectFinancePlusAttachmentView', 'ActivityRequestCopy');
						$this->UserDefaultView->deleteAll(array('UserDefaultView.employee_id' => $id),false);
						$this->UserLastUpdated->deleteAll(array('UserLastUpdated.employee_id' => $id),false);
						$this->UserStatusView->deleteAll(array('UserStatusView.employee_id' => $id),false);
						$this->UserView->deleteAll(array('UserView.employee_id' => $id),false);
						$this->UserStatusViewActivity->deleteAll(array('UserStatusViewActivity.employee_id' => $id),false);
						$this->UserStatusViewSaleDeal->deleteAll(array('UserStatusViewSaleDeal.employee_id' => $id),false);
						$this->UserStatusViewSale->deleteAll(array('UserStatusViewSale.employee_id' => $id),false);
						$this->ProjectTaskTxtRefer->deleteAll(array('ProjectTaskTxtRefer.employee_id' => $id),false);
						$this->ProjectTaskAttachmentView->deleteAll(array('ProjectTaskAttachmentView.employee_id' => $id),false);
						$this->ProjectFinancePlusAttachmentView->deleteAll(array('ProjectFinancePlusAttachmentView.employee_id' => $id),false);
						$this->ActivityRequestCopy->deleteAll(array('ActivityRequestCopy.employee_id' => $id),false);
                        // Delete: Validator of Project Evolution, Actors of Project Deliverable
                        $result['message'] = sprintf(__('Employee %s has been deleted', true), '<strong>' . $name_chuanbixoa . '</strong>');
                        $result['result'] = true;
						$new_path = IMAGES . 'avatar' . DS ;
						if (file_exists($new_path . $id . '_avatar.png' )) {
							@unlink($new_path . $id . '_avatar.png');
						}
						if (file_exists($new_path . $id . '.png' )) {
							@unlink($new_path . $id . '.png');
						}
                    }
                } else {
                    $result['message'] = __('You do not have permission to delete this resource', true);
                ...
                $result['message'] = __('Cannot delete resource', true);
            }
        }
        die(json_encode($result));
    }
    ...
    function login_bk() {
        ...
        $this->layout = "login";
        $this->getVersion();
        $this->getAttachmentImage();
        if ($this->Auth->user()) {
            $employee_session = $this->Auth->user();
            $employee_all_info = $this->CompanyEmployeeReference->find("all", array("conditions" => array("Employee.email" => $this->Auth->user('email'))));
            if (empty($employee_all_info) || count($employee_all_info) > 1 || $this->Auth->user('is_sas') == 1 || (!empty($employee_session['Employee']['is_enable_new_design']) && $employee_session['Employee']['is_enable_new_design'] == 1)) {
     ...
     * @return void
     * @access public
	 * Neu edit function nay, can edit tuong tu o function ss0_logins/buildUserSession
     */
    function login() {
        $this->layout = "login";
        $this->getVersion();
        $this->getAttachmentImage();
		$sso_companies = $all_companies = array();
        if ($this->Auth->user()) {
            $employee_session = $this->Auth->user();
            $employee_all_info = $this->CompanyEmployeeReference->find("all", array("conditions" => array("Employee.email" => $this->Auth->user('email'))));
			$count_company = count($employee_all_info);
            $employee_all_info = !empty( $employee_all_info[0]) ? $employee_all_info[0] : '';
            if (empty($employee_all_info) || $count_company > 1) {
				if( !empty($this->params['url']['continue'])){
					$this->redirect("/employees/get_company_role?continue=".urlencode($this->params['url']['continue']));
				}
                $this->redirect("/employees/get_company_role");
            }
            if( $this->_is('POST') )$this->writeLog($this->data, $employee_all_info, 'Logon to system');
            ...
            $employee_all_info['Employee']['profit_center_id'] = $myPc['ProjectEmployeeProfitFunctionRefer']['profit_center_id'];
            $myColor = $this->get_company_colors($employee_all_info['Company']['id']);
            $employee_all_info['Color'] = $myColor['Color'];
			$this->loadModel('CompanyConfig');
			$company_two_factor_auth = $this->CompanyConfig->find('first', array(
				'recursive' => -1,
				'conditions' => array(
					'company' => $employee_all_info['Company']['id'],
					'cf_name' => 'company_two_factor_auth',
				),
				'fields' => array('cf_value')
			));
			if(!empty($company_two_factor_auth) && !empty($company_two_factor_auth['CompanyConfig']['cf_value']) && !empty($employee_all_info["Employee"]['two_factor_auth'])){
				$this->Session->write('Auth.employee_otp', $employee_all_info);
				$cookie_otp = $this->cookie_two_factor_auth();
				if(!$cookie_otp){
					 $this->redirect("otp_authentication");
				}
            ...
            $this->ApiKey->generate($this->Session);			
			// Saved last login
			$this->Employee->id = $employee_all_info['Employee']['id'];
			$this->Employee->saveField('last_login', time());
			$this->saveLastLogin($employee_all_info);
            $this->loadModel('HistoryFilter');
            $check = $this->HistoryFilter->find('first', array(
                'recursive' => -1,
                'conditions' => array(
			...
			$this->loadModels('SsoLogin', 'CompanyConfig');
			$all_companies = $this->Company->find('list', array(
				'recursive' => -1,
				'fields' => array('Company.id', 'Company.company_name')
			));
			$sso_companies = $this->CompanyConfig->find('list', array(
				'recursive' => -1,
				'conditions' => array(
					'company' => array_keys($all_companies),
					'cf_name' => 'enable_sso',
					'cf_value' => 1,
				),
				'fields' => array('company', 'company')
			));
			$sso_companies = !empty( $sso_companies) ? array_values( $sso_companies) : array();
			
		}
		$this->set(compact('sso_companies', 'all_companies'));
    }
	private function get_company_colors($company_id=null){
		if( $company_id == null) return;
		return ClassRegistry::init('Color')->find('first',array(
			'recursive' => -1,
			'conditions' => array(
				'company_id' => $company_id,
			),
    ...
    function get_company_role() {
        $this->layout = "login";
        $this->loadModels('Color', 'HistoryFilter', 'CompanyConfig');
        $employee_info = $this->Auth->user();
         if (empty($employee_info)) {
             $this->redirect("/login");
         }
        $role_admin = $this->Employee->CompanyEmployeeReference->find("first", array(
            "recursive" => -1,
            "conditions" => array(
                "CompanyEmployeeReference.employee_id" => $employee_info['Employee']['id'],
                "CompanyEmployeeReference.role_id" => 2
            ),
            'fields' => 'employee_id'
        ));
        
        $this->set('role_admin',$role_admin);
        ...
        if (!empty($this->data)) {
			$continue = !empty($this->data["Employee"]['continue']) ? $this->data["Employee"]['continue'] : 0;
            if (isset($this->data["Employee"]["is_sas"]) && $this->data["Employee"]["is_sas"] == 1 && empty($this->data["Employee"]["company_role"])) {
				...
				// Saved last login
				$this->Employee->id = $employee_info['Employee']['id'];
				$this->Employee->saveField('last_login', time());
				$this->saveLastLogin($employee_all_info);
                $this->ApiKey->generate($this->Session);
                ...
                if (!empty($this->data["Employee"]["company_role"])) {
                    if( $employee_info["Employee"]["is_sas"] == 1) {
						...
						/* Vi mot so employee null nhung van co data trong bang CompanyEmployeeReference nen phai them dieu kien:
						 * employee co data: "Employee.company_id" => $company['Company']['id'],
						 */
                        $admin_of_company = $this->Employee->CompanyEmployeeReference->find("first", array(
                            "recursive" => -1,
                            "conditions" => array(
                                "Employee.company_id" => $company['Company']['id'],
                            ...
                            'fields' => 'CompanyEmployeeReference.employee_id'
                        ));
						...
						$login_employee = array_merge($info_sas_login_to_company,$company,$employee_info);
                        ...
                        // update new design field in color table
                        $color = $this->get_company_colors($company['Company']['id']);
                        $color['Color']['is_new_design'] = !empty($this->data['Employee']['design']) ? 1 : 0;
                        $this->data['Color']['is_new_design'] = !empty($this->data['Employee']['design']) ? 1 : 0;
                        ...
                        $color = $this->get_company_colors($employee_all_info['Company']['id']);
                        $color['Color']['is_new_design'] = !empty($this->data['Employee']['design']) ? 1 : 0;
                        $this->data['Color']['is_new_design'] = !empty($this->data['Employee']['design']) ? 1 : 0;
						
                        // update session
                        $employee_all_info['Color'] = $color['Color'];
						$login_employee = $employee_all_info;
                    }
                    $employee_all_info["Employee"]["is_sas"] = 0;
					$company_two_factor_auth = $this->CompanyConfig->find('first', array(
						'recursive' => -1,
					...
					$this->Employee->saveField('last_login', time());
					$this->saveLastLogin($employee_all_info);
                    $this->writeLog($this->data, $login_employee, sprintf('Logon as %s to %s', $employee_all_info["Role"]["desc"], $employee_all_info['Company']['company_name']));
                    $check = $this->HistoryFilter->find('first', array(
                        'recursive' => -1,
                        'conditions' => array(
                            'path' => 'rollback_url_employee_when_login',
                    ...
                    if(!empty($check)){
                        $url = $check['HistoryFilter']['params'];
                        // if(!$this->data['Color']['is_new_design'] && strpos($url, "_preview")){
                            // $this->redirect("/index");
                        // }
                        if(strpos($url, "employees/get_company_role")){
                            $this->redirect("/index");
                        }
                        $this->redirect($url);
	...
	private function saveLastLogin($employee){
		$this->loadModels('EmployeeLastLogin', 'CompanyConfig');
		$employee = $this->Session->read('Auth.employee_info');
		$last_login = $this->EmployeeLastLogin->find('first', array(
			'recursive' => -1,
			'conditions' => array(
		...
		if( !empty($last_login['EmployeeLastLogin']['current_key'])){
			$this->EmployeeLastLogin->id = $last_login['EmployeeLastLogin']['id'];
			$this->EmployeeLastLogin->saveField('last_login', time());
			$current_key = $last_login['EmployeeLastLogin']['current_key'];
			$cookie_key = $this->Cookie->read('last_login');
			if( !empty( $employee['Company'] ) && ($current_key != $cookie_key)){
				// Sendmail
				$company_two_factor_auth = $this->CompanyConfig->find('first', array(
					'recursive' => -1,
					'conditions' => array(
				...
				if( empty($company_two_factor_auth['CompanyConfig']['cf_value'])){
					...
					$browser = AgentParser::getBrowserInfo($_SERVER['HTTP_USER_AGENT']);
					$this->set(compact('fullname', 'browser', 'realIP'));
					//_sendEmail($to, $subject, $element, $debug = false, $cc = array(), $fileAttach = null) {
					$this->_sendEmail($employee['Employee']['email'], sprintf(__('Security message and information message z0 Gravity for %s/%s', true), $fullname, $company_name), 'login_other_device_warning', false, array('warning@globalsi.fr'));
				}
				// write new key
				$cookie_key = Security::hash(time() . $employee['Employee']['id'] . $employee['Company']['id']);
				$this->EmployeeLastLogin->saveField('current_key', $cookie_key);
			}
			// renew
			$this->Cookie->write('last_login', $cookie_key, true, '+1 month');
		}else{
			$cookie_key = Security::hash(time() . $employee['Employee']['id'] . $employee['Company']['id']);
			$this->EmployeeLastLogin->create();
			$this->EmployeeLastLogin->save(array(
				'employee_id' => $employee['Employee']['id'],
				'last_login' => time(),
				'current_key' => $cookie_key,
				'updated' => time(),
			));
			$this->Cookie->write('last_login', $cookie_key, true, '+1 month');
		}
    ...
    function logout() {
        ...
        if(!empty($_SERVER['HTTP_REFERER']) && $employeeId){
            $check = $this->HistoryFilter->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'path' => 'rollback_url_employee_when_login',
    ...
    function save_file_import() {
        if (!empty($this->data)) {
            ...
            if ($task === 'do') {//import
                ...
                $complete = 0;
                //$password = md5('pm123');
				$this->loadModel('EmployeeDefaultProfile');
				$default_user_profile = $this->EmployeeDefaultProfile->find('first', array(
					'recursive' => -1,
					'conditions' => array(
						'company_id' => $this->employee_info['Company']['id'],
					),
				...
				if($default_user_profile){
					$default_user_profile = (array) @unserialize($default_user_profile['EmployeeDefaultProfile']['df_value']);
				}
                foreach ($import as $data) {
					...
					if($default_user_profile){
						$data['tjm'] = empty($data['tjm']) ? $default_user_profile['tjm'] : $data['tjm'];
						$data['capacity_by_year'] = empty($data['capacity_by_year']) ? $default_user_profile['capacity_by_year'] : $data['capacity_by_year'];
						$data['email_receive'] = $default_user_profile['email_receive'];
						$data['activate_copy'] = $default_user_profile['activate_copy'];
						$data['is_enable_popup'] = $default_user_profile['is_enable_popup'];
						$data['auto_timesheet'] = $default_user_profile['auto_timesheet'];
						$data['auto_absence'] = $default_user_profile['auto_absence'];
						$data['auto_by_himself'] = $default_user_profile['auto_by_himself'];
						$data['update_your_form'] = $default_user_profile['update_your_form'];
						$data['create_a_project'] = $default_user_profile['create_a_project'];
						$data['delete_a_project'] = $default_user_profile['delete_a_project'];
						$data['change_status_project'] = $default_user_profile['change_status_project'];
					}
					// debug($data);exit;
                    if ($this->Employee->save($data)) {
                        ...
                        foreach ((array) $data['role_id'] as $fun) {
							...
							if($default_user_profile){
								$_data['control_resource'] = $default_user_profile['control_resource'];
							}
						...
						$infoAvatar = array();
						$infoAvatar['Employee']['company_id'] = $data['company_id'];
						$infoAvatar['Employee']['id'] = $this->Employee->id;
						$infoAvatar['Employee']['first_name'] = $data['first_name'];
						$infoAvatar['Employee']['last_name'] = $data['last_name'];
						if(empty($hasAvatar['Employee']['avatar'])){
							$this->generateEmployeeAvatar($infoAvatar, true);
						}
                    }
                }
    ...
    */
        ...
        if(!empty($_FILES)){
            ...
            if (!empty($attachment)) {
                ...
                if ($this->Employee->save(array(
                    ...
                    'avatar_color' => '',
					'update_by' => $this->employee_info['Employee']['id'],
					'updated' => time(),
                    ))) {
    ...
    public function attachment($employee_id = null, $type = 'avatar_resize') {
        $this->layout = false;
        $link = '';
        $employee = $this->Employee->find("first", array(
            'recursive' => -1,
            'fields' => array('id', 'avatar', 'avatar_resize'),
            "conditions" => array('id' => $employee_id))
        );
        if(!empty($employee['Employee'][$type])){
            $link = trim($this->_getPath($this->employee_info['Company']['id'], $employee_id). $employee['Employee'][$type]);
        }else {
            $link = APP. 'webroot' . DS . 'img' . DS. 'business' . DS . 'avatar.gif';
            // unlink(APP. 'webroot' . DS . 'img' . DS. 'business' . DS . 'avatar.gif');
        }
        if (empty($link)) {
            $link = '';
        } else {
    ...
    public function avatar($employee_id = null, $type = 'avatar_resize') {
		$type =  ($type == 'small' || $type == 'avatar_resize') ? 'avatar_resize' : 'avatar';
        $this->layout = false;
        $link = '';
        $employee = $this->Employee->find("first", array(
            'recursive' => -1,
            'fields' => array('id', 'avatar', 'avatar_resize', 'first_name', 'last_name', 'avatar_color'),
            "conditions" => array('id' => $employee_id))
        );
        if(!empty($employee['Employee'][$type])){
            $link = trim($this->_getPath($this->employee_info['Company']['id'], $employee_id). $employee['Employee'][$type]);
		...
		if (!(file_exists($link) && is_file($link))){
			// Phần này để test ticket #415
            $link = trim($this->_getPath($this->employee_info['Company']['id'], $employee_id). $employee_id . '_' .$type. '.png');
			// debug( $link ); exit;
        }
		if (file_exists($link) && is_file($link)) {               
            $this->view = 'Media';
			...
			$first_name = $employee['Employee']['first_name'];
			$last_name = $employee['Employee']['last_name'];
			$this->set( compact('type', 'first_name', 'last_name', 'avatar_color'));
		}
	 ...
	 * return avatar tag: 
	 employee_id => '<a class="circle-name" title=" FULL_NAME "><img src="SRC"></a>'
	 employee_id => '<a class="circle-name" title="FULL_NAME"><span> FN </span></a>'
     */
	public function get_list_avatar($list_eployee_id = array(), $type = 'avatar_resize'){
		if( empty( $this->employee_info['Employee'])){
		...
		$employees = $this->Employee->find("all", array(
            'recursive' => -1,
            'fields' => array('id', 'avatar', 'avatar_resize', 'first_name', 'last_name', 'avatar_color'),
            "conditions" => array('id' => $list_eployee_id))
        );
		$tag = '';
		$list_eployees = array();
		foreach( $employees as $employee) {
			if(!empty($employee['Employee'][$type])){
				$tag = '<a class="circle-name" title="' .$employee['Employee']['first_name'] . ' ' . $employee['Employee']['last_name']. '"><span data-id="' . $employee['Employee']['id'] . '"><img src="' . '/img/avatar/' . $employee['Employee']['id'] . '.png" alt="' . $employee['Employee']['first_name'] . ' ' . $employee['Employee']['last_name'] . '"></span></a>';
			}else {
				$_color = isset( $employee['Employee']['avatar_color'] ) ? $employee['Employee']['avatar_color'] : '#6dabd4';
				$_style = !empty( $_color ) ? 'style="background-color: ' . $_color . ';"' : '';
				$name = substr( trim($employee['Employee']['first_name']),  0, 1) . substr( trim($employee['Employee']['last_name']),  0, 1);
				$tag = '<a class="circle-name" title=" ' .$employee['Employee']['first_name'] . ' ' . $employee['Employee']['last_name']. '" '. $_style .'><span data-id="' . $employee['Employee']['id'] . '">' . $name . '</span></a>';
			}
			$list_eployees[$employee['Employee']['id']] = array(
				'id' => $employee['Employee']['id'],
				'first_name' => $employee['Employee']['first_name'],
				'last_name' => $employee['Employee']['last_name'],
				'full_name' =>  $employee['Employee']['first_name'] . ' ' . $employee['Employee']['last_name'],
				'tag' => $tag
			);
		}
		return $list_eployees;
    ...
    public function profile($company_id = null){
        ...
        if ($this->is_sas && empty($company_id)) {
            ...
            $profiles = $this->Profile->find('all', array(
                ...
                'fields' => array('id', 'name', 'tjm', 'capacity_by_year')
            ));
			...
			$this->set(compact('company_id', 'companyName'));
        }
		$this->set(compact('profiles', 'profileValues', 'lastYear', 'nextYear'));
    }
    ...
    function check_code_id($code_id) {
        ...
        if (!empty($check)) {
			$result = Set::combine($check, '{n}.Employee.id', '{n}.0.full_name');
			echo json_encode($result);
        }
        exit;
    }
    ...
    */
	...
	/* return all background image for login screeen */
    public function getAttachmentImage(){
        ...
        // }
        // debug($cond); exit;
        $bg_logins = $this->Color->find('all',array(
            'recursive' => -1,
            'conditions' => $cond,
		...
		$sas_setting = $this->SasSetting->find('all',array(
            'recursive' => -1,
			'conditions' => array(
				'name' => array('first_text', 'last_text', 'logo_client', 'company_testimonial')
			),
            'fields' => array('id', 'name', 'value', 'content')
        ));
		
		$logo_client = array();
		$text_about = array();
		$company_testimonial = array();
			...
			foreach($sas_setting as $key => $setting){
				...
				if(!empty($setting)){
					...
					if(!empty($value['name'])){
						...
						if($value['name'] == 'company_testimonial'){
							$company_testimonial[$value['id']] = $value;
						}
		...
		$rand = rand(0, count($bg_logins)-1);
		$bg_login = !empty($bg_logins[$rand]) ? $bg_logins[$rand] : array();
        $this->set(compact('bg_login', 'text_about', 'company_testimonial', 'logo_client'));
        return true;
    }
    public function getVersion(){
        ...
        $version = !empty($version) ? $version['Version']['name'] : '8.2';
    ...
    public function historyCleanFilter(){
        $this->loadModel('HistoryFilter');
        if (!empty($_POST)){
            $path = implode("/", $_POST['path']);
            $budget_internal = $this->HistoryFilter->find('list', array(
                'recursive' => -1,
                'conditions' => array(
                    'path' => $path,
                    'employee_id' => $this->employee_info['Employee']['id'],
            ...
            $budget_internal = !empty($budget_internal) ? Set::combine($budget_internal, '{n}', '{n}.HistoryFilter.id') : array();
            if(!empty($budget_internal)){
                foreach ($budget_internal as $key => $value) {
                    $this->HistoryFilter->delete($value);
                }
                die(1);
            } 
            die(0);
    ...
    public function save_company_default_user(){
        $this->loadModel('EmployeeDefaultProfile');
		$company_id = $this->employee_info['Company']['id'];
        $default_val = $this->Employee->getDefaultProfile();
		$data = '';
		$result = 'failed';
		if( !empty( $this->data)){
			$default_user_profile = $this->EmployeeDefaultProfile->find('first', array(
			...
			if( empty($default_user_profile )){
				$this->EmployeeDefaultProfile->create();
				$default_user_profile['created'] = time();
				$default_user_profile['employee_id'] = $this->employee_info['Employee']['id'];
			}else{
				$default_user_profile = $default_user_profile['EmployeeDefaultProfile'];
			}
			$default_user_profile['company_id'] = $company_id;
			$default_user_profile['updated'] = time();
			$df_value = array();
			foreach($this->data as $val){
				if( $val['name'] != '_method' ){
					$df_value[str_replace(']','', str_replace('data[EmployeeDefault][','',$val['name']))] = $val['value'];
				}					
			}
			$df_value = serialize($df_value);
			// debug( $df_value); exit;
			$default_user_profile['df_value'] = $df_value;
			// debug( $default_user_profile); exit;
			$data = $this->EmployeeDefaultProfile->save($default_user_profile);
	...
	public function switchUpdate(){
		...
		if (isset($this->employee_info['Role']['id'])
			...
			if(!empty($_POST)){
				...
				$field = $_POST['field'];
				$value = $_POST['value'];
				if($field == 'control_resource'){
					if($this->CompanyEmployeeReference->updateAll(
						array('CompanyEmployeeReference.control_resource' => $value),	// fields
						array('CompanyEmployeeReference.employee_id' => $id)		// conditions
					)){
						$result = 1;
					}
				}else{
					$this->Employee->id = $id;
					if($this->Employee->saveField($field, $value)){
						$result = 1;
					}
				}
			}
	...
	public function update_budget(){
		...
		if (isset($this->employee_info['Role']['id'])
			...
			if(!empty($this->data)) {
				...
				$this->Employee->id = $id;
				if($this->Employee->saveField('update_budget', $update_budget)){
					if($this->CompanyEmployeeReference->updateAll(
						array('CompanyEmployeeReference.see_budget' => $see_budget),	// fields
						array('CompanyEmployeeReference.employee_id' => $id)		// conditions
					)){
						$result = true;
					}else{
						$this->Session->setFlash(__('The budget could not be saved. Please, try again.', true), 'error');
					}
				}else{
					$this->Session->setFlash(__('The budget could not be saved. Please, try again.', true), 'error');
				}
    ...
    public function export_excel_index() {
        if (!empty($this->data)) {
			...
			$view_name = Inflector::slug($view_options[$view_id]);
			
            $this->SlickExporter->init();
            $data = json_decode($this->data['data'], true);
            $this->SlickExporter
                    ->setT('Employees') //auto translate
                    ->save($data, 'Employee_'. $view_name .'.xls');
        }
        die;
    }
	public function otp_authentication(){
		$this->layout = "login";
		$this->getVersion();
		$this->loadModels('TwoFactorAuthen', 'HistoryFilter');
		
		$employee_otp = $this->Session->read('Auth.employee_otp');
		if(empty($employee_otp)){
			$this->redirect("/index");
		}
		...
		$last_item =  $this->TwoFactorAuthen->find('first', array(
			'recursive' => -1,
			'conditions' => array(
				'company_id' => $employee_otp['Company']['id'],
				'employee_id' => $employee_otp['Employee']['id'],
			),
			'fields' => array('id', 'secret_code', 'duration_expired', 'current_cookie', 'updated'),
		));
		
		$secret = '';
		$this->loadModel('CompanyConfigs');
        $_2fa_company_configs = $this->CompanyConfigs->find('list', array(
            'recursive' => -1,
            'conditions' => array(
				'company' => $employee_otp['Company']['id'], 
				'cf_name' => array('company_two_factor_auth', 'otp_duration_expired')
			),
            'fields' => array('cf_name', 'cf_value'),
        ));
		
		$otp_duration_expired = (!empty($_2fa_company_configs) && !empty($_2fa_company_configs['otp_duration_expired']))? $_2fa_company_configs['otp_duration_expired'] : 'week';
		$company_two_factor_auth = (!empty($_2fa_company_configs) && !empty($_2fa_company_configs['company_two_factor_auth'])) ? $_2fa_company_configs['company_two_factor_auth'] : 0;
		$re_show_qrcode = ( empty($last_item['TwoFactorAuthen']['current_cookie']) && ($company_two_factor_auth != 'send_mail') );
		// debug( $re_show_qrcode);
		$saved = array(
			'company_id' => $employee_otp['Company']['id'],
			'employee_id' => $employee_otp['Employee']['id'],
			'updated' => time(),
			'duration_expired' => $otp_duration_expired,
		);
		if(empty($last_item)){
			$this->TwoFactorAuthen->create();
			$secret = $tfa->createSecret();	
			$saved['created'] = time();
			$saved['secret_code'] = $secret;
		}else{
			$secret =  $last_item['TwoFactorAuthen']['secret_code'];
		}
		if(!empty($this->data)){
			$url = '/';
			$otp_code = $this->data['Employee']['otp_code'];
			$otp_mail = $tfa->getCode($secret, $last_item['TwoFactorAuthen']['updated']);
			$period_check = time();
			if($tfa->verifyCode($secret, $otp_code) === true || ($company_two_factor_auth == 'send_mail' && $otp_mail == $otp_code)){
				// Save cookie
				$duration_expired = '+1 '. $otp_duration_expired;
				$cookie_key = $employee_otp['Employee']['id'] . '_' . $employee_otp['Company']['id'];
				$cookie_value = array(
					'value' => $otp_code,
					'time' => ($company_two_factor_auth == 'send_mail') ? $last_item['TwoFactorAuthen']['updated'] : $period_check,
				);
				$this->Cookie->write('2FA_'. $cookie_key, $cookie_value, true, $duration_expired);
				if(!empty($last_item)){
					$this->TwoFactorAuthen->id = $last_item['TwoFactorAuthen']['id'];
					$this->TwoFactorAuthen->saveField('current_cookie', $otp_code);
				}
				 $this->Employee->id = $employee_otp['Employee']['id'];
				 $this->Employee->saveField('last_login', time());
				 $this->saveLastLogin($employee_otp);
				 $this->Session->write('Auth.employee_info', $employee_otp);
				 $this->Session->delete('Auth.employee_otp');
				 $this->ApiKey->generate($this->Session);
				 $this->writeLog($this->data, $employee_otp, sprintf('Logon as %s to %s', $employee_otp["Role"]["desc"],$employee_otp['Company']['company_name']));
				$url = '/index';
				if (!empty($this->params['url']['continue'])) {
					$url = $this->params['url']['continue'];
				}
				$rollback_url = $this->HistoryFilter->find('first', array(
					'recursive' => -1,
					'conditions' => array(
						'path' => 'rollback_url_employee_when_login',
						'employee_id' => $employee_otp['Employee']['id']
				...
				if(!empty($rollback_url)){
					$url = $rollback_url['HistoryFilter']['params'];
				}
				if(strpos($url, "employees/get_company_role") || strpos($url, "employees/otp_authentication")){
					$url = '/index';
				}
				$this->redirect($url);
			}else{
				$this->Session->setFlash(__('OTP wrong'), 'error');
			}
		}
		
		$url_qrcode = '';
		$fullname = $employee_otp['Employee']['first_name'] . ' ' . $employee_otp['Employee']['last_name'];
		if(!empty($company_two_factor_auth) && (empty($last_item))){
			if($this->TwoFactorAuthen->save($saved)){
				if( $company_two_factor_auth != 'send_mail' ){
					$url_qrcode = $tfa->getQRCodeImageAsDataUri('z0gravity '.$fullname, $secret);
				}else{
					$otp_send = $tfa->getCode($secret, $saved['updated']);
					$this->set('otp_send', $otp_send);
					$this->_sendEmail($employee_otp['Employee']['email'], __('[Z0Gravity] Two-factor authentication', true), 'otp');
				}
			}
		}else{
			if( $re_show_qrcode) $url_qrcode = $tfa->getQRCodeImageAsDataUri('z0gravity '.$fullname, $secret);
		}
		if(!empty($last_item)){
			$otp_send =  $tfa->getCode($secret, $last_item['TwoFactorAuthen']['updated']);
			$duration_expired = '+1 '. $otp_duration_expired;
			$next_send = strtotime($duration_expired, $last_item['TwoFactorAuthen']['updated']);
			if($next_send < time()){
				$resend_time = time();
				$this->TwoFactorAuthen->id = $last_item['TwoFactorAuthen']['id'];
				if($this->TwoFactorAuthen->saveField('updated', $resend_time)){
					$otp_send = $tfa->getCode($secret, $resend_time);
				}
			}
			// Re-send mail with setting duration expired
			if($company_two_factor_auth == 'send_mail'){
				$this->set('otp_send', $otp_send);
				$this->_sendEmail($employee_otp['Employee']['email'], __('[Z0Gravity] Two-factor authentication', true), 'otp');
			}
		}
		$this->set(compact('url_qrcode', 'company_two_factor_auth'));
	}
	public function cookie_two_factor_auth(){
		$result = false;
		$employee_otp = $this->Session->read('Auth.employee_otp');
		if(empty($employee_otp)){
			$this->redirect("/index");
		}
		$cookie_key = '2FA_'. ($employee_otp['Employee']['id'] .'_'. $employee_otp['Company']['id']);
		$otp_cookie = $this->Cookie->read($cookie_key);
		// debug( $otp_cookie ); exit;

		if(!empty($otp_cookie)){
			$this->loadModels('CompanyConfigs', 'TwoFactorAuthen');
			// $_2fa_company_configs = $this->CompanyConfigs->find('list', array(
				// 'recursive' => -1,
				// 'conditions' => array(
					// 'company' => $employee_otp['Company']['id'], 
			...
			if(!empty($last_item)){
				if(!empty($last_item['TwoFactorAuthen']['current_cookie']) && ($cookie_value != $last_item['TwoFactorAuthen']['current_cookie'])){
					$this->Session->setFlash( __('You have signed in to a new device. To continue, please enter OTP', true), 'default', array(), 'auth');
					return false;
				}
				$next_send = strtotime($duration_expired, $last_item['TwoFactorAuthen']['updated']);
				if($next_send >= time()){ // Chua het han
					$tfa = new TwoFactorAuth();
					$secret =  $last_item['TwoFactorAuthen']['secret_code'];
					$otp_code = $tfa->getCode($secret, $cookie_time);
					if($otp_code == $cookie_value){
						$result = true;
						$this->Session->delete('Auth.employee_otp');
					}
				}
