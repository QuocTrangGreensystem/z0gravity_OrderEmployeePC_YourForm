 ...
 */
class ProjectLocalViewsController extends AppController {
    ...
    function index($project_id = null) {
        if(isset($this->employee_info['Color']['is_new_design']) && $this->employee_info['Color']['is_new_design']){
            $this->redirect(array(
                'controller' => 'project_local_views_preview',
    ...
    function delete($id = null) {
		...
		if($canModified){
			if ($this->ProjectLocalView->delete($id)) {
				...
				$this->Session->setFlash(__('Project local view was deleted', true), 'success');
				$this->redirect($this->referer());
			}
		}
        $this->Session->setFlash(__('Project local view was not deleted', true), 'error');
        $this->redirect($this->referer());
    }
