...
class KanbanController extends AppController {
     ...
     * @access public
     */
    function index($project_id = null, $filter = null) {
		/* Huynh 14/09/2019 Removed filter function */
		$this->_checkRole(false, $project_id);
		//QuanNV. 09/07/2019. _checkWriteProfile de lay 2 bien $_isProfile, $_canWrite
		$this->_checkWriteProfile('kanban');
        $this->loadModels('Project', 'ProjectStatus', 'ProjectTask', 'ProjectPhasePlan' ,'ProjectTaskEmployeeRefer' ,'LogSystem', 'ProjectEmployeeManager', 'ActivityRequest', 'ProjectTaskTxt','ProjectTaskAttachment', 'ProjectTaskAttachmentView', 'ProjectTaskTxtRefer', 'Translation');
        ...
        // Manager resources of project, follow it in heading
        ...
        $listProjectStatus = !empty($projectStatus) ? Set::combine($projectStatus, '{n}.ProjectStatus.id', '{n}.ProjectStatus') : array();

        $projectStatus = !empty($projectStatus) ? Set::combine($projectStatus, '{n}.ProjectStatus.id', '{n}.ProjectStatus.name') : array();
        $projectTask = $this->ProjectTask->find('all', array(
            'recursive' => -1,
            'conditions' => array(
        ...
        $listProjectTask = !empty($projectTask) ? Set::combine($projectTask, '{n}.ProjectTask.id', '{n}.ProjectTask') : array();
        $list_task_id = !empty($projectTask) ? Set::combine($projectTask, '{n}.ProjectTask.id', '{n}.ProjectTask.id') : array();
		$parent_task = !empty($projectTask) ? Set::classicExtract($projectTask, '{n}.ProjectTask.parent_id') : array();
		...
		// Get count project task
		$count_task_comment = $this->ProjectTaskTxt->find('all', array(
			'recursive' => -1,
			'conditions' => array('project_task_id' => $list_task_id),
			'group' => array('project_task_id'),
			'fields' => array('project_task_id', 'count(id) as comment_count')
		));
		$task_comment_status = $this->ProjectTaskTxtRefer->find('list', array(
				'recursive' => -1,
                'conditions' => array(
		...
		// debug($task_comment_status);
		// exit;
		$count_task_comment = !empty( $count_task_comment ) ? Set::combine( $count_task_comment, '{n}.ProjectTaskTxt.project_task_id', '{n}.0.comment_count') : array();
		
		// Get count project file upload
		$count_task_file = $this->ProjectTaskAttachment->find('all', array(
			'recursive' => -1,
			'conditions' => array('project_id' => $project_id),
			'group' => array('task_id'),
			'fields' => array('task_id', 'count(task_id) as attachment_count')
		));
		$task_file_status = $this->ProjectTaskAttachmentView->find('list', array(
				'recursive' => -1,
                'conditions' => array(
					'task_id' => $list_task_id,
					'employee_id' => $this->employee_info['Employee']['id']
		...
		$count_task_file = !empty( $count_task_file ) ? Set::combine( $count_task_file, '{n}.ProjectTaskAttachment.task_id', '{n}.0.attachment_count') : array();

        $projectPlanedPhaseID = !empty($projectTask) ? Set::combine($projectTask, '{n}.ProjectTask.id', '{n}.ProjectTask.project_planed_phase_id') : array();
		
		// List assign to

        $listAssiged = $this->ProjectTask->find('all', array(
            'recursive' => -1,
            'conditions' => array(
            ...
            'fields' => array('ProjectTaskEmployeeRefer.project_task_id', 'ProjectTaskEmployeeRefer.reference_id','ProjectTaskEmployeeRefer.estimated', 'ProjectTaskEmployeeRefer.is_profit_center')
        ));
		$emp_assignned = array();
		if(!empty($listAssiged)){
			foreach($listAssiged as $ass){
				$ass = $ass['ProjectTaskEmployeeRefer'];
				$emp_assignned[$ass['project_task_id']][] = $ass;
			}
		}
        ...
        foreach ($projectStatus as $key => $value) {
			$taskStatus[$key] = array();
            foreach ($listProjectTask as $task_id => $task_value) {
				if(!in_array($task_id,$parent_task)){
					if($key == $task_value['task_status_id']){
						$estimated = !empty( $task_value['estimated']) ? $task_value['estimated'] : 0;
						$consumed = !empty($task_value['special']) ? ( isset( $task_value['special_consumed']) ? $task_value['special_consumed'] : 0 ) : ( isset($listTaskConsumed[$task_id]) ? $listTaskConsumed[$task_id] : 0 ) ;
						
						$progress = 100;
						if( $estimated != 0) $progress = ($consumed / $estimated) * 100;
						elseif( empty( $consumed) ) $progress = 0;
						$task_value['progress'] = round($progress);
						$task_value['estimated'] = round($estimated, 2);
						$task_value['consumed'] = round($consumed, 2);
						
						
						if(!empty($task_value['task_end_date'])){
							$task_end_date_format = date('d-F-Y', strtotime($task_value['task_end_date']));
							$task_value['task_end_date_format'] = $task_end_date_format;
							$task_value['late'] = 0;
							$end_date = DateTime::createFromFormat('Y-m-d', $task_value['task_end_date']);
						...
						// commment
						$task_value['comment_count'] = !empty($count_task_comment[$task_id]) ? $count_task_comment[$task_id] : 0;
						$task_value['comment_read_status'] = 0;
						
						if ( array_key_exists($task_id, $task_comment_status )) $task_value['comment_read_status'] = intval($task_comment_status[$task_id]);
						
						// attachment
						$task_value['attachment_count'] = !empty($count_task_file[$task_id]) ? $count_task_file[$task_id] : 0;
						$task_value['file_read_status'] = 0;
						if( !empty($task_value['attachment'])) {
							$task_value['attachment_count'] += 1;
							$task_value['file_read_status'] = 1;
						}
						if ( array_key_exists($task_id, $task_file_status )) $task_value['file_read_status'] = intval($task_file_status[$task_id]);
						$task_value['assigned'] = !empty($emp_assignned[$task_id]) ? $emp_assignned[$task_id] : array();
						$taskStatus[$key][$task_id] = $task_value;
						$localDataTask[] = $task_value;
        ...
        $listAvata =$this->listEmployeeAssigned($project_id, true);
		$checkAvatar = $this->checkAvatar();
        // kiem tra role
        $myRole = $this->employee_info['Role']['name'];
        $emp_id_login = $this->employee_info['Employee']['id'];
        // list resource assign to
        $listEmployee = $this->_getTeamEmployees($project_id);
	
		$listEmployeeName = $this->_getListEmployee($company_id, true);
        // attached file
        $attachedFile = $this->get_attached_file($project_id);
		...
		foreach($phasePlans as $Phase){
			...
			$phases[$Phase['ProjectPhasePlan']['id']] = array_merge($Phase['ProjectPhasePlan'], $Phase['ProjectPhase']);
		}
		$list_phases = !empty($phases) ? Set::combine($phases, '{n}.id', '{n}.name') : array();
		$list_phase_colors = !empty($phases) ? Set::combine($phases, '{n}.id', '{n}.color') : array();
        ...
        $this->set(compact('localDataTask','projectStatusEX','taskStatus', 'projectStatus' ,'listProjectStatus', 'projectName', 'listAvata', 'listEmployee', 'listEmployeeName', 'checkAvatar', 'filter', 'attachedFile', 'project_id', 'list_phases', 'list_phase_colors', 'list_phase_date', 'adminTaskSetting', 'myRole', 'emp_id_login'));

    }
	/*
	function listEmployeeAssigned
	@params: project_id
	*/
	public function listEmployeeAssigned($project_id, $return = false){
		$this->loadModels('ProjectTask', 'ProjectTeam');
		$listAssiged = $this->ProjectTask->find('all', array(
            'recursive' => -1,
            'conditions' => array(
                'ProjectTask.project_id' => $project_id
            ...
            'fields' => array('ProjectTaskEmployeeRefer.project_task_id', 'ProjectTaskEmployeeRefer.reference_id', 'ProjectTaskEmployeeRefer.is_profit_center')
        ));
        $listEmployeeAssigned = $listPCAssigned = array();
        foreach ($listAssiged as $key => $value) {
            if(!empty($value['ProjectTaskEmployeeRefer']['reference_id'])){
				if($value['ProjectTaskEmployeeRefer']['is_profit_center']){
					$listPCAssigned[] =  $value['ProjectTaskEmployeeRefer']['reference_id'];
				}else{
					$listEmployeeAssigned[] =  $value['ProjectTaskEmployeeRefer']['reference_id'];
				}
            }
        }
        $listEmployeeAssigned = array_unique($listEmployeeAssigned);
        $listPCAssigned = array_unique($listPCAssigned);
        $listEmployees = $this->Employee->find('all', array(
			'recursive' => -1,
            'conditions' => array(
                'company_id' => $this->employee_info['Company']['id'],
                'id' => $listEmployeeAssigned,
            ),
		...
		foreach($listEmployees as $emp){
			$listEmployeeAssigned[$emp['Employee']['id']] = $emp['Employee']['first_name'] . ' ' . $emp['Employee']['last_name'];
		}
		
		$listPCs = $this->ProjectTeam->ProfitCenter->find('list', array(
			'recursive' => -1,
			'order' => array('ProfitCenter.name' => 'asc'),
			'conditions' => array(
				'company_id' => $this->employee_info['Company']['id'],
				'ProfitCenter.id' => $listPCAssigned
			),
			'fields' => array('id', 'name' )
			
		));
		foreach($listPCs as $key => $pc){
			$listPCs[$key] = 'PC / ' . $pc;
		}
		$result = array(
			'listEmployeeAssigned' => $listEmployeeAssigned,
			'listPCAssigned' => $listPCs,			
		);
    ...
    private function getLocalDataTask($project_id = null, $filter = null){
        $localDataTask = array();
        $projectTask = $this->ProjectTask->find('all', array(
            'recursive' => -1,
            'conditions' => array(
        ...
        foreach ($projectTask as $key => $value) {
			if(!(in_array($value['ProjectTask']['id'], $parent_task))){
				if(!empty($filter) && !empty($value['ProjectTask']['task_end_date'])){
					$end_day = strtotime($value['ProjectTask']['task_end_date']);
					$week = date('W',strtotime($value['ProjectTask']['task_end_date']));
					$month = date('m',strtotime($value['ProjectTask']['task_end_date']));
					$year = date('Y',strtotime($value['ProjectTask']['task_end_date']));

					if($year == date("Y")){
						if($filter == 'day' && $value['ProjectTask']['task_end_date'] == $curDate){
							$localDataTask[$key] = $value['ProjectTask'];
						}

						if($filter == 'month' && $month == date("m")){
							$localDataTask[$key] = $value['ProjectTask'];
						}

						if($filter == 'week' && $week == date("W")){
							$localDataTask[$key] = $value['ProjectTask'];
						}
					}
					//task late
					// task_end_date < current_date and status not close
					if($filter == 'late' && $end_day < time()){
						...
						if(!empty($value['ProjectTask']['task_status_id']) && in_array($value['ProjectTask']['task_status_id'], $list_status)){
							$localDataTask[$key] = $value['ProjectTask'];
						}
					}
    ...
    public function openFileAttach($projects){
    ...
    **/
    ...
    function update_task() {
        ...
        $this->data["ProjectTask"] = array();
        if (!empty($this->data)) {
            $this->data["ProjectTask"] = $this->data;
            unset($this->data["ProjectTask"]["ProjectTask"]);
            App::import("vendor", "str_utility");
            $str_utility = new str_utility();
            if ($this->ProjectTask->save($this->data)) {
                $this->Session->setFlash(sprintf(__('The project task %s has been saved.', true), '<b>' . $this->data["ProjectTask"]["task_title"] . '</b>'), 'success');
    ...
    function delete($id = null, $project_id = null) {
        if (!$id) {
            $this->Session->setFlash(__('Invalid id for project task', true), 'error');
            $this->redirect(array('action' => 'index', $project_id));
        }
        if ($this->_checkRole(true, $project_id)) {
            if ($this->ProjectTask->delete($id)) {
                $this->Session->setFlash(__('Project task has been deleted', true), 'success');
                $this->redirect(array('action' => 'index', $project_id));
            }
            $this->Session->setFlash(__('Project task was not deleted', true), 'error');
        }
        $this->redirect(array('action' => 'index', $project_id));
    }
    /**
    ...
    function delete_task($id = null) {  
        ...
        if (!$id) {
			...
			if($activityRequests == 0){
				if ($this->ProjectTask->delete($id)) {
					$this->loadModel('ProjectTaskEmployeeRefer');
					$this->ProjectTaskEmployeeRefer->deleteAll(array('project_task_id' => $id), false);
					die('1');
				}
			}
            die(0);
                ...
                if ($children) {
                    ...
                    if($element['parent_id'] == -1){
                        ...
                        foreach($element['children'] as $id => $value) {
                            ...
                            $_remains += $value['remain'];
                            $_wait += $value['wait'];
                            $_estimated += $value['estimated'];
                            $_initial_estimated += $value['initial_estimated'];
                            ...
                            if(isset($value['is_activity'])) {
                                if($value['is_activity'] == 'true') {
                                    $_totalPrvious = $value['consumed'];
                                    $_totalEstimatedPr = $value['estimated'];
                                    $_totalInitialEstimatedPr = $value['initial_estimated'];
  ...
  /**
    ...
    public function createTaskJson($project_id) {
        ...
        if (!empty($this->data)) {
            ...
            $success = true;
            if(!empty($data["ProjectTask"]['parent_id'])){
            ...
            if($success == true){
                ...
                if($flag > 0){
                    ...
                    if(!empty($data["ProjectTask"]['predecessor'])) $_predecessor   = $data["ProjectTask"]['predecessor'];
                    ...
                    if(!empty($data["ProjectTask"]['duration'])) $_duration = $data["ProjectTask"]['duration'];
                    ...
                    if (empty($data["ProjectTask"]['parentId']) || $data["ProjectTask"]['parentId'] > 999999999999) {
                    ...
                    if(!empty($data["ProjectTask"]['task_assign_to_text'])) $tmp_task_assign_to_text = $data["ProjectTask"]['task_assign_to_text'];
                    ...
                    if (!empty($data['ProjectTask']['profile_id']) && !$data['ProjectTask']['profile_id'] && !empty($data['ProjectTask']['project_planed_phase_id'])) {
    ...
    public function addNewTask($project_id) {
        $data = array();
        if (isset($_POST['task_title'])) {
            $data['project_id'] = $project_id;
            $data['task_title'] = $_POST['task_title'];
            $data['task_status_id'] = $_POST['task_status_id'];
            $data['task_start_date'] = $data['task_end_date'] = date('Y-m-d');

            // Check task_title
            $conditions = array(
                "ProjectTask.task_title" => $data['task_title'],
          ...
          $this->ProjectTask->create();
            ...
            $new_task = $this->ProjectTask->save($data);
			// $task_id = $this->ProjectTask->id;
            $project_new =   $this->ProjectTask->find('first', array(
                'recursive' => -1,
                'conditions' => array('project_id' => $new_task['ProjectTask']['project_id']),
                'order'  => 'id DESC',
            ));
			if(!empty($project_new)){
				$project_new['ProjectTask']['progress'] = 0;
				$project_new['ProjectTask']['comment_count'] = 0;
				$project_new['ProjectTask']['attachment_count'] = 0;
			}
           ...
           // isset duration
            if (isset($_duration) && $_duration != 0) {
                if (isset($_startDate)) { // duration isset and start day is set
                    if (isset($_endDate)) { // duration isset and start day is set and enddate is set
                        //$_duration = null;
                        if ($oldDataOfTask['ProjectTask']['duration'] == $_duration) {
                            //do nothing
                            $_duration = null;
                            $_duration = $this->getWorkingDays($_startDate, $_endDate, $_duration);
                        } else {
   ...
   //          }
            ...
            if( !empty($val['parent']) ){
                $parent = $this->ProjectTask->find('first', array(
                    ...
                    'conditions' => array(
                        ...
                        'project_id' => $project_id,
                        '1=1'   //thêm cái này vào để tránh trường hợp cakephp cache lại result của query bên dưới (phần kiểm tra task tồn tại chưa)
                    )
                ));
                if( !empty($parent) ){
    ...
    public function update_document($project_id = null){
        $this->loadModel('ProjectTaskAttachment');
		if(empty($project_id)) $project_id = $this->data['Upload']['project_id'];
        $data = array(
            'status' => false,
            'attachment' => ''
        );
		if(!empty($this->data['Upload']['id'])){
			$idProject = $this->ProjectTask->find("first", array(
				...
				'conditions' => array('id' => $this->data['Upload']['id'])
			));
			$project_id = $idProject['ProjectTask']['project_id'];
		}
        // neu co url
        if(!empty($this->data['Upload']['url']) && empty($_FILES)){
            $data = array(
                'project_id'=> $project_id,
                'task_id' => $this->data['Upload']['id'],
		...
		// add for widget task 
		if( !empty( $this->data['Upload']['return_ajax']) ) die(json_encode($data));
		// END add for widget task 
    ...
    public function attachment($file_id = null) {
        $this->loadModel('ProjectTaskAttachment');
        $this->layout = false;
        $link = '';
        $attachment_file = $this->ProjectTaskAttachment->find("first", array(
            'recursive' => -1, 'fields' => array('attachment','is_file', 'task_id'),
            "conditions" => array('id' => $file_id)));

        if ($attachment_file) {
            $link = trim($this->_getPath($attachment_file['ProjectTaskAttachment']['task_id'])
        ...
        if($attachment_file['ProjectTaskAttachment']['is_file'] == 0){
           $this->set('url',$attachment_file['ProjectTaskAttachment']['attachment']);
           $this->set('is_https',$attachment_file['ProjectTaskAttachment']['is_https']);
        }
    }
    function ajax($file_id = null, $view = 'ajax') {

        $this->loadModel('ProjectTaskAttachment');
        $type = '';
        $attachment_file = $this->ProjectTaskAttachment->find("first", array(
            'recursive' => -1, 'fields' => array('id','attachment','is_file', 'task_id'),
            "conditions" => array('id' => $file_id)));

        if ($attachment_file) {
            $type = $attachment_file['ProjectTaskAttachment']['is_file'] == 1 ? 'file' : 'url';
            $link = trim($this->_getPath($attachment_file['ProjectTaskAttachment']['task_id'])
                    . $attachment_file['ProjectTaskAttachment']['attachment']);
        ...
        $this->set(compact('file_id', 'attachment_file', 'type', 'view'));
        $this->render('ajax');
    }
    public function getTaskAttachment($task_id){
        ...
        $data['task_title'] = !empty($taskName['projectTasks']['task_title']) ? $taskName['projectTasks']['task_title'] : '';
		...
		$data['isPmProject'] = !empty($checkPmManager) ? 1 : 0;
		
		$count = $this->ProjectTaskAttachment->find('count', array(
			'recursive' => -1,
			'conditions' => array('task_id' => $task_id),
		));
		$data['attachment_count'] = $count;
	...
	/* 
	* Function Update read status for task attachment
	* @param: Project task ID
	*/
	public function update_task_attachment_read_status($pTaskId = null){
		$this->loadModels('ProjectTask','ProjectTaskAttachmentView');
		if(!$pTaskId){
		...
		// debug( $ProjectTask); exit;
		if( $ProjectTask ){
			$this->loadModel('ProjectTaskAttachmentView');
			$data = $this->ProjectTaskAttachmentView->find('first', array(
				'conditions' => array(
				'task_id' => $pTaskId,
				'employee_id' => $this->employee_info['Employee']['id'] 
				),
			...
			if( empty( $data)){
				$data = $this->ProjectTaskAttachmentView->create();
				$data['ProjectTaskAttachmentView']['task_id'] = $pTaskId;
				$data['ProjectTaskAttachmentView']['employee_id'] = $this->employee_info['Employee']['id'];
			}
			$data['ProjectTaskAttachmentView']['read_status'] = 1;
			$data = $this->ProjectTaskAttachmentView->save($data);
			if( $data) $result = true;
		}
		return;
		// if( $isAjax ){
	...
	/* 
	* Function Set read_status for other employee to UNREAD
	* @param: Project task ID
	*/
	public function task_attachment_unread($pTaskId = null){
		$this->loadModels('ProjectTask','ProjectTaskAttachmentView');
		if(!$pTaskId){
			$this->cakeError('error404');
		}
		...
		if( $ProjectTask ){
			$this->loadModel('ProjectTaskAttachmentView');
			$list_stt = $this->ProjectTaskAttachmentView->find('all', array(
				'conditions' => array(
				'task_id' => $pTaskId,
				'employee_id !=' => $this->employee_info['Employee']['id'] 
				),
			...
			foreach ($list_stt as $val) {
				$_this_data = $val;
				$_this_data['ProjectTaskAttachmentView']['read_status'] = 0;
				$this->ProjectTaskAttachmentView->id = $val['ProjectTaskAttachmentView']['id'];
				$result = $this->ProjectTaskAttachmentView->save($_this_data);
				// debug($result);
			}
		}
		return;
    ...
    protected  function _deleteFile($task_id){
        ...
        return $this->ProjectTask->saveField('attachment', null);
    }
    protected  function _deleteFileKanban($file_id){
        $this->loadModels('projectTasks', 'ProjectTaskAttachment');
        $p = $this->ProjectTaskAttachment->read('attachment', $file_id);
        if( !empty($p['ProjectTaskAttachment']['attachment']) ){
            $data = $p['ProjectTaskAttachment']['attachment'];
            $path = $this->_getPath($file_id);
            if( $data){
        ...
        $this->ProjectTaskAttachment->delete($file_id);
        return;
    }
    public function delete_attachment($att_id = null, $task_id=null){
		$attachId = !empty($_POST['attachId']) ? $_POST['attachId'] : '';
		$taskId = !empty($_POST['taskId']) ? $_POST['taskId'] : '';
		if( !empty($attachId) && $attachId != 0){
			$this->loadModels('ProjectTaskAttachment');
			$this->_deleteFileKanban($attachId);
			$count = 0;
			if(!empty($_POST['attachId'])){
				$count = $this->ProjectTaskAttachment->find('count', array(
					'recursive' => -1,
					'conditions' => array('task_id' => $_POST['attachId']),
				));
			}
			die(json_encode($count));
		}else{
			$this->_deleteFile($taskId);
			die(json_encode($taskId));
		}
    }
    ...
    public function update_text(){
        ...
        if( !empty($this->data['id']) ){
			...
			$count = $this->ProjectTaskTxt->find('count', array(
				'recursive' => -1,
				'conditions' => array('project_task_id' => $this->data['id']),
			));
    ...
    public function update_task_status(){
		$this->loadModels('ActivityTask', 'ProjectTaskEmployeeRefer', 'Projects', 'ProjectEmployeeManager');
        $result = false;
        $data = array();
        $message = '';
        ...
        if(!empty($_POST)){
            ...
            $project_id = !empty( $project_id) ? $project_id['ProjectTask']['project_id'] : 0;
			
			$project_manager = $this->Projects->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'id' => $project_id,
					'project_manager_id' => $this->employee_info['Employee']['id']
			...
			$list_project_manager = $this->ProjectEmployeeManager->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'project_id' => $project_id,
					'project_manager_id' => $this->employee_info['Employee']['id'],
			...
			if(!empty($pc_assigned)){
				...
				if(!empty($managerPC) && in_array($this->employee_info['Employee']['id'], $managerPC)){
					$canUpdateStatus = 1;
				}else{
					if(!empty($listPCRefer)){
						...
						if($bk_manager > 0) $canUpdateStatus = 1;
					}
				}
			}
            if( $this->employee_info['Role']['name'] == 'admin' || !empty($project_manager) || !empty($list_project_manager) || (!empty($employee_assigned)  && in_array($this->employee_info['Employee']['id'], $employee_assigned) && !empty($this->companyConfigs['diary_modify']) && !empty($this->companyConfigs['diary_status'])) || $canUpdateStatus == 1){
                $this->ProjectTask->id = $_POST['id'];
                $data = $this->ProjectTask->save(array(
                    'id' => $this->ProjectTask->id, 
                    'task_status_id' => $_POST['status']
                ));
                if( $data ){
					$result = true;
					$activity_task_id = $this->ActivityTask->find('first', array(
						'recursive' => -1,
						'conditions' => array(
							'project_task_id' => $_POST['id'],
						),
					...
					if(!empty($activity_task_id)){
						$this->ActivityTask->save(array(
							'id' => $activity_task_id['ActivityTask']['id'], 
							'task_status_id' => $_POST['status']
						));
					}
					
				}
				$this->Session->setFlash(__('The status has been saved.', true), 'success');
            }else{
			...
			$this->Session->setFlash(__('Missing data.', true), 'error');
        }
    ...
    public function updateAssignTask($task_id = null){
        $this->loadModel('ProjectTaskEmployeeRefer');
        if(!empty($_POST) && !empty($task_id)){
            $reference_id = $_POST['employee_id'];
            $is_profit_center = $_POST['is_profit'];
            $checkEmployeeRefer = $this->ProjectTaskEmployeeRefer->find('all', array(
                'recursive' => -1,
                'conditions' => array(
                    'project_task_id' => $task_id,
    ...
    public function deleteAssign($task_id = null){
        $this->loadModel('ProjectTaskEmployeeRefer');
		$message = '';
        if(!empty($_POST) && !empty($task_id)){
            $reference_id = $_POST['employee_id'];
			$reference_id = explode( '-', $reference_id );
			$is_pc = !empty( $reference_id[1]) ? $reference_id[1] : 0;
			$reference_id = $reference_id[0];
			// debug( $reference_id);
			// debug( $is_pc);
			// exit;
            $checkEmployeeRefer = $this->ProjectTaskEmployeeRefer->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'project_task_id' => $task_id,
                    'reference_id' => $reference_id,
            ...
            if(!empty($checkEmployeeRefer)){ 
                $this->ProjectTaskEmployeeRefer->id = $checkEmployeeRefer['ProjectTaskEmployeeRefer']['id'];
                $this->ProjectTaskEmployeeRefer->delete();
            }else{
				$message = __('Employee is not exist', true);
			}
        }else{
			$message = __('Failed to remove employee', true);
		}
		$listAssigedForTask = $this->ProjectTaskEmployeeRefer->find('all', array(
			'recursive' => -1,
			'conditions' => array(
				'project_task_id' => $task_id,
			),
		...
		$listAssigned = array();
		if( !empty( $listAssigedForTask)){
			foreach( $listAssigedForTask as $emp){
				$listAssigned[] = $emp['ProjectTaskEmployeeRefer'];
			}
		}
		$project_id = $this->ProjectTask->find('first', array(
			'recursive' => -1,
		...
		$project_id = !empty( $project_id ) ? $project_id['ProjectTask']['project_id'] : '';
		if( !empty( $project_id )) $this->ProjectTask->staffingSystem($project_id);
		die(json_encode(array(
			'result' => !empty($checkEmployeeRefer) ? 'success' : 'failed',
			'data' => !empty($listAssigned) ? $listAssigned : array(),
			'message' => $message
		)));
    }
    ...
    public function getCommentTxt($pTaskId = null){
        $this->layout = false;
        $results = array();
        if($pTaskId){
            // $pTaskId = $_POST['id'];
            $results['id'] = $pTaskId;
            $this->loadModels('ProjectTaskTxt', 'projectTasks', 'Employee');
            ...
            $taskName = $this->projectTasks->find('list', array(
                'recursive' => -1,
            ...
            if(!empty($result)){
                ...
                foreach ($result as $key => $value) {
                    $employeeId[] = $value['ProjectTaskTxt']['employee_id'];
                }
                $employeeName = $this->Employee->find('all', array(
                    'recursive' => -1,
                    'conditions' => array('id' => $employeeId),
                    'fields' => array('id','first_name', 'last_name')
                ));
               
                $employeeName = !empty($employeeName) ? Set::combine($employeeName, '{n}.Employee.id', '{n}.Employee') : '';
                $results['employee_info'] = $employeeName;
            }
            // debug($taskName); exit;
            $results['task_title'] = $taskName[$pTaskId];
            $results['result'] = $result;
        }
		if( !empty($results['result']) ) $this->requestAction('/project_tasks/update_read_status/'. $pTaskId);
    ...
    function get_attached_file($project_id = null) {
        $this->loadModel('ProjectTaskAttachment');
        $datas = $this->ProjectTaskAttachment->find('all', array(
            'recursive' => -1,
            'conditions' => array(
                'project_id'  => $project_id,
            ),
        ...
        $datas = !empty($datas) ? Set::combine($datas, '{n}.ProjectTaskAttachment.id', '{n}.ProjectTaskAttachment.attachment') : array();
        return $datas;
    }
    public function task_detail($task_id = null){
        $this->loadModels('ProjectTasks', 'ProjectTaskTxt', 'Translation', 'Project', 'ProjectPhasePlan', 'ProjectStatus', 'ProjectPhase','ProjectTaskAttachment');
        if($task_id){
            // Task detail
            $this->_checkRole(false,  $this->data['project_id']);
            $this->_checkWriteProfile('kanban');
            $project_task = $this->ProjectTasks->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'id' => $task_id,
                ),
            ...
            $this->data = $project_task['ProjectTasks'];
            ...
            // debug($this->data['attachments']); exit;
            $projectName = $this->Project->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'Project.id' => $project_task['ProjectTasks']['project_id']
                ),
                'fields' => array('id','project_name')
            ));
            // Tast texts - task commnet
            $this->data['project_name'] = $projectName['Project']['project_name'];
            $this->data['project_id'] = $projectName['Project']['id'];
            ...
            if(!empty($projectTaskTxt)){
                ...
                $employeeName = !empty($employeeName) ? Set::combine($employeeName, '{n}.Employee.id', '{n}.Employee') : '';
                $projectTaskTxt['employee_info'] = $employeeName;
            }
            // debug($projectTaskTxt); exit;

            // task phase name
            $phaseName = $this->ProjectPhasePlan->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'ProjectPhasePlan.id' => $this->data['project_planed_phase_id']
                ),
                'joins' => array(
                    array(
                        'table' => 'project_phases',
                        'alias' => 'ProjectPhase',
                        'type' => 'left',
                        'conditions' => array('ProjectPhasePlan.project_planed_phase_id = ProjectPhase.id')
                    )
                ),
                'fields' => array('ProjectPhasePlan.id', 'ProjectPhase.name')
            ));
            $this->data['phase_name'] = $phaseName['ProjectPhase']['name'];
            
            // list assign
            $listAssiged = $this->ProjectTask->find('all', array(
                'recursive' => -1,
                'conditions' => array(
                    'ProjectTask.project_id' => $projectName['Project']['id'],
                ),
                'joins' => array(
                    array(
                        'table' => 'project_task_employee_refers',
            ...
            $columns_order = $this->requestAction('/admin_task/getTaskSettings');
            ...
            // list phase
            $projectPhases = $this->_getPhaseses($projectName['Project']['id']);
            $projectPhases = Set::combine($projectPhases, '{n}.ProjectPhasePlan.id', '{n}.ProjectPhase.name');
            ...
            // debug($employeeName); exit;

            $this->set(compact('projectTaskTxt', 'project_task' ,'translation_data', 'listAssiged', 'task_id', 'columns_order', 'projectStatus', 'projectPhases', 'checkAvatar', 'employeeName'));

            $this->render('task_detail');
        } else {
            echo "Error";
            die;
        }
    }
    public function task_new($status_id = null){
        $this->loadModels('ProjectTasks', 'ProjectTaskTxt', 'Translation', 'Project', 'ProjectPhasePlan', 'ProjectStatus', 'ProjectPhase');
        if($status_id){
            $project_id = $_POST['project_id'];
            ...
            $this->set(compact('columns_order', 'projectStatus', 'projectPhases', 'task_status_id'));
     ...
     * @access public
     */
    public function saveFieldAssignProject($project_id = null) {
        $result = false;
        $this->layout = false;
        $this->loadModel('ProjectEmployeeProfitFunctionRefer');
        $employeeRefers = $this->ProjectEmployeeProfitFunctionRefer->find('list', array(
        ...
        if(!empty($_POST)){
            $dataA = array();
            $dataA['employee_id'] = $_POST['value'];
            $dataA['project_id'] = $project_id;
            $this->ProjectTeam->create();
            //if ($this->_checkRole(true)) {
                //if ($this->ProjectTeam->save(array_diff_key($this->data, array('employee_id' => null, 'backup' => null)))) {
                //if ($this->ProjectTeam->save(array_intersect_key($this->data, array('project_id' => '', 'project_function_id' => '', 'profit_center_id' => '', 'price_by_date' => '', 'work_expected' => '')))) {
                /**
                 ...
                 */
                if ($this->ProjectTeam->save(array_intersect_key($dataA, array('project_id' => '', 'project_function_id' => '', 'price_by_date' => '', 'work_expected' => '')))) {
                    $result = true;
                    // $this->Session->setFlash(__('Saved', true), 'success');

                    $dataA['id'] = $this->ProjectTeam->id;
                    $datas = $arrProfitCenters = array();
                    if(!empty($dataA['employee_id'])){
                        foreach ($dataA['employee_id'] as $employee) {
                            $datas[] = array(
                                'employee_id' => $employee,
	...
	function edit_task_popup($project_id = null) {
		// exit;
		...
		$listAllPhases = $this->ProjectPhasePlan->find('all', array(
			...
			'fields' => array('ProjectPhasePlan.id', 'ProjectPhasePlan.project_planed_phase_id', 'ProjectPhasePlan.project_id', 'ProjectPhasePlan.phase_planed_start_date', 'ProjectPhasePlan.phase_planed_end_date', 'ProjectPhasePlan.phase_real_start_date', 'ProjectPhasePlan.phase_real_end_date', 'ProjectsPhase.name')
		));
		// $listPhases = !empty( $listPhases) ? Set::combine($listPhases, '{n}.ProjectPhasePlan.id', '{n}.ProjectPhasePlan') : array();
		$listPhases = array();
		...
		// debug( $listPhases); exit;
		if( empty ($listAllStatus)) $listAllStatus = $this->Project->ProjectStatus->find('list', array('fields' => array('ProjectStatus.id', 'ProjectStatus.name'), 'conditions' => array('ProjectStatus.company_id' => $company_id)));
		$projectPriorities = $this->ProjectTask->ProjectPriority->find('list', array(
			'recursive' => -1,
			'conditions' => array(
				'company_id' => $company_id
    ...
    public function deleteCommentTxt(){
        ...
        if(!empty($_POST)){
            ...
            $cm_count = $this->ProjectTaskTxt->find('count', array(
                'recursive' => -1,
