 ...
 */
class ProjectImagesPreviewController extends AppController {
    var $uses = array('ProjectImage');
    var $name = 'ProjectImagesPreview';
    ...
    public function upload($company_id = null, $projectId = null){
		if( !$this->_checkRole(false, $projectId)){
			die(json_encode(array(
				'status' => 'error',
				'message' => __('Not allowed', true)
			)));
		}
                ...
                if( $type[0] == 'image' ){
                    try {
                        ...
                        if($this->MultiFileUpload->otherServer == true){
                            $this->MultiFileUpload->uploadFileToServerOther($path, $attachment, '/project_images_preview/index/' . $projectId);
                            $this->MultiFileUpload->uploadFileToServerOther($path, 'l_' . $attachment, '/project_images_preview/index/' . $projectId);
                            $this->MultiFileUpload->uploadFileToServerOther($path, 'r_' . $attachment, '/project_images_preview/index/' . $projectId);
                        }
                    } catch (Exception $ex) {
                        ...
                        if($this->MultiFileUpload->otherServer == true){
                            $this->MultiFileUpload->deleteFileToServerOther($path, $attachment, '/project_images_preview/index/' . $projectId);
                            $this->MultiFileUpload->deleteFileToServerOther($path, 'r_' . $attachment, '/project_images_preview/index/' . $projectId);
                            $this->MultiFileUpload->deleteFileToServerOther($path, 'l_' . $attachment, '/project_images_preview/index/' . $projectId);
                        }
                        die(json_encode(array(
                            'status' => 'error',
