 ...
 */
class ProjectRisksController extends AppController {
    ...
    function index($project_id = null) {
        ...
        if( !empty($this->employee_info['Color']['is_new_design']) && $this->employee_info['Color']['is_new_design']){
            $this->redirect(array('controller' => 'project_risks_preview','action' => 'index/'. $project_id));
        }
        ...
        $riskSeverities = $this->ProjectRisk->ProjectRiskSeverity->find('list', array(
            'fields' => array('id', 'risk_severity'),
            'conditions' => array(
                'company_id' => $projectName['Project']['company_id']
            )
                ));
        $riskOccurrences = $this->ProjectRisk->ProjectRiskOccurrence->find('list', array(
        ...
        $this->loadModels('LogSystem', 'Menu', 'Employee','HistoryFilter');
        ...
        $checkDisplayProjectTeam = $this->Menu->find('first', array(
            ...
            //ob_clean();
           // debug($checkDisplayProjectTeam); exit;
        // if checkDisplayProjectTeam == 1 {
        ...
        $this->set(compact('issueStatus', 'company_id', 'employeeLoginId', 'avatarEmployeeLogin', 'employeeLoginName', 'companyName', 'avatarEmploys', 'logSystems', 'projectName', 'riskSeverities', 'riskOccurrences', 'employees', 'project_id', 'projectRisks', 'loadFilter'));
    }
        ...
        if (!empty($this->data)) {
            ...
            if ($this->_checkRole(true)) {
                ...
                if ($this->ProjectRisk->save(array_merge($this->data, $data))) {
                    $result = true;
                    // $this->Session->setFlash(__('The Risk has been saved.', true), 'success');
                } else {
