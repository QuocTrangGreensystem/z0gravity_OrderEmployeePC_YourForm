 ...
 */
class ProjectGlobalViewsController extends AppController {
    ...
    public function upload($project_id = null) {
        if($this->data['ProjectGlobalView']['is_file']){
            ...
            if($last){
				$this->ProjectGlobalView->id = $last['ProjectGlobalView']['id'];
                    @unlink($path . $last['ProjectGlobalView']['attachmentURL']);
            }
            $is_https = 0;
            $pos = strpos($this->data['ProjectGlobalView']['attachmentURL'],'https');
            if($pos !== false){
                $this->data['ProjectGlobalView']['attachment'] = str_replace('https://','',$this->data['ProjectGlobalView']['attachmentURL']);
                $is_https = 1;
            }
            else{
                $this->data['ProjectGlobalView']['attachment'] = str_replace('http://','',$this->data['ProjectGlobalView']['attachmentURL']);
                //$is_https = 0;
            }
            $result = $this->ProjectGlobalView->save(array(
				'project_id' => $project_id,
				'attachment' => $this->data['ProjectGlobalView']['attachment'],
				'is_file' => $this->data['ProjectGlobalView']['is_file'],
				'is_https' => $is_https
    ...
    public function attachment_index($project_id = null, $size='small') {
		$size_width = array(
			'small' => 320,
			'medium' => 800,
			'large' => 1024,
				...
				if (!file_exists($link_index) || !is_file($link_index)) {
					...
					if($type[0] == 'image'){
						try {
							...
							$resize->resizeTo($size_width[$size], 0, 'maxwidth');
