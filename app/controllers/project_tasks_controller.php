...
class ProjectTasksController extends AppController {
    ...
    function index($project_id = null) {
        ...
        $this->set(compact('workdays', 'remainInter', 'remainExter', 'projectName', 'project_id', 'listPrioritiesJson','budgetInters','getDataProjectTasks','var','myRole','settingP','modifyF','budgetExters','varInter','workloadInter','workloadExter','delay','consumedInter','consumedExter','projectMilestones', 'listEmployee','budget_settings', 'isEmployeeManager', 'employee_id', '_isPmOfPj', 'bg_currency', 'listTaskName'));
    }
    ...
    **/
	...
	Array(
		[ProjectTask] => Array
			(
				[return] => https://z0gravity.local/project_amrs_preview/indicator/2881
				[id] => 12592
				[task_title] => Présentation budget
				[project_planed_phase_id] => 5047
				[task_status_id] => 540
				[task_start_date] => 26-07-2018
				[task_end_date] => 29-03-2019
			)

		[task_assign_to_id] => Array
			(
				[5] => 2343 // eployee
				[6] => 0 // skip
				[14] => 767-1 // pc
	...
	*/
    public function update_task(){
		...
		$_Task = '';
        if (!empty($this->data)) {
			$taskID = $this->data['ProjectTask']['id'];
            if (!empty($taskID)) {
                $this->ProjectTask->id = $taskID;
            }
			$project_id = $this->ProjectTask->find('first', array(
				'recursive' => -1,
			...
			$startDateOld = (empty($oldDataOfTask['ProjectTask']['task_start_date']) || $oldDataOfTask['ProjectTask']['task_start_date']=="" ) ? null : $oldDataOfTask['ProjectTask']['task_start_date'];
            $endDateOld = (empty($oldDataOfTask['ProjectTask']['task_end_date']) || $oldDataOfTask['ProjectTask']['task_end_date']=="") ? null : $oldDataOfTask['ProjectTask']['task_end_date'];
			$project_id = $project_id['ProjectTask']['project_id'];
            $data = array();
            foreach (array('task_start_date', 'task_end_date', 'task_real_end_date') as $key) {
                if (!empty($this->data['ProjectTask'][$key])) {
                    $data['ProjectTask'][$key] = $this->ProjectTask->convertTime($this->data['ProjectTask'][$key]);
                }
            }
            if ($this->_checkRole(true, $project_id)) {
			// debug( $this->_checkRole(false, $project_id)); exit;
                unset($this->data['ProjectTask']['id']);
				$list_empl_assign = array();
				$estimated = 0;
				if( !empty( $this->data['workloads'])){
					foreach( $this->data['workloads'] as $emp => $es){
						$val = explode('-',$emp.'-0');
						$list_empl_assign[] = array(
							'reference_id' => $val[0],
					...
					$this->data['ProjectTask']['estimated'] = $estimated;
				}
				if($this->data['ProjectTask']['estimated'] >= 0){
					if( isset($this->data['ProjectTask']['manual_consumed'])){
						$this->data['ProjectTask']['manual_consumed'] = round($this->data['ProjectTask']['manual_consumed'], 2);
					}
					if(isset($this->data['ProjectTask']['milestone_of_task'])){
						$this->data['ProjectTask']['milestone_id'] = $this->data['ProjectTask']['milestone_of_task'];
					}
					if(($this->data['ProjectTask']['task_start_date'] != '') && ($this->data['ProjectTask']['task_start_date'] != '0000-00-00')){
						$temp_start_date = strtotime($this->data['ProjectTask']['task_start_date']);
						$temp_start_date_old = strtotime($startDateOld);
						$temp_end_date = strtotime($this->data['ProjectTask']['task_end_date']);
						$temp_end_date_old = strtotime($endDateOld);
						if( !isset($this->data['ProjectTask']['duration_of_task'])) $this->data['ProjectTask']['duration_of_task'] = 0 ;
						if(($this->data['ProjectTask']['duration_of_task'] > 0) && ($temp_start_date == $temp_start_date_old) && ($temp_end_date == $temp_end_date_old)){
							$data['ProjectTask']['task_start_date'] = $startDateOld;
							$data['ProjectTask']['task_end_date'] = $this->_durationEndDate($startDateOld, $endDateOld, $this->data['ProjectTask']['duration_of_task']);
							$this->data['ProjectTask']['duration'] = $this->data['ProjectTask']['duration_of_task'];
						}else{
							$this->data['ProjectTask']['duration'] = $this->getWorkingDays($this->data['ProjectTask']['task_start_date'], $this->data['ProjectTask']['task_end_date'], $this->data['ProjectTask']['duration_of_task']);
						}
					}
					if(empty($data['ProjectTask'])){
						$_Task = $this->ProjectTask->save($this->data['ProjectTask'], array('employee_id' => ''));
					}else{
						$_Task = $this->ProjectTask->save(array_merge($this->data['ProjectTask'], $data['ProjectTask']), array('employee_id' => ''));
					}
					if ($_Task) {
						$result = true;
						$this->Session->setFlash(__('The Task has been saved.', true), 'success');
                        $this->notifyForUpdateTask($project_id, $this->ProjectTask->id, $_Task['ProjectTask']['task_title']); // Notify to users.

					} else {
						$this->Session->setFlash(__('The Task could not be saved. Please, try again.', true), 'error');
					}
					$this->data['ProjectTask']['id'] = $this->ProjectTask->id;
				}else{
					$this->Session->setFlash(__('The value estimated more than 0.', true), 'error');
				}
            }
			...
			// debug( $list_empl_assign);
            if (!empty($this->data['task_assign_to_id']) && empty( $this->data['workloads'])) {
				foreach($this->data['task_assign_to_id'] as $key => $val){
					if(!empty($val)){
						$val = explode('-',$val.'-0');
						$list_empl_assign[] = array(
							'reference_id' => $val[0],
			...
			//Ticket #1661. Update phase date, project date, parent date. Created by VN
			if($result){
				...
				$this->_attachPhasePlansDuration($project_id);
				$this->_syncPhasePlanTime($task);
				$this->_saveStartEndDateAllTask($project_id);
				$this->_syncActivityTask($project_id, $task, $taskID);
			}
			
			//Ticket #1661. Update by QuanNV
			$task_prede = $this->ProjectTask->find('first', array(
				'recursive' => -1,
                'conditions' => array('predecessor' => $taskID),
                'fields' => array('id', 'duration')
			));
			$data_task_prede = array();
			if(!empty($task_prede) && !empty($this->data['ProjectTask']['task_end_date']) && ($this->data['ProjectTask']['task_end_date'] != '0000-00-00')){
				$date = strtotime($this->data['ProjectTask']['task_end_date']);
				$date = mktime(0, 0, 0, date("m", $date), date("d", $date)+1, date("Y", $date));
				$date = date('Y-m-d', $date);
				$data_task_prede = array(
					'task_start_date' => $date
				);
				$data_task_prede['task_end_date'] = $this->_durationEndDate($data_task_prede['task_start_date'], null, $task_prede['ProjectTask']['duration']);
				$this->ProjectTask->id = $task_prede['ProjectTask']['id'];
				$this->ProjectTask->save($data_task_prede);
			}
		...
		if( $this->params['isAjax']){
			$return_data = $this->get_task_info($taskID, 1);
			// Format endate
			$return_data['task_end_date'] = !empty($return_data['task_end_date']) ? $return_data['task_end_date'] : '' ;
			$return_data['end_date_format'] = !empty($return_data['task_end_date']) ? date('d F Y', strtotime($return_data['task_end_date'])) : '' ;
			$return_data['task_end_date_format'] = !empty($return_data['task_end_date']) ? date('d-F-Y', strtotime($return_data['task_end_date'])) : '' ;
			$return_data['late'] = 0;
			$end_date = DateTime::createFromFormat('Y-m-d', $return_data['task_end_date']);
			if($end_date == '' || $end_date == '00-00-0000'){
			}else{
				$end_date->setTime(0, 0, 0);
				if(strtotime('today') > $end_date->getTimestamp()){
					$return_data['late'] = 1;
				}
			}
			if(!empty($data_task_prede)){
				$return_data['prede_task_id'] = $task_prede['ProjectTask']['id'];
				$return_data['prede_task_startdate'] = $data_task_prede['task_start_date'];
			}
			die( json_encode(array(
				'result' => ($result ? 'success' : 'failed' ) ,
				'message' => $return_data,
			)));
		}elseif( !empty( $this->data['ProjectTask']['return'] ) ) {
			$this->redirect($this->data['ProjectTask']['return']);
		}else{
			$this->redirect(array(
    ...
    function exportExcel($project_id = null) {
        ...
        $this->set(compact('projectTasks','typeExport','settingP','checkP', 'bg_currency'));
    }
            ...
            if ($element['parent_id'] == $parentId) {
                ...
                if ($children) {
                    ...
                    if($element['parent_id'] == -1){
                        $_remains = $_overload = $_manualOverload = $_manualCons = $_unitPrice = $_consumedEuro = $_remainEuro = $_workloadEuro = $_estimatedEuro = $_estimated = $_initial_estimated = $_consumed = $_totalPrvious = $_totalEstimatedPr = $_totalInitialEstimatedPr = $_wait = $_eac = 0;
                        ...
                        foreach($element['children'] as $id => $value) {
                            ...
                            $_eac += isset($value['eac']) ? $value['eac'] : 0;
                            $_initial_estimated += !empty($value['initial_estimated']) ? $value['initial_estimated'] : 0; 
                        ...
                        $sumRemain = $sumEstimated = $sumInitialEstimated = $sumOverload = $sumWait = $sumManualOverload = $unitPrice = $consumedEuro = $remainEuro = $workloadEuro = $estimatedEuro = $sumManualCs = $sumEac = 0;
                        ...
                        foreach ($children as $keys => $child){
                            ...
                            $sumEac += !empty($child['eac']) ? $child['eac'] : 0;
    ...
    */
    ...
    private function _getAllProjectTasks($project_id) {
		...
		*/
		...
		// Get attactment
		$this->loadModel('ProjectTaskAttachments');
		...
		$index = 0;
		// debug($projectTaskAttachments); exit;
		...
		foreach( $projectTasks as $projectTask){
			...
			// Neu bang project tasks co attachment thi cho trang thai la da doc
			$projectTasks[$index]['ProjectTask']['attach_read_status'] = 1;
			if( in_array($_taskID, $projectTaskAttachments )){
				// Neu bang projectTaskAttachments co attachment thi cho trang thai la chua doc
				$projectTasks[$index]['ProjectTask']['attach_read_status'] = 0;
				// Neu bang project tasks k co attachment ma bang projectTaskAttachments co thi dat gia tri 1
				if( empty($projectTasks[$index]['ProjectTask']['attachment'])){
					$projectTasks[$index]['ProjectTask']['attachment'] = 1;
				}
			}
			
			// foreach( $projectTaskAttachments as $attach){
				// if( $projectTask['ProjectTask']['id'] == $attach){
					// $projectTasks[$index]['ProjectTask']['attach_read_status'] = 0;
					// if( empty($projectTasks[$index]['ProjectTask']['attachment'])){
						// $projectTasks[$index]['ProjectTask']['attachment'] = 1;
					// }
		...
		// add read_status
		$this->loadModels('ProjectTaskTxtRefer', 'ProjectTaskAttachmentView');
		$has_read = $this->ProjectTaskTxtRefer->find('list', array(
				'recursive' => -1,
                'conditions' => array(
					'task_id' => $list_task,
					'employee_id' => $this->employee_info['Employee']['id']
		...
		for ( $index = 0; $index < count($projectTasks); $index++){
			$_id = $projectTasks[$index]['ProjectTask']['id'];
			$projectTasks[$index]['ProjectTask']['read_status'] = isset( $has_read[$_id] ) ? $has_read[$_id] : 0;
		}
		
		$has_read = $this->ProjectTaskAttachmentView->find('list', array(
				'recursive' => -1,
                'conditions' => array(
					'task_id' => $list_task,
					'employee_id' => $this->employee_info['Employee']['id']
		...
		for ( $index = 0; $index < count($projectTasks); $index++){
			$_id = $projectTasks[$index]['ProjectTask']['id'];
			if( isset( $has_read[$_id]) ) $projectTasks[$index]['ProjectTask']['attach_read_status'] =  $has_read[$_id];
		}
    ...
    private function _updateParentTask($project_id) {
        ...
        $caculates = $this->ProjectTask->find('all', array(
                'fields' => array(
                    ...
                    'SUM(overload) as s_over', // tu khoa over duoc su dung trong MariaDB
        ...
        foreach($caculates as $caculate){
            if($caculate['ProjectTask']['parent_id'] != 0 && $caculate['ProjectTask']['parent_id'] < 999999999){
				...
				$duration_parent_task[$caculate['ProjectTask']['parent_id']] = $this->getWorkingDays($caculate[0]['min_date'], $caculate[0]['max_date'], '');
            }
        }
		return $duration_parent_task;
    }
...
/**
     ...
     * @author : BANGVN
     */
    public function get_team_employees($project_id = null) {
		if( $project_id ) die( json_encode( $this->_getTeamEmployees( $project_id )));
    }
  ...
  /**
    ...
    private function _getTeamEmployeesForLoad($project_id, $task_id, $start = null, $end = null) {
        ...
        if(!empty($checkDisplayProjectTeam) && $checkDisplayProjectTeam['Menu']['display'] == 1){
			...
			$consd_add_normal = array();
			if($start == -1 && $end == -1){
				// Case add normal task.
				$consd_add_normal['OR'] =  array(
					array('end_date' => '0000-00-00'),
					array('end_date IS NULL'),
					array('end_date >=' => date('Y-m-d')),
				);
    ...
    private function _attachPhasePartStructure($project_id) {
		...
		// $rows = isset($history['expanded_setting']) ? $history['expanded_setting'] : array();
		// debug($rows); exit;
        ...
        foreach ($projectParts as $key => $projectPart) {
			...
			// $children[$key]['expanded'] = (bool) isset($rows['root']) ? $rows['root'] : true;
            $children[$key]['children']         = array();
            $children[$key]['is_part']          = 'true';
        }
        return $children;
    }
	private function _attachPhasePlansDuration($project_id) {
		$projectPhasePlans = $this->_getAllPojectPhasePlans($project_id);
		$children = array();
		foreach ($projectPhasePlans as $key => $projectPhasePlan) {
			$phase_id =  (string)(999999999999 + $projectPhasePlan['ProjectPhase']['id']);
			$task_start_date = isset($projectPhasePlan['ProjectPhase']['phase_real_start_date']) ? $projectPhasePlan['ProjectPhase']['phase_real_start_date'] : 0;
            $task_end_date = isset($projectPhasePlan['ProjectPhase']['phase_real_end_date']) ? $projectPhasePlan['ProjectPhase']['phase_real_end_date'] : 0;
			$children[$phase_id] = $this->getWorkingDays($task_start_date, $task_end_date, '');
		}
		return $children;
	}
    /**
    ...
    private function _attachEACForParent($projectTasks) {
        $total = 0;
        $parentIds = array_unique(Set::classicExtract($projectTasks, '{n}.ProjectTask.parent_id'));
        foreach($projectTasks as $key => $projectTask){
            foreach($parentIds as $parentId){
        ...
        foreach ($projectTasks as $key => $projectTask) {
            if (!empty($projectTask['ProjectTask']['eac']) && is_numeric($projectTask['ProjectTask']['eac'])) {
                $total += $projectTask['ProjectTask']['eac'];
            }
        }
        return $total;
    }
    ...
    private function _durationEndDate($startDate, $endDate, $duration){
		#2121 tinh lai end_date khi chi thay doi duration (ko thay doi startDate va endDate)
        $_durationEndDate = '';
        if(!empty($duration)){
    ...
    private function _buildJsonStructure($projectTasks, $project_id) {
        ...
        //milestone.
        $this->loadModel('ProjectMilestone');
        $milestoneProject = $this->ProjectMilestone->find('all', array(
            'recursive' => -1,
            'conditions' => array('project_id' => $project_id),
            'fields' => array('id', 'project_milestone', 'milestone_date')
        ));
		$milestoneCompanies = array();
		if(!empty($milestoneProject)){
			foreach($milestoneProject as $key => $miVal){
				$miles = $miVal['ProjectMilestone'];
				$milestoneCompanies[$miles['id']] = $miles['project_milestone'];
				if(!(empty($miles['milestone_date']) || $miles['milestone_date'] == '0000-00-00')){
					$milestoneCompanies[$miles['id']] .= ' '. date('d-m-y', strtotime($miles['milestone_date']));
				}
			}
		}
        ...
        foreach ($projectTasks as $key => $projectTask) {
            ...
            if (empty($projectTask['ProjectTask']['parent_id'])) {
                ...
                $children[$key]['leaf'] = 'false';
                $children[$key]['children'] = array();
				$id = (string) $projectTask['ProjectTask']['id'];
				$children[$key]['expanded'] = (bool) isset($rows[ $id ]) ? $rows[ $id ] : true;
                // $children[$key]['expanded'] = true;
            } else {
                        ...
                        if (isset($value['reference_id'])) {
                            foreach($employees as $employee){
                                if($value['reference_id'] == $employee['Employee']['id'] && $value['is_profit_center'] == $employee['Employee']['is_profit_center']){
                                    if( !isset($children[$key]['task_assign_to_text']) || !in_array($employee['Employee']['name'], $children[$key]['task_assign_to_text']) )
			...
			$children[$key]['eac'] = isset($projectTask['ProjectTask']['eac']) ? $projectTask['ProjectTask']['eac'] : 0;
            ...
            $amount += (float) $children[$key]['amount'];
            $children[$key]['progress_order'] = $projectTask['ProjectTask']['progress_order'];
            $children[$key]['text_1'] = $projectTask['ProjectTask']['text_1'];
            $children[$key]['read_status'] = isset( $projectTask['ProjectTask']['read_status'] ) ? $projectTask['ProjectTask']['read_status'] : 0;
            $children[$key]['attach_read_status'] = isset( $projectTask['ProjectTask']['attach_read_status'] ) ? $projectTask['ProjectTask']['attach_read_status'] : 1;
			...
			$children[$key]['slider'] = isset( $projectTask['ProjectTask']['slider'] ) ? $projectTask['ProjectTask']['slider'] : 0;
        }
		...
		$root_child['expanded'] = (bool) isset($rows['root']) ? $rows['root'] : true;
        ...
        $_eac         		= $this->_attachEACForParent($projectTasks);
    ...
    /**
     * Function: Save Employees with in current Project Task
     * @var     : int $project_task_id
     * @var     : array $employess
     * @return  :
     * @author : HUUPC
     */
    private function _saveEmployees($employees, $profitCenters, $project_task_id, $workload, $tmp_estimated_details = array(), $callbacks = true) {
        $this->loadModels('ProjectTask', 'ProjectTaskEmployeeRefer');
        $saveds = $this->ProjectTaskEmployeeRefer->find('all', array(
            'recursive' => -1,
            'conditions' => array('ProjectTaskEmployeeRefer.project_task_id' => $project_task_id),
            'fields' => array('id')
        ));
        foreach($employees as $k => $v){
            if($v == '' || $v == null){
        ...
        if (!empty($employees)) {
            ...
            if(!empty($_employees)){
				...
				// debug($_employees); exit;
				// $this->ProjectTask->id = $project_task_id;
				$project_id = $this->ProjectTask->find('first', array(
					'recursive' => -1,
					'conditions' => array('id' => $project_task_id),
					'fields' => array('id', 'project_id'),
				));
				$project_id = $project_id['ProjectTask']['project_id'];
				// debug($project_id); exit;
				$this->updateEmployeeAssignedToTeam($project_id, $_employees);
            }
        } else {
            if (!empty($saveds)) {
                foreach ($saveds as $key) {
    ...
    public function createTaskJson($project_id, $data_import = array()) {
        $this->loadModel('ActivityTask');
        $this->loadModel('ActivityRequest');
        $this->loadModel('ProjectPhasePlan');
        $this->layout = 'ajax';
		...
		if(!empty($data_import)){
			$this->data = $data_import;
		}
        if (!empty($this->data)) {
			$es_start =  date('Y-m-01', time());
            ...
            if($success == true){
				...
				// Add task in kanban can't select phase
				// then select a phase in this project
				
				if( empty($data['ProjectTask']['project_planed_phase_id']) ){
					$project_phase = $this->ProjectPhasePlan->find('first', array(
						'recursive' => -1, 
						'conditions' => array('project_id' => $project_id),
						'fields' => array('id'),
					));
					if(!empty($project_phase)){
						$data['ProjectTask']['project_planed_phase_id'] = $project_phase['ProjectPhasePlan']['id'];
					}
				}
                if(!empty($data["ProjectTask"]['project_planed_phase_id'])){
                    if($data["ProjectTask"]['parent_id'] == 0){
                ...
                if($flag > 0){
                    ...
                    if (!empty($data["ProjectTask"]['task_priority_id']) && !$this->_hasPriority($data["ProjectTask"]['task_priority_id']) ) {
                        unset($data["ProjectTask"]['task_priority_id']);
                    }
                    if ( !empty($data["ProjectTask"]['milestone_id']) && !$this->_hasMilestone($data["ProjectTask"]['milestone_id']) ) {
                    ...
                    $_predecessor   = !empty($data["ProjectTask"]['predecessor']) ? $data["ProjectTask"]['predecessor'] : 0;
                    $_startDate     = !empty($data["ProjectTask"]['task_start_date']) ? $data["ProjectTask"]['task_start_date'] : $startDateTmp;
                    $_endDate       = !empty($data["ProjectTask"]['task_end_date']) ? $data["ProjectTask"]['task_end_date'] : $endDateTmp;
                    $_duration  =  !empty($data["ProjectTask"]['duration']) ? $data["ProjectTask"]['duration'] : 0;
                    ...
                    if (isset($_duration) && $_duration != 0) {
                        if (isset($_startDate)) { // duration isset and start day is set
							if(empty($data["ProjectTask"]['task_end_date'])){ // duration isset, start day is set, task_end_date not isset, 
								// get date of end with data duration
								$_endDate = null;
					...
					if(empty($data["ProjectTask"]['parentId'])){
						 $data["ProjectTask"]['parentId'] = 0;
					}
                    if ($data["ProjectTask"]['parentId'] > 999999999999) {
                        $data["ProjectTask"]['parentId'] = 0;
                    }
                    ...
                    $tmp_task_assign_to_text = !empty($data["ProjectTask"]['task_assign_to_text']) ? $data["ProjectTask"]['task_assign_to_text'] : '';
                    ...
                    if ( !empty($data['ProjectTask']['profile_id']) && !empty($data['ProjectTask']['project_planed_phase_id'])) {
                    ...
                    if (!empty($tmp_task_assign_to_id)) {
                        if(is_array($tmp_task_assign_to_id) && (count($tmp_task_assign_to_id) >0)){
							...
							if($data["ProjectTask"]['estimated'] > 0){
								// Caculate average estimated for new assigned employees 
								$count_employee = count($employee);
								if($count_employee > 0){
									$tmp_estimated = $data["ProjectTask"]['estimated'];
									$avage_estimated = floor( $tmp_estimated / $count_employee);
									$n = 0;
									foreach($employee as $index => $employee_id){
										if($n == 0){
											$val = $avage_estimated + ($tmp_estimated - $avage_estimated*$count_employee);
										}else $val = $avage_estimated;
										$tmp_estimated_detail[$index] = number_format($val, 2);
										$n++;
									}
								}
							}
                            $assign = $this->_saveEmployees($employee, $profitCenter, $data['id'], $data["ProjectTask"]['estimated'], $tmp_estimated_detail, false);
                        }
                    }
                    if ($result) {
                        ...
                        $result['ProjectTask']['task_end_date_format'] = date('d-F-Y', strtotime($_endDate));
                        $result['ProjectTask']['id'] = $data['id'];
						if($data["ProjectTask"]['estimated'] > 0){
							$tmp_result = array();
							$tmp_result[] = $result;
							$tmp_result = $this->getValueEACTask($tmp_result, array());
							$result = $tmp_result[0];
						}
						...
						// add comment task
						if(!empty($data["ProjectTask"]['text'])){
							$task_comment = array(
								'id' => $project_task_id,
								'text_1' => $data["ProjectTask"]['text']
							);
							$this->update_text($task_comment);
						}
                        //Tracking action
                        $projectName = $this->Project->find("first", array(
                        'recursive' => -1,
						...
						$import_log = !empty($data_import) ? __('Import Task', true) : __('Project Task', true);
                        $message = $employ . ' ' . date('d/m/Y H:i') . ' ' . $import_log . ' ' .  sprintf('Create task `%s` under `%s`', $result['ProjectTask']['task_title'], $projectName['Project']['project_name']);
		...
		if(!empty($data_import)) die(json_encode($result));
        $this->set(compact('result'));
    }

    public function updateTaskJson($project_id, $id = null, $data_import = array()) {
		$this->_checkRole(false, $project_id);
        $useManual = isset($this->companyConfigs['manual_consumed']) ? $this->companyConfigs['manual_consumed'] : 0;
        $this->layout = 'ajax';
        $failed='';
        ...
        if (!empty($this->data)) {
            ...
            if (!empty($data['task_priority_id']) && !$this->_hasPriority($data['task_priority_id']) ) {
                unset($data['task_priority_id']);
            }
			
			/* Update by Viet Nguyen
			   Request from mail: Z0G 24/2/2020: Important email for demo - Impossible to remove a milestone
			   Possible update empty milestone.
			*/
            if(empty($data['milestone_id']) && !empty($data['milestone_text'])){
				$data['milestone_text'] = '';
			}else if(!empty($data['milestone_id']) && !$this->_hasMilestone($data['milestone_id'])){
            ...
            if ($data['estimated'] == 0) {
				...
				// Reformat estimated with data submit
				// Tranh truong hop updated task nhung khong add estimated details. 
				if(!empty($data_import)){
					...
					// Keep old data employees estimated
					$tmp_estimated = $data['estimated'];
					if (!empty($data['estimated_detail'])) {
						foreach($data['estimated_detail'] as $index => $value){
							if($value > 0 && $tmp_estimated > 0){
								$val = ($tmp_estimated > $value ) ? $value : $tmp_estimated;
								$tmp_estimated_detail[$index] = number_format($val, 2);
								$tmp_estimated -= $val;
								unset($data['estimated_detail'][$index]);
							}
							// Truong hop chi update them estimated va khong thay doi employee assigned
							// Gia tri estimated con lai se cong cho employee cuoi cung.
							if(empty($data['estimated_detail']) && $tmp_estimated > 0){
								$tmp_estimated_detail[$index] = number_format($tmp_estimated_detail[$index] + $tmp_estimated, 2);
							}
							
						}
						// Caculate average estimated for new assigned employees 
						$count_employee = count($data['estimated_detail']);
						if($count_employee > 0){
							$avage_estimated = floor($tmp_estimated / $count_employee);
							$n = 0;
							foreach($data['estimated_detail'] as $index => $employee_id){
								if($n == 0){
									$val = $avage_estimated + ($tmp_estimated - $avage_estimated*$count_employee);
								}else $val = $avage_estimated;
								$tmp_estimated_detail[$index] = number_format($val, 2);
            ...
            if(isset($data['task_assign_to_text'])) unset($data['task_assign_to_text']);
            if(isset($data['task_assign_to'])) unset($data['task_assign_to']);
            if(isset($data['estimated_detail'])) unset($data['estimated_detail']);
            ...
            if(isset($data['task_assign_to_id'])) unset($data['task_assign_to_id']);
            if(isset($data['is_profit_center'])) unset($data['is_profit_center']);
			if(!empty($data_import)){
				$_predecessor = !empty($data['predecessor']) ? $this->_updatePredeces($data['id'], $data['predecessor']) : $oldDataOfTask['ProjectTask']['predecessor'];
			}else{
				$_predecessor = !empty($data['predecessor']) ? $this->_updatePredeces($data['id'], $data['predecessor']) : '';
			}
            ...
            $_duration = !empty($data['duration']) ? $data['duration'] : null;
			//Ticket #1191. Luon tinh lai duration. #2121 khong thay doi startDate va endDate thi duration = duration nhap vao va tinh lai endDate.
            $_keepDuration  = 0;
            $_taskId        = $data['id'];
			// isset duration
            if (isset($_duration) && $_duration != 0) {
                if (isset($_startDate)) { // duration isset and start day is set
                    if (isset($_endDate)) { // duration isset and start day is set and enddate is set
						...
						if($_keepDuration == 1){
							$_endDate = null;
                            $_endDate = $this->_durationEndDate($_startDate, $_endDate, $_duration);
						}elseif(($oldDataOfTask['ProjectTask']['task_start_date'] == $_startDate) && ($oldDataOfTask['ProjectTask']['task_end_date'] == $_endDate)){
                            $_endDate = $this->_durationEndDate($_startDate, $_endDate, $_duration);
                        }else{
                            $_duration = null;
                            $_duration = $this->getWorkingDays($_startDate, $_endDate, $_duration);
            ...
            if (isset($_predecessor) && $_predecessor != 0 && $_predecessor != '') {
            ...
            if (isset($data['consumed']) && $data['consumed'] < 0) {
                $data['estimated'] = 0;
            }
            ...
            if (isset($data['parentId']) && $data['parentId'] > 999999999999) {
                $data['parentId'] = 0;
            }
            ...
            if ( $useManual ) {
                $consume = isset($data['manual_consumed']) ? $data['manual_consumed'] : 0;
            ...
            if (isset($data['is_phase']) && $data['is_phase'] == 'true') {
                ...
                if ($result) {
					// add comment task. Ticket #2151
					if((!empty($data_import)) && (!empty($data['text']))){
						$task_comment = array(
							'id' => $data['id'],
							'text_1' => $data['text']
						);
                    ...
                    $result['ProjectTask']['remain'] = isset($data['remain']) ? $data['remain'] : 0;
                    $result['ProjectTask']['amount'] = isset($data['amount']) ? $data['amount'] : 0;
                    $result['ProjectTask']['overload'] = isset($data['overload']) ? $data['overload'] : 0;
                    $result['ProjectTask']['manual_overload'] = isset($data['manual_overload']) ? $data['manual_overload'] : 0;
                    $consume = $useManual ? (isset($data['manual_consumed']) ? $data['manual_consumed'] : 0) : (isset($data['consumed']) ? $data['consumed'] : 0);
                    $overload = $useManual ? $data['manual_overload'] : (isset($data['overload']) ? $data['overload'] : 0);
                    $result['ProjectTask']['completed'] = ($data['estimated'] > 0) ? round($consume / ( $overload + $data['estimated'] ) * 100, 2) : 0;
                    if(!empty($data['amount']))$result['ProjectTask']['progress_order_amount'] = round((isset($data['progress_order']) ? $data['progress_order'] : 0) * $data['amount'] / 100, 2);
                    $i = 0;
					...
					$result['ProjectTask']['task_parent_duration'] = $this->_updateParentTask($project_id);
					$result['ProjectTask']['phase_duration'] = $this->_attachPhasePlansDuration($project_id);
                    $this->_syncPhasePlanTime($result);
                    $project_duration = $this->_saveStartEndDateAllTask($project_id);
					$result['ProjectTask']['project_duration'] = $project_duration;
					$this->_syncActivityTask($project_id, $result, $_taskId);
					if($data['estimated'] > 0 || $consume > 0){
						$this->loadModel('ActivityTask');
						$activity_task_id = $this->ActivityTask->find('list', array(
							'recursive' => -1,
							'conditions' => array('project_task_id' => $_taskId),
							'fields' => array('project_task_id', 'id')
						));
						$tmp_result = array();
						$tmp_result[] = $result;
						$tmp_result = $this->getValueEACTask($tmp_result, $activity_task_id);
						$result = $tmp_result[0];
					}
                    // $this->ProjectTask->staffingSystem($project_id);
					...
					$import_log = !empty($data_import) ? __('Import Task', true) : __('Project Task', true);
                    $message = $employ . ' ' . date('d/m/Y H:i') . ' ' . $import_log . ' ' .  __('Update', true) . ' ';
                    ...
                    $this->writeLog($result, $this->employee_info, $message);
                    $this->notifyForUpdateTask($project_id, $_taskId,$result['ProjectTask']['task_title']); // Notify to users.
     ...
     * Xoa task
     */
    public function destroyTaskJson($id = null, $return = false) {
        $this->loadModel('ActivityTask');
        $this->loadModel('ActivityRequest');
        $this->layout = 'ajax';
        ...
        if (!empty($id) && is_numeric($id) ) {
            ...
            if ($hasUsed != 0) {
				...
				/* Xoa attachment truoc, neu xoa sau thi khong lay duoc file name */
				// delete file attachment
				$this->delete_attachment($listTasks, false);
                if (!empty( $listTasks) && $this->ProjectTask->deleteAll(array('ProjectTask.id' => $listTasks), false)) {
                    /**
                     * Xoa cac assign.
                     */
                    $this->loadModel('ProjectTaskEmployeeRefer');
     ...
     *
     * Task id l� 802 co sub task la 803 -> tuong ung voi activity task id la: 826
    ...
    public function getVocationDetail($id_employee = null, $stardate = null, $enddate = null){
		...
		$valueOnView = array();
		if(!empty($_POST)){
			$id_employee = $_POST['employee_id'];
			$stardate = $_POST['task_data']['task_start_date'];
			$enddate = $_POST['task_data']['task_end_date'];
			$valueOnView = $_POST['task_data'];
			if(!empty($_POST['task_data']['workload_resource'])){
				$workload_resource = $_POST['task_data']['workload_resource'];
				if(!empty($workload_resource)){
					foreach($workload_resource as $key => $value){
						$key = explode('-',$key);
						if($key[0] == $id_employee){
							$valueOnView['employee_estimated'] = $value;
						}
					}
				}
			}
        ...
        $result =  $this->_getVocationDetail($id_employee, $stardate, $enddate, $valueOnView);
        $this->set(compact('result'));
    }
    ...
    public function getManDayOfEmployeesForPopup($model = null, $modeld = null, $resources = array(),  $startDate = null, $endDate = null, $valueOnView = array()){
        $this->layout = false;
        $this->loadModel('ProjectTaskEmployeeRefer');
        $this->loadModel('ActivityTaskEmployeeRefer');
        $this->loadModel('ProjectEmployeeProfitFunctionRefer');
		...
		$_pc = array();
		if(!empty($resources)){
			foreach($resources as $ind =>$val){
				if($val['is_profit_center'] == 1) $_pc[] = $val['reference_id'];
				else if(!in_array($val['reference_id'], $_employees)) $_employees[] = $val['reference_id'];
			}
		}
        $result = array();
        $manDays['val'] = 0;
        if (!empty($_employees)) { //employee
			foreach($_employees as $employee_id){
				if(!empty($valueOnView)){
					if( !empty($valueOnView['workload_resource']) && !empty($valueOnView['workload_resource'][$employee_id . '-0'])){
						$valueOnView['employee_estimated'] = $valueOnView['workload_resource'][$employee_id . '-0'];
					}else{
						$valueOnView['employee_estimated'] = 0;
					}
				}
				$data = $this->_getVocationDetailForEmployees($employee_id, $startDate, $endDate, $valueOnView);
				$value = array_sum($data['avaiTotalYear']);
				//Ticket #1115. Check user hoat dong trong moc thoi gian cua task.
				$emp_info = $this->Employee->find('first', array(
					'recursive' => -1,
					'conditions' => array(
						'id' => $employee_id
					),
				...
				$start_emp = (!empty($emp_info['Employee']['start_date']) && ($emp_info['Employee']['start_date'] !='0000-00-00')) ? strtotime($emp_info['Employee']['start_date'] . ' 00:00:00') : 0;
				$end_emp = (!empty($emp_info['Employee']['end_date']) && ($emp_info['Employee']['end_date'] !='0000-00-00')) ? strtotime($emp_info['Employee']['end_date'] . ' 00:00:00') : 0;
				$start_task = (!empty($startDate) && ($startDate !='0000-00-00')) ? strtotime($startDate . ' 00:00:00') : 0;
				$end_task = (!empty($endDate) && ($endDate !='0000-00-00')) ? strtotime($endDate . ' 00:00:00') : 0;
				if((!empty($start_emp) && ($start_emp > $end_task)) || (!empty($end_emp) && ($end_emp < $start_task))){
					$result[$employee_id] = 0;
				}else{
					$result[$employee_id] = round($value,2);
				}
			}
        }
		
		if(!empty($_pc)){ //profit center
            $profitCenters = $this->ProjectEmployeeProfitFunctionRefer->find('all',array(
                'recursive' => -1,
                'conditions' => array('profit_center_id' => $_pc),
                'fields' => array('id', 'profit_center_id', 'employee_id'),
            ));
			if(!empty($profitCenters)){
				$profitCenters = Set::combine($profitCenters, '{n}.ProjectEmployeeProfitFunctionRefer.id', '{n}.ProjectEmployeeProfitFunctionRefer');
				$pc_refer = array();
				foreach($profitCenters as $id => $_val){
					if(empty($pc_refer[$_val['profit_center_id']]))$pc_refer[$_val['profit_center_id']] = array();
					$pc_refer[$_val['profit_center_id']][] = $_val['employee_id'];
				}
				if(!empty($pc_refer)){
					foreach($pc_refer as $pc_id => $empls){
						$totalManday = 0;
						$totalOverload = 0;
						foreach($empls as $e_id){
							$value = 0;
							if(!empty($result[$e_id])) {
								$value = $result[$e_id];
							}else{
								if(!empty($valueOnView)){
									if( !empty($valueOnView['workload_resource']) && !empty($valueOnView['workload_resource'][$e_id . '-0'])){
										$valueOnView['employee_estimated'] = $valueOnView['workload_resource'][$e_id . '-0'];
									}else{
										$valueOnView['employee_estimated'] = 0;
									}
								}
								$data = $this->_getVocationDetailForEmployees($e_id, $startDate, $endDate, $valueOnView);
								$value = array_sum($data['avaiTotalYear']);
								if($value < 0){
									$totalOverload += $value;
									$value = 0;
						...
						if($totalManday == 0) $totalManday = $totalOverload;
						$result[$pc_id] = round($totalManday,2);
					}
				}
			}
        }
	...
	private function _getVocationDetailForEmployees($employees = null, $stardate = null, $enddate = null, $valueOnView = array()){
        $this->loadModel('Holiday');
        $vocation = array();
        $woking = array();
        $otherTaskDate = $stardate;
        ...
        $requests_absence = Set::combine(ClassRegistry::init('AbsenceRequest')->find("all", array(
			...
			"conditions" => array(
			'or' => array('response_am' => 'validated', 'response_pm' => 'validated'),
			'date BETWEEN ? AND ?' => array($_start, $_end), 'employee_id' => $employees)))
			, '{n}.AbsenceRequest.date', '{n}.AbsenceRequest'
        );
        $intdate = $_start;
        while($intdate <= $_end){
            ...
            $vocation[$otherTaskDate] = 0;
			if (!empty($holidays[$intdate])) {
				$hl_am = isset($holidays[$intdate]['am']) ? 0.5 : 0;
				$hl_pm = isset($holidays[$intdate]['pm']) ? 0.5 : 0;
				$vocation[$otherTaskDate] += ($hl_am + $hl_pm);
			}
            if (!empty($requests_absence[$intdate])) {
                $response = 0;
                if ($requests_absence[$intdate]['response_am'] == 'validated') {
			...
			$vocation[$otherTaskDate] = $vocation[$otherTaskDate] > 1 ? 1 : $vocation[$otherTaskDate];
            $intdate = strtotime('+ 1 day', $intdate);
        }
		
        $vocationDetails = array();
        if (!empty($vocation)) {
        ...
        $listTaskDetails = $this->_getAllProjectTaskForEmployeeDetail($employees, $stardate, $enddate, $valueOnView);
		//NCT Task
        $listNctProjectTaskDetails = $this->getNctWorkload($employees, $stardate, $enddate);
        $listNctActivityTaskDetails = $this->getNctWorkload($employees, $stardate, $enddate, 'ActivityTask');
        ...
        $avaiTotalYears = array();
		// vocationDetails => ngay Absence.
		// Ticket #1115. QuanNV. So sanh moc thoi gian emp con hoat dong.
		$emp_info = $this->Employee->find('first', array(
			'recursive' => -1,
			'conditions' => array(
				'id' => $employees
        ...
        if(!empty($totalWorkload)){
            foreach($totalWorkload as $time => $values){
				...
				// check user co active trong ngay do hay ko.
				$is_active = 0;
				if(($start_emp == 0) && ($end_emp == 0))$is_active = 1;
				if(($start_emp != 0) && ($end_emp == 0) && ($start_emp <= $time))$is_active = 1;
				if(($start_emp != 0) && ($end_emp != 0) && ($start_emp <= $time) && ($time <= $end_emp))$is_active = 1;
                if (!isset($avaiTotalYears[$year])) {
                    $avaiTotalYears[$year] = 0;
                }
                if ((!empty($vocationDetails[$year][$day_month]) && $vocationDetails[$year][$day_month] == 1) || ($is_active == 0)) {
                    $avaiTotalYears[$year] += 0;
            ...
            foreach($vocationDetails as $year => $vocationDetail) {
                foreach($vocationDetail as $time => $values) {
                    if ($values == 1) {
                        ...
                        if (isset($totalWorkload[date('Y-m-d', $_time)])) {
                            //do nothing
                        } else {
                            if (!isset($avaiTotalYears[date('Y', $_time)])) {
                                $avaiTotalYears[date('Y', $_time)] = 0;
                            }
                            $avaiTotalYears[date('Y', $_time)] += (1 - $values);
                        }
                    }
                }
            }
    ...
    private function _getVocationDetail($id_employee = null, $stardate = null, $enddate = null, $valueOnView = array()){
        $this->loadModel('Holiday');
        $vocation = array();
        $woking = array();
        $otherTaskDate = $stardate;
        ...
        $vocationDetails = $vocationDetailMonths = $vocationDetailYears = $working = array();
        if (!empty($vocation)) {
            foreach($vocation as $day => $value){
                $time = strtotime($day);
                $year = date('Y', $time);
        ...
        $listTaskDetails = $this->_getAllProjectTaskForEmployeeDetail($id_employee, $stardate, $enddate, $valueOnView);
        //NCT Task
        ...
        $totalWorkload = array();
        $listDateDatas = $listMonthDatas = $listYearDatas = $listTotalDatas = $listIdFamilies = array();
            ...
            foreach($listTaskDetails as $listTaskDetail) {
                ...
                if (!empty($dx)) {
                    ...
                    if (!empty($dx['Workingday'])) {
						...
						$task_start = strtotime($dx['task_start_date']);
						$task_end = strtotime($dx['task_end_date']);
						if (!isset($listTotalDatas[$dx['family_id']]['pr-'.$dx['project_id']][$dx['id']])) {
							$listTotalDatas[$dx['family_id']]['pr-'.$dx['project_id']][$dx['id']] = 0;
						}
						$task_estimated = 0;
						if($_start <= $task_start && $_end >= $task_end){
							$task_estimated = $dx['estimated'];
						}
						$listTotalDatas[$dx['family_id']]['pr-'.$dx['project_id']][$dx['id']] += $task_estimated;
					}
                ...
                foreach($vocationDetail as $time => $values) {
                    if ($values == 1) {
                        ...
                        if (isset($totalWorkload[date('Y-m-d', $_time)])) {
                            ...
                            if (!isset($avaiTotals[date('Y-M', $_time)])) {
                                $avaiTotals[date('Y-M', $_time)] = 0;
                            }
                            $avaiTotals[date('Y-M', $_time)] += (1 - $values);
		...
		// Total
		$result['listTotalDatas'] = $listTotalDatas;
            ...
            foreach($workloads as $val){
                ...
                if( isset($tasks[$taskID]) ){
                    ...
                    foreach($dateRange as $date){
						if($date >= strtotime($startdate) && $date <= strtotime($enddate)){
    ...
    private function _getAllProjectTaskForEmployeeDetail($id_employee = null, $stardate, $enddate, $valueOnView = array()){
        $this->layout = 'ajax';
        $this->loadModel('Holiday');
        $this->loadModel('Project');
        $this->loadModel('ProjectTaskEmployeeRefer');
        $this->loadModel('ProjectPriority');
        $projectPriority = $this->ProjectPriority->find('list');
        $listprojecttask = $this->ProjectTaskEmployeeRefer->find('list',array(
            'conditions' => array(
                'reference_id' => $id_employee,
                'is_profit_center' => 0
                ),
            'recursive' => -1,
			'fields' => array('project_task_id', 'estimated'),
			
        ));
		$current_project_id = 0;
		if(!empty($valueOnView) ){
			if(!empty($valueOnView['id'])) $listprojecttask[$valueOnView['id']] = $valueOnView['employee_estimated'];
			$current_project_id = $valueOnView['project_id'];
		}
		
		// Ticket #1115 Do not get project opportunity, expected the current project
		$avai_list_tasks = array();
		if(!empty($listprojecttask)){
			$list_task_id = array_keys($listprojecttask);
			$avai_list_tasks = $this->Project->ProjectTask->find('all',array(
				// 'recursive' => -1,
				'conditions' => array(
					'ProjectTask.id' => $list_task_id,
					'AND' => array(
                        'OR' => array(
                            'ProjectTask.task_start_date BETWEEN ? AND ?' => array($stardate, $enddate),
                            'ProjectTask.task_end_date BETWEEN ? AND ?' => array($stardate, $enddate),
				...
				'fields'=> array('Project.id', 'Project.category', 'Project.project_name', 'Project.activity_id', 'ProjectTask.*')
			));
			
		}
		// Case add task popup: get avaibility for resource
		if(!empty($valueOnView) && empty($valueOnView['id'])){
			$tmp_valueOnView = array();
			$valueOnView['id'] = 0;
			$valueOnView['is_nct'] = 0;
			$valueOnView['task_priority_id'] = '';
			$avai_project = $this->Project->find('first',array(
				'recursive' => -1,
				'conditions' => array(
					'id' => $current_project_id
				),
				'fields'=> array('Project.id', 'Project.category', 'Project.project_name')
			));
			$tmp_valueOnView = $avai_project;
			$tmp_valueOnView['ProjectTask'] = $valueOnView;
			array_push($avai_list_tasks, $tmp_valueOnView);
			
		}

        $listProjectTaskFromDates = array();
        if(!empty($avai_list_tasks )){
			foreach($avai_list_tasks as $index => $p_task){
			   // Do not get project in opportunity for availibility expected the current project
			   if($p_task['Project']['category'] != 2 || (!empty($current_project_id) && $p_task['Project']['id'] == $current_project_id)){
					if(!empty($valueOnView) && !empty($valueOnView['id']) && $p_task['ProjectTask']['id'] == $valueOnView['id']){
						$p_task['ProjectTask'] = array();
						$p_task['ProjectTask'] = $valueOnView;
					}
					 if (!empty($p_task)) {
						$p_task['ProjectTask']['employee_estimated'] = !empty($listprojecttask[$p_task['ProjectTask']['id']]) ? $listprojecttask[$p_task['ProjectTask']['id']] : 0;
						if ($p_task['ProjectTask']['task_end_date']=='0000-00-00') {
							$p_task['ProjectTask']['task_end_date'] = $p_task['ProjectTask']['task_start_date'];
						}
						if ($p_task['ProjectTask']['task_start_date'] =='0000-00-00') {
							$p_task['ProjectTask']['task_start_date'] = $p_task['ProjectTask']['task_end_date'];
						}
						if ($p_task['ProjectTask']['task_start_date'] > $p_task['ProjectTask']['task_end_date'] ) {
							$p_task['ProjectTask']['task_start_date'] = $p_task['ProjectTask']['task_end_date'];
						}
						if (!empty($projectPriority[$p_task['ProjectTask']['task_priority_id']])) {
							$p_task['ProjectTask']['priority_title'] =    $projectPriority[$p_task['ProjectTask']['task_priority_id']];
						} else {
							$p_task['ProjectTask']['priority_title'] = "";
						}

						if (!empty($p_task['Project']['activity_id'])) {
							$this->loadModel('Activity');
							$familyId = $this->Activity->find('first', array(
								'recursive' => -1,
								'conditions' => array('Activity.id' => $p_task['Project']['activity_id']),
								'fields' => array('family_id')
							));
							if (!empty($familyId) && !empty($familyId['Activity']['family_id'])) {
								$p_task['ProjectTask']['family_id'] = $familyId['Activity']['family_id'];
							} else {
								$p_task['ProjectTask']['family_id'] = -1;
							}
						} else {
							$p_task['ProjectTask']['family_id'] = -1;
						}
						$p_task['ProjectTask']['project_name'] = !empty($p_task['Project']['project_name']) ? $p_task['Project']['project_name'] : '';
						if(!empty($valueOnView) && $p_task['ProjectTask']['id'] == $valueOnView['id']){
							$p_task['ProjectTask']['estimated'] = $valueOnView['estimated'];
							$p_task['ProjectTask']['task_start_date'] = $valueOnView['task_start_date'];
							$p_task['ProjectTask']['task_end_date'] = $valueOnView['task_end_date'];
							if(isset($valueOnView['employee_estimated'])) $p_task['ProjectTask']['employee_estimated'] = $valueOnView['employee_estimated'];
						}
						$listProjectTaskFromDates[] = $p_task;
					}
				}
			}
		}
    ...
    private function _AvgWorkingThisTaskForEmployee($task,$id_employee){
        ...
        while($otherTaskDate <= $task['task_end_date']){
            ...
            if (!empty($holidays[$intdate]) && !(strtolower(date('l',$intdate))=='sunday' || strtolower(date('l',$intdate)) == 'saturday')) {
				$hl_am = isset($holidays[$intdate]['am']) ? 0.5 : 0;
				$hl_pm = isset($holidays[$intdate]['pm']) ? 0.5 : 0;
                $duration  -= ($hl_am + $hl_pm);
				$daysOff[$otherTaskDate] = $hl_am + $hl_pm;
            }
            $flag=0;
            if (!empty($requests_absence[$intdate])) {
                if ($requests_absence[$intdate]['response_am'] == 'validated') {
            ...
            if ($flag!=0) {
				if(isset($daysOff[$otherTaskDate])){
					$daysOff[$otherTaskDate] += $flag;
                }else{
					$daysOff[$otherTaskDate] = $flag;
				}
				if($daysOff[$otherTaskDate] > 1) $daysOff[$otherTaskDate] = 1;
            }
            $otherTaskDate =  date('Y-m-d',strtotime($otherTaskDate. ' + 1 day'));
        }
    ...
    public function drapDrop() {
        ...
        if ($projectDetail['Project']['activity_id'] != '' && $projectDetail['Project']['activity_id'] != 0) {
			...
			$activity_update =  ClassRegistry::init('ActivityTask')->find('first', array(
				'recursive' => -1,
				'fields'=>'id',
                'conditions'=> array('ActivityTask.activity_id' => $activityId,'ActivityTask.project_task_id' => $taskCurrentDetail['ProjectTask']['id']))
            );
			if(!empty($activity_update)){
				ClassRegistry::init('ActivityTask')->updateAll(
					array('ActivityTask.name' => "'$newNameActivityTask'",'ActivityTask.parent_id' => "'$newParentId'"),
					array('ActivityTask.activity_id' => $activityId,'ActivityTask.project_task_id' => $taskCurrentDetail['ProjectTask']['id'])
				);
			}
        }
        if ($moveType == 1) {
                            ...
                            if (!empty($row['Start Date']) || !empty($row['End Date'])) {
								...
								if( empty($_start) || empty($_end)){
									if( empty($_start)){
										...
										$row['error'][] = __('Invalid Start date', true).' ('. __('dd-mm-yyyy is required', true).')';
									}
									if(empty($_end)){
										$row['columnHighLight']['End Date'] = '';
										$row['error'][] = __('Invalid End date', true).' ('. __('dd-mm-yyyy is required', true).')';
									}
								}
								// old logic {
									// if($_start == 0){
    ...
    private function _formatDateCustom($date = null){
		...
		if( !preg_match("/^([0-9]{1,2})\-([0-9]{1,2})\-([0-9]{2}|[0-9]{4})$/", $date) ) return '';
        $date = explode('-', $date);
		...
		if( preg_match("/^([0-9]{2})$/", $year)) $year = intval( $century.$year);
            ...
            if( !empty($val['parent']) ){
                $parent = $this->ProjectTask->find('first', array(
                    ...
                    'conditions' => array(
                        ...
                        'project_id' => $project_id,
                        '1=1'   //th�m c�i n�y v�o d? tr�nh tru?ng h?p cakephp cache l?i result c?a query b�n du?i (ph?n ki?m tra task t?n t?i chua)
                    )
                ));
                if( !empty($parent) ){
    ...
    public function getNcTask(){
        ...
        if( isset($this->data['id']) ){
            ...
            if( !empty($task) ){
				...
				$list = array();
				// Order by: Employee -> PC
				foreach( $taskEmployeeRefers as $refer_id => $is_pc){
					if( !$is_pc) $list[] = $refer_id . '-' . $is_pc;
				}
				foreach( $taskEmployeeRefers as $refer_id => $is_pc){
					if( $is_pc) $list[] = $refer_id . '-' . $is_pc;
				}
                if( !empty($assigns) ){
                    ...
                    foreach($list as $id){
                        ...
                        else $name = (!empty($listPcs[ $x[0] ]) ? $listPcs[ $x[0] ] : '');
    ...
    public function saveNcTask(){
        ...
        if( !empty($_POST) ){
			if(empty($this->data['task'])){
				$decode_data = json_decode($_POST['data'], true);
				$this->data = $decode_data['data'];
			}
			if( !empty( $this->data['NCTTask'])){
				$this->data = array_merge( $this->data,$this->data['NCTTask']);
				unset( $this->data['NCTTask']);
			}
			if(  empty( $this->data['task']['id']) ) $this->data['task']['id'] = 0;
            $pid = $this->data['id'];
			...
			if( empty( $this->data['task']['id']) ){
				...
				$check = $this->ProjectTask->find('count', array(
					...
					'conditions' => array(
						'project_id' => $pid,
						'project_planed_phase_id' => $this->data['task']['project_planed_phase_id'],
				...
				if( $check > 0){
					$result = array(
						'result' => false, 
						'message' => __('The task name already exists', true),
						'data' => array()						
					);
					goto _end_;
				}
            ...
            if( !empty($project) ){
                ...
                $this->loadModel('Employee');
                $this->loadModel('ProjectEmployeeProfitFunctionRefer');
				if( !empty( $this->data['task_assign_to_id'])){
					$list_empl_assign = array();
					foreach( $this->data['task_assign_to_id'] as $emp){
						$emp = explode('-', $emp);
						$list_empl_assign[] = array(
							'reference_id' => $emp[0],
							'is_profit_center' => $emp[1]
						);
					}
					
					$this->updateEmployeeAssignedToTeam($pid, $list_empl_assign);
				}
                $assignText = array();
                if( $final ){
                ...
                $pri = $this->getPriority(!empty($task['task_priority_id']) ? $task['task_priority_id'] : 0);
                $mil = $this->getMilestone(!empty($task['milestone_id']) ? $task['milestone_id'] : 0);
                ...
                $pro = $this->getProfile(!empty($task['profile_id']) ? $task['profile_id'] : 0);
            ...
            if( !empty( $this->data['run_staffing'])) $this->ProjectTask->staffingSystem($pid);
        }
        _end_:
		if($result['result'] && !empty($_FILES)){
            ...
            if(!empty($_FILES['FileField'])){
                if (!empty($_FILES['FileField']['name']['attachment'])) {
                    ...
                    $path = $this->_getPath($result['data']['id']);
                    ...
                    if( !empty($file) ){
                        ...
                        $data_upload = $this->ProjectTaskAttachment->save($data);
		...
		if( !empty( $data_upload)){
			$result['result_upload'] = true;
			$result['data_upload'] = $data_upload;
		}
        die(json_encode($result));
    }
    ...
    protected function _deleteFile($task_id, $projectTaskAttachments){
		...
		// remvove file upload in user file
    ...
    public function delete_attachment($task_id, $callback = true){
    ...
    public function delete_comments($task_id){
		$this->loadModels('ProjectTaskTxt', 'ProjectTaskTxtRefer');
		
		// delete data task upload in table ProjectTaskAttachment
		if($this->ProjectTaskTxt->deleteAll(array('ProjectTaskTxt.project_task_id' => $task_id), false)){
			// delete data status of attchment file task  in table ProjectTaskAttachment
    ...
    public function update_text($data = array()){
        $result = array();
		if(!empty($data)){
			// add comment in create task json
			$this->data = $data;
		}
        if( !empty($this->data['id']) ){
            ...
            $res = $this->ProjectTaskTxt->save(array(
                'project_task_id' => $this->data['id'],
                'employee_id' => $employee,
                'comment' => $this->data['text_1'],
                'creadted' => $result['text_time']
            ));
			if($res)  $result['id'] = $this->ProjectTaskTxt->id;
    ...
    public function getCommentTxt(){
        ...
        if(!empty($_POST)){
            ...
            $this->loadModels('ProjectTaskTxt', 'ProjectEmployeeManager');
            $result = $this->ProjectTaskTxt->find('all', array(
                'recursive' => -1,
                'conditions' => array('project_task_id' => $pTaskId),
                'fields' => array('ProjectTaskTxt.id', 'ProjectTaskTxt.employee_id', 'ProjectTaskTxt.comment', 'ProjectTaskTxt.created','Employee.first_name', 'Employee.last_name'),
				'joins' => array(
					array(
						'table' => 'employees',
						'alias' => 'Employee',
			...
			//Ticket #1138. Them value check user la project manager.
			...
			$checkPmManager = array();
			$checkPmManager = $this->ProjectEmployeeManager->find('first', array(
				'recursive' => -1,
				'conditions' => array(
			...
			$results['isPmProject'] = !empty($checkPmManager) ? 1 : 0;
        }
		if( !empty($results['result']) ) $this->update_read_status($pTaskId);
        die(json_encode($results));
    }
    public function deleteCommentTxt(){
        $this->layout = false;
        $result = '';
        if(!empty($_POST)){
            $cm_id = $_POST['cm_id'];
            $this->loadModels('ProjectTaskTxt');
            $projectTaskTxt = $this->ProjectTaskTxt->find('first', array(
                'recursive' => -1,
                'conditions' => array('id' => $cm_id),
				'fields' => array('id', 'employee_id')
            ));
			if(!empty($projectTaskTxt)){
				$employee_role = $this->employee_info['Role']['name'];
				$employee_current = $this->employee_info['Employee']['id'];
				$employee_cmt = !empty($projectTaskTxt) ? $projectTaskTxt['ProjectTaskTxt']['employee_id'] : 0;
				if($employee_role == 'admin' || $employee_cmt == $employee_current){
					$this->ProjectTaskTxt->id = $cm_id;
					if($this->ProjectTaskTxt->delete()){
						$result = "success";
					}
				}else{
					$result = "You have not permission to access this function";
				}
			}
        }
        die(json_encode($result));
	...
	* @param: Project task ID
	*/
	public function get_task_info($pTaskId = null, $return = 0){
		if(empty($pTaskId) && !empty($this->data['id'])){
			$pTaskId = $this->data['id'];
		}
		if(empty($pTaskId)){
			$this->cakeError('error404');
		}
		$projectTask =  $this->ProjectTask->find('first', array(
			'recursive' => -1,
			'conditions' => array(
				'ProjectTask.ID' => $pTaskId,
			),
		));
		$projectTask = !empty( $projectTask ) ? $projectTask['ProjectTask'] : array();
		
		
		$result = false;
		$data = '';
		$message = '';
		$employees_actif = array();
		if( $projectTask){
			$project_id = $projectTask['project_id'];
			$this->_checkRole(false, $project_id, array(), $pTaskId);
			$isAjax = $this->params['isAjax'];
			$this->loadModel('ProjectTaskEmployeeRefer');
			$emp_assigneds =  $this->ProjectTaskEmployeeRefer->find('all', array(
				'recursive' => -1,
				'conditions' => array(
					'project_task_id' => $pTaskId,
				),
				'fields' => array('reference_id', 'is_profit_center', 'estimated'),
			));
			$employees_actif = $this->_getTeamEmployeesForLoad($projectTask['project_id'], $projectTask['id'], $projectTask['task_start_date'],  $projectTask['task_end_date']);
			$projectTask['assigned'] = array();
			foreach ( $emp_assigneds as $empl){
				$projectTask['assigned'][] = $empl['ProjectTaskEmployeeRefer'];
			}
						
			// workload
			$projectTask['workload'] = 0;
			if(!empty($projectTask['estimated'])){
				$projectTask['workload'] = $projectTask['estimated'];
			}
			$this->loadModels('ProjectTaskAttachments', 'ProjectTaskAttachmentView', 'ProjectTaskTxt', 'ProjectTaskTxtRefer', 'ActivityRequest', 'ActivityTask', 'Project');
			
			$project_title = $this->Project->find('first', array(
				'recursive' => -1,
				'conditions' => array(
					'id' => $projectTask['project_id']
				),
				'fields' => array('project_name')
			));
			$projectTask ['project_name'] = $project_title['Project']['project_name'];
			// attachment
			$projectTaskAttachments = $this->ProjectTaskAttachments->find('list', array(
				'recursive' => -1,
				'conditions' => array(
			...
			$consumeOfActivityTasks = $consumeds = array();
			if(!empty($listActivityTasks)){
				$consumeOfActivityTasks = $this->ActivityRequest->find('first', array(
					'recursive' => -1,
					'conditions' => array(
				...
				$consumed_by_employee = $this->ActivityRequest->find('all', array(
					'recursive' => -1,
					'conditions' => array(
						'task_id' => $listActivityTasks,
						'value !=' => 0,
					...
					'fields' => array('ActivityRequest.employee_id', 'sum(ActivityRequest.value) as consumed'),
					'group' => array('ActivityRequest.employee_id')
				));
				$consumeds = !empty($consumed_by_employee) ? Set::combine($consumed_by_employee, '{n}.ActivityRequest.employee_id', '{n}.0.consumed') : array();
			}
			if (!empty($consumeOfActivityTasks)) {
				$projectTask['consume'] = (!empty($consumeOfActivityTasks) ) ? round($consumeOfActivityTasks[0]['consumed'], 2) : 0;
			} else {
				$projectTask['consume'] = 0.00;
			}
			
			$projectTask['consumeds'] = $consumeds;
			
			// Get Resource manday
			$resource_md = $projectTask['assigned'];
			if(!empty($consumeds)){
				foreach($consumeds as $c_employee_id =>$c_val){
					if(!in_array($c_employee_id, $resource_md)){
						$c_resource = array(
							'reference_id' => $c_employee_id,
							'is_profit_center' => 0,
						);
						$resource_md[] = $c_resource;
					}
				}
			}
			$employee_manday = array();
			$display_disponibility = isset($this->companyConfigs['display_disponibility']) ? $this->companyConfigs['display_disponibility'] : 0;			
			$display_activity_forecast = isset($this->companyConfigs['display_activity_forecast']) ? $this->companyConfigs['display_activity_forecast'] : 0;
			$projectTask['resource_manday'] = $employee_manday;
			
			// attachment
			$projectTask['attachment_count'] = 0;
			$projectTask['attach_read_status'] = 0;
			if( !empty($task['ProjectTask']['attachment'])) {
				$projectTask['attachment_count'] += 1;
				$projectTask['attach_read_status'] = 1;
			}
			...
			if ( array_key_exists($pTaskId, $projectTaskAttachmentRead )) $projectTask['attach_read_status'] = intval($projectTaskAttachmentRead[$pTaskId]);
			
			// commment
			$projectTask['comment_count'] = 0;
			$projectTask['read_status'] = 0;
			if( !empty($task['ProjectTask']['text_1'])) {
				$projectTask['comment_count'] += 1;
				$projectTask['read_status'] = 1;
			}
			...
			if ( array_key_exists($pTaskId, $projectTaskCommentRead )) $projectTask['read_status'] = intval($projectTaskCommentRead[$pTaskId]);
			
			if($projectTask['task_start_date'] == '0000-00-00') $projectTask['task_start_date'] = '';
			if($projectTask['task_end_date'] == '0000-00-00') $projectTask['task_end_date'] = '';
			
			$result = 'success';
			$data = $projectTask;
		}else{
			$message = __('T�che non trouv�e',true);
		}
		if( $return) return ( $result ? $data : $message);		
		$data = array(
			'result' => $result,
			'data' => $data,
			'message' => $message,	
			'employees_actif' => $employees_actif,
		);
		if( $isAjax ){
	...
	public function getManDayForResource(){
		if(!empty($this->data)){
			$list_employee = array();
			foreach($this->data['employees'] as $index =>$value){
				
				$refer = explode('-', $value['id']);
				$c_resource = array(
					'reference_id' => $refer[0],
					'is_profit_center' => $refer[1],
				);
				$list_employee[] = $c_resource;
				
			}
			$sDate = $this->data['task_data']['task_start_date'];
			$eDate = $this->data['task_data']['task_end_date'];
			$taskOnView = $this->data['task_data'];
			$employee_manday = array();
			$date_pattern = '/^[0-9]{4}-(((0[13578]|(10|12))-(0[1-9]|[1-2][0-9]|3[0-1]))|(02-(0[1-9]|[1-2][0-9]))|((0[469]|11)-(0[1-9]|[1-2][0-9]|30)))$/';
			if( preg_match($date_pattern, $sDate, $matches) &&  preg_match($date_pattern, $eDate, $matches)){
				$employee_manday = $this->getManDayOfEmployeesForPopup('project', $this->data['project_id'], $list_employee, $sDate, $eDate, $taskOnView);
			}
			die(json_encode($employee_manday));
		}
	}
	public function switchEmployeeHistory(){
		$success = false;
		if(!empty($_POST)){
			$this->loadModel('HistoryFilter');
	...
	function batchUpdateTasksByProject($project_id = null, $return = false){
		$result = false;
		$data = array();
		$message = '';
		$saved = $employee_ids = $pcs = $list_empl_assign = array();
		
		if ($this->_checkRole(true, $project_id) && !empty($this->data['ProjectTask']['task_id'])){
			$this->loadModels('Employee', 'ProfitCenter', 'ProjectTaskEmployeeRefer');
			if( !empty( $this->data['task_assign_to_id'])){
				$this->data['ProjectTask']['task_assign_to_id'] = $this->data['task_assign_to_id'];
			}
			...
			$tasks = $this->ProjectTask->find('all', array(
				...
				'fields' => array('id', 'task_status_id', 'task_start_date', 'task_end_date', 'estimated', 'is_nct', 'predecessor' )
			));
			// Validate input
			if( empty( $data['task_id'])){
				$message = __('No exists tasks', true);
				$this->_functionStop($result, $data, $message, false, array('action' => 'index', $project_id));
			}
			if( count($tasks) != count($data['task_id'])){
				$message = __('No exists tasks', true);
				$this->_functionStop($result, $data, $message, false, array('action' => 'index', $project_id));
			}
			// Check is_nct: If is_nct: update status only
			// check has predecessor
			$has_nct = 0;
			$_has_predecessor = 0;
			foreach( $tasks as $task){
				if( $task['ProjectTask']['is_nct'] != '0'){
					$has_nct = 1;
					break;
				}
				if( !empty($task['ProjectTask']['predecessor'])){
					$_has_predecessor = 1;
				}
			}
			if( $has_nct ){
				if( isset($data['task_start_date'])) unset( $data['task_start_date']);
				if( isset($data['task_end_date']))unset( $data['task_end_date']);
				if( isset($data['add_assigned_to_id']))unset( $data['add_assigned_to_id']);
				if( isset($data['replace_assigned_to_id']))unset( $data['replace_assigned_to_id']);
			}elseif($_has_predecessor){
				if( isset($data['task_start_date'])) unset( $data['task_start_date']);
				if( isset($data['task_end_date']))unset( $data['task_end_date']);
			}
			// END Check is_nct
			
			// Check predecessor: If predecessor: can not update the date
			if( !$has_nct && !$_has_predecessor ){
				$is_predecesor = $this->ProjectTask->find('list', array(
					'recursive' => -1,
					'conditions' => array(
						'project_id' => $project_id,
						'predecessor' => $data['task_id'],
					),
					'fields' => array('id', 'predecessor')
				));
				if( !empty( $is_predecesor )) {
					if( isset($data['task_start_date'])) unset( $data['task_start_date']);
					if( isset($data['task_end_date'])) unset( $data['task_end_date']);
				}
			}
			// end Check predecessor:
			// END Validate input 
			// debug( $data); exit;
			if( $data['task_status_id'] ) $saved['task_status_id'] =  $data['task_status_id'];
			...
			if( !empty( $data['task_start_date'] )){
				if( !preg_match($date_pattern, $data['task_start_date'], $matches)) $this->_functionStop($result, $data, __('The start_date not in date format.', true), false, array('action' => 'index', $project_id));
			}
			if( !empty( $data['task_end_date'] )){
				if( !preg_match($date_pattern, $data['task_end_date'], $matches)) $this->_functionStop($result, $data, __('The end_date not in date format.', true), false, array('action' => 'index', $project_id));
			}
			if( !empty( $data['task_start_date'] ) && empty($data['task_end_date'])){
				$data['task_end_date'] = $data['task_start_date'];
			}
			if( empty( $data['task_start_date'] ) && !empty($data['task_end_date'])){
				$data['task_start_date'] = $data['task_end_date'];
			}
			if( !empty( $data['task_start_date'] ) && !empty($data['task_end_date'])){
				$start = date_create_from_format('d-m-Y',  $data['task_start_date']);
				$end = date_create_from_format('d-m-Y',  $data['task_end_date']);
				if( $start > $end){
					$this->_functionStop($result, $data, __('The end date must be greater than start date.', true), false, array('action' => 'index', $project_id));
				}
				$saved['task_start_date'] = (string)$start->format('Y-m-d');
				$saved['task_end_date'] = (string)$end->format('Y-m-d');
				$saved['duration'] = $this->getWorkingDays($saved['task_start_date'], $saved['task_end_date'], '');
			}
			
			// debug( $saved); exit;
			// Case Update
			if( !empty($data['update_existing_employee'])){
				// check list new assign
				if( !empty( $data['add_assigned_to_id'])){
					$data['add_assigned_to_id'] = array_filter($data['add_assigned_to_id']);
					foreach($data['add_assigned_to_id'] as $e){
						$e = explode('-', $e);
						if( !empty($e[1])) $pcs[] = $e[0];
						else $employee_ids[] = $e[0];
					}
					...
					if( count( $employees) != count($employee_ids)){
						$message = __('No exists employees', true);
						$this->_functionStop($result, $data, $message, false, array('action' => 'index', $project_id));
					}
					$profits = $this->ProfitCenter->find('list', array(
						'recursive' => -1,
						'conditions' => array(
							'ProfitCenter.company_id' => $this->employee_info['Company']['id'],
							'ProfitCenter.id' => $pcs,
						),
						'fields' => array('id')
					));
					if( count( $profits) != count($pcs)){
						$message = __('No exists profit_center', true);
						$this->_functionStop($result, $data, $message, false, array('action' => 'index', $project_id));
					}
				...
				// Save Task 
				if( !empty( $saved)){
					// Cai nay loi do cakePHP khong dua date vao dau nhay cua string ""
					// $result = $this->ProjectTask->updateAll(
						// $saved, // data
						// array(
							// 'ProjectTask.idx' => $data['task_id']
						// ) //conditions
					// );
					foreach( $data['task_id'] as $t_id){
						$this->ProjectTask->id = $t_id;
						$result = $this->ProjectTask->save( $saved); 
					}
				}
				// Save current 
				if( isset( $data['update_estimated']) && $data['update_estimated'] !== ''){
					$this->ProjectTaskEmployeeRefer->updateAll(
						array('ProjectTaskEmployeeRefer.estimated' => floatval($data['update_estimated'])), // data
						array('ProjectTaskEmployeeRefer.project_task_id' => $data['task_id']) //conditions
					);
				}
				// add new assigned
				$new_assigned = array_merge(array($employee_ids),array($pcs));
				$isAlreadyAssigneds = array();
				$refs = $this->ProjectTaskEmployeeRefer->find('all', array(
					'recursive'=> -1,
					'conditions' => array(
				...
				foreach( $refs as $ref){
					...
					$isAlreadyAssigneds[$ref['project_task_id']][$ref['is_profit_center']][$ref['reference_id']] = $ref['id'];
				}
				foreach( $data['task_id'] as $task_id){
					foreach($new_assigned as $is_pc => $refs){
						foreach( $refs as $ref){
							if( empty($isAlreadyAssigneds[$task_id][$is_pc][$ref])){
								$this->ProjectTaskEmployeeRefer->create();
								$this->ProjectTaskEmployeeRefer->save(array(
									'reference_id'=> $ref, 
									'project_task_id'=> $task_id,
									'estimated' => floatval($data['add_estimated']),
									'is_profit_center' => $is_pc,
									'created' => time(),
									'updated' => time(),
									
								));
							}else{
								$this->ProjectTaskEmployeeRefer->id = $isAlreadyAssigneds[$task_id][$is_pc][$ref];
								$this->ProjectTaskEmployeeRefer->save(array(
									'estimated' => floatval($data['add_estimated']),
									'updated' => time(),
			...
			// END Case Update
			// Case Replace 
			}elseif( isset( $data['update_existing_employee'])){
				// validate
				if( !empty($data['replace_assigned_to_id'])){
					$data['replace_assigned_to_id'] = array_filter($data['replace_assigned_to_id']);
					foreach($data['replace_assigned_to_id'] as $e){
						$e = explode('-', $e);
						if( !empty($e[1])) $pcs[] = $e[0];
						else $employee_ids[] = $e[0];
					}
				...
				$this->ProjectTaskEmployeeRefer->deleteAll(array(
					'ProjectTaskEmployeeRefer.project_task_id' => $data['task_id'],
				), false);
				foreach( $data['task_id'] as $task_id){
					$_last_id = null;
					foreach(array_merge(array($employee_ids),array($pcs)) as $is_pc => $refs){
						foreach( $refs as $ref){
							$this->ProjectTaskEmployeeRefer->create();
							$this->ProjectTaskEmployeeRefer->save(array(
								'reference_id'=> $ref, 
								'project_task_id'=> $task_id,
								'estimated' => floatval($data['replace_estimated']),
								'is_profit_center' => $is_pc,
								'created' => time(),
								'updated' => time(),
			...
			// Case Replace without workload 
			elseif( !isset( $data['update_existing_employee'])){
				if( !empty( $data['edit_assigned_to_id']) ){
					$data['edit_assigned_to_id'] = array_filter($data['edit_assigned_to_id']);
					foreach($data['edit_assigned_to_id'] as $e){
						$e = explode('-', $e);
						if( !empty($e[1])) $pcs[] = $e[0];
						else $employee_ids[] = $e[0];
					}
			...
			// END Case Replace without workload 
			/* Update data after save task */
			// exit;
			// Update Sum Estimated to task 
			$sum_es = $this->ProjectTaskEmployeeRefer->find('all', array(
				'recursive' => -1,
				'conditions' => array( 'ProjectTaskEmployeeRefer.project_task_id' => $data['task_id']),
				'fields' => array('ProjectTaskEmployeeRefer.project_task_id', 'SUM(ProjectTaskEmployeeRefer.estimated) as sum_estimated'),
				'group' => array( 'ProjectTaskEmployeeRefer.project_task_id'),
			));
			$sum_es = !empty( $sum_es) ? Set::combine( $sum_es, '{n}.ProjectTaskEmployeeRefer.project_task_id', '{n}.0.sum_estimated') : array();
			foreach( $sum_es as $task_id => $task_estimated){
				$this->ProjectTask->id = $task_id;
				$result = $this->ProjectTask->save( array('estimated' => $task_estimated)); 
			}
			// callback
			$this->updateEmployeeAssignedToTeam($project_id, $list_empl_assign);
			$this->_updateParentTask($project_id);
			...
			$tasks = $this->ProjectTask->find("all", array(
				'recursive' => -1,
				'conditions' => array( 'id' => $data['task_id'])
			));
			foreach( $tasks as $task ){
	...
	function batchDeleteTasks(){
		...
		if( $this->_checkRole(false, $project_id)){
			...
			if( count( $tasks) != count($data['task_id'])) $this->_functionStop($result, $data, __('Not saved', true), false, array('action' => 'index', $project_id));
			foreach($data['task_id'] as $task_id){
				$this->destroyTaskJson($task_id, true);
			}
			$project_planed_phase_ids = !empty( $tasks ) ? Set::combine($tasks, '{n}.ProjectTask.project_planed_phase_id', '{n}.ProjectTask.project_planed_phase_id') : array();
			foreach( $project_planed_phase_ids as $project_planed_phase_id){
				$this->_syncPhasePlanTimeAfterDeleteTask($project_id, $project_planed_phase_id);
			}
			$this->writeLog($tasks, $this->employee_info, __("Batch delete Task", true));
			$result = true;
			$message = __('Deleted', true);
		}else{
			$this->_functionStop($result, $data, __('You have not permission to access this function', true), false, array('action' => 'index', $project_id));
		}
		$this->_functionStop($result, $data, $message, false, array('action' => 'index', $project_id));
	}
	/* 
	* Ticket #2121
	* Function recalculated_start_end_date_duration_of_task
	* @param: start date, end date, duration
	*/
	function calculate_enddate(){
        $result = array();
		if(!empty($_POST)){
			$result['duration'] = $_POST['data']['duration'];
			$result['start_date'] = $_POST['data']['start_date'];
			$result['end_date'] = $this->_durationEndDate($result['start_date'], null, $result['duration']);
			$date = strtotime($result['end_date']); // 1553641200
			$result['end_date'] = date('d-m-Y', $date);
		}
		echo json_encode($result);
		exit;
	}
	/*
	 * Function batchUpdateWorkloadByConsumed
	 * Descriptions: Adjusted the workload with consumed
	 * @parrams:  POST[data] => array{
			 project_id => (int) <project_id>
			 task_id => array() <task_ids>
		 }
	 * Permission: admin, PM can write
	 * Result: 
		* if NTC selected then do not display adjusted the workload
		* If the task has no employee assigned to then save workload = consumed
		* Save workload for each employee = consumed of his
		* If an employee is not assigned to the task: Assigned him to the task
		* If an employee is assigned to the task but not have consumed: workload = 0
		* update team after saved
		* Check staffing after saved
	 * Ticket #2041
	 */
	
	public function batchUpdateWorkloadByConsumed(){
		$result = false;
		$data = array();
		$message = '';
		$data = array_merge( array(
		...
		$project_id = $data['project_id'];
		if( $this->companyConfigs['manual_consumed'] != '0') $this->_functionStop($result, $data, __('Can not update workload when manual consumed is enabled', true), false, array('action' => 'index', $project_id));
		//Check Role
		if( $this->_checkRole(false, $project_id)){
			$this->loadModels( 'ProjectTaskEmployeeRefer');
			$tasks = $this->ProjectTask->find('all', array(
				...
				'fields' => array('id', 'estimated', 'manual_consumed', 'project_id', 'is_nct')
			));
			if( count( $tasks) != count($data['task_id'])) $this->_functionStop($result, $data, __('Not saved', true), false, array('action' => 'index', $project_id));
			
			// Get consumed by task 
			$activityTasks = $this->_getActivityTasks($data['task_id']);
			$activityRequests = $this->_getTaskConsumedByEmployee($activityTasks);
			
			//Get assignedTos
			$assignedTos = $this->ProjectTaskEmployeeRefer->find('all', array(
				'recursive' => -1,
				'conditions' => array(
					'project_task_id' => $data['task_id'],
				)
			));
			$assignedTos = !empty( $assignedTos ) ? Set::combine(
				$assignedTos, 
				array('{0}-{1}', '{n}.ProjectTaskEmployeeRefer.reference_id', '{n}.ProjectTaskEmployeeRefer.is_profit_center'),
				'{n}.ProjectTaskEmployeeRefer.id',
				'{n}.ProjectTaskEmployeeRefer.project_task_id'
				) : array();
			
			// 1. Update all workload to zero
			$this->ProjectTaskEmployeeRefer->updateAll(
				array( 'ProjectTaskEmployeeRefer.estimated' => 0), // data
				array( 'ProjectTaskEmployeeRefer.project_task_id' => $data['task_id']) //conditions
			);
			// 2. Update new workload
			$projectTasks = array_flip( $activityTasks);
			$list_empl_assign = array();
			foreach($activityRequests as $activity_task_id => $emp_consumed){
				$task_id = $projectTasks[$activity_task_id];
				foreach($emp_consumed as $_emp => $_consumed){
					$ref = $_emp . '-0';
					if( empty($assignedTos[$task_id][$ref])){
						$this->ProjectTaskEmployeeRefer->create();
						$this->ProjectTaskEmployeeRefer->save(array(
							'reference_id'=> $_emp, 
							'project_task_id'=> $task_id,
							'estimated' => floatval($activityRequests[$activity_task_id][$_emp]),
							'is_profit_center' => 0,
							'created' => time(),
							'updated' => time(),
						...
						$this->ProjectTaskEmployeeRefer->id = $assignedTos[$task_id][$ref];
						$this->ProjectTaskEmployeeRefer->save(array(
							'estimated' => floatval($activityRequests[$activity_task_id][$_emp]),
							'updated' => time(),
			...
			$this->writeLog($tasks, $this->employee_info, __("Adjusted the workload with consumed", true));
			// 4. callback
			$this->updateEmployeeAssignedToTeam($project_id, $list_empl_assign);
			$this->_updateParentTask($project_id);
	...
	private function _getTaskConsumedByEmployee($activityTasks){
		$this->loadModel('ActivityRequest');
        $activityRequests = $this->ActivityRequest->find('all', array(
            'recursive' => -1,
            'fields' => array(
                ...
                'employee_id',
                'task_id',
                'SUM(CASE WHEN `status` = 2 THEN `value` ELSE 0 END) AS consumed',
            ),
            'group' => array('task_id', 'employee_id'),
            'conditions' => array(
                'task_id' => $activityTasks,
            )
        ));
        $activityRequests = !empty($activityRequests) ? Set::combine($activityRequests, '{n}.ActivityRequest.employee_id', '{n}.0.consumed', '{n}.ActivityRequest.task_id') : array();; 
        return $activityRequests;
	}
	function importUpdateTask(){
		...
		if (!empty($this->data)) {
			$result = true;
			$message = '';
			// Validated data submit
			$project_id = $this->data['project_id'];
			$company_id = $this->employee_info['Company']['id'];
			// Validate phase planed
			if(!empty($this->data['project_planed_phase_text'])){
				$project_phase_name = $this->ProjectPhase->find('first',array(
					'recursive' => -1,
					'conditions' => array(
						'name' => trim($this->data['project_planed_phase_text']),
						'company_id' => $company_id,
					),
					'fields' => array('id', 'name', 'color')
				));
				if(!empty($project_phase_name)){
					$project_phase_id = $project_phase_name['ProjectPhase']['id'];
					$project_phase_color = $project_phase_name['ProjectPhase']['color'];
					$phase_plane_conds = array(
						'project_id' => $project_id,
						'project_planed_phase_id' => $project_phase_id,
						'project_part_id' => null,
					);
					if(!empty($this->data['project_planed_part_text'])){
						// Validated linked Part - Phase
						$project_part = $this->ProjectPart->find('first',array(
							'recursive' => -1,
							'conditions' => array(
								'title' => trim($this->data['project_planed_part_text']),
								'project_id' => $project_id,
							),
							'fields' => array('id', 'title')
						));
						if(!empty($project_part)){
							$project_part_id = $project_part['ProjectPart']['id'];
							$phase_plane_conds['project_part_id'] = $project_part_id;
						}else{
							 $this->ProjectPart->create();
							 $res_part = $this->ProjectPart->save(array(
								'title' => trim($this->data['project_planed_part_text']),
								'project_id' => $project_id,
							 ));
							 if(!empty($res_part)){
								 $phase_plane_conds['project_part_id'] = $this->ProjectPart->id;
							 }
						}
					}
					$projectPhasePlan = $this->ProjectPhasePlan->find('first',array(
						'recursive' => -1,
						'conditions' => $phase_plane_conds,
						'fields' => array('id')
					));
					if(!empty($projectPhasePlan)){
						$this->data['project_planed_phase_id'] = $projectPhasePlan['ProjectPhasePlan']['id'];
					}else{
						// Create phase planed
						if(!empty($this->data['task_start_date'])){
							$phase_plane_conds['phase_planed_start_date'] = $this->data['task_start_date'];
							$phase_plane_conds['phase_real_start_date'] = $this->data['task_start_date'];
						}
						if(!empty($this->data['task_end_date'])){
							$phase_plane_conds['phase_planed_end_date'] = $this->data['task_end_date'];
							$phase_plane_conds['phase_real_end_date'] = $this->data['task_end_date'];
						}
						$phase_plane_conds['color'] = !empty($project_phase_color) ? $project_phase_color : '#004380';
						$phase_plane_conds['created'] = time();
						$phase_plane_conds['updated'] = time();
						
						$this->ProjectPhasePlan->create();
						if ($this->ProjectPhasePlan->save($phase_plane_conds)) {
							$this->data['project_planed_phase_id'] = $this->ProjectPhasePlan->id;
						}else{
							$result = false;
							$message = __('Phase not created', true);
						}
					}
				}else{
					// can't save
			...
			if(!empty($this->data['milestone_text'])){
				$projectMilestones = $this->ProjectMilestone->find('first', array(
					'recursive' => -1,
					'conditions' => array(
						'project_id' => $project_id,
						'project_milestone' => trim($this->data['milestone_text']),
					),
					'fields' => array('id')
				));
				if(!empty($projectMilestones)){
					$this->data['milestone_id'] = $projectMilestones['ProjectMilestone']['id'];
				}else{
					// Create new milestone without day
					$miles = array(
						'project_id' => $project_id,
						'project_milestone' => $this->data['milestone_text'],
					);
					...
					if(!empty($listWeight)){
						...
						$miles['weight'] = $maxWeight + 1;
					}
					$miles['milestone_date'] = '0000-00-00';
					$miles['validated'] = 0;
					$miles['created'] = time();
					$miles['updated'] = time();
					$this->ProjectMilestone->create();
					if ($this->ProjectMilestone->save($miles)) {
						$this->data['milestone_id'] = $this->ProjectMilestone->id;
					}
				}
			}
			if(!empty($this->data['task_title'])){
				$this->data['parent_id'] = 0;
				if(!empty($this->data['task']) && !empty($this->data['sub_task'])){
					// Get parent id
					$parent_task = $this->ProjectTask->find('first', array(
						'recursive' => -1,
						'conditions' => array(
							'project_id' => $project_id,
							'task_title' => trim($this->data['task']),
							'project_planed_phase_id' => $this->data['project_planed_phase_id']
						),
						'fields' => array('id', 'project_id')
					));
					if(!empty($parent_task)){
						$this->data['parent_id'] = $parent_task['ProjectTask']['id'];
					}else{
						$result = false;
						$message = __('Task of sub task doesn\'t exist', true);
					}
				}
				$project_task = $this->ProjectTask->find('first', array(
					'recursive' => -1,
					'conditions' => array(
						...
						'task_title' => trim($this->data['task_title']),
						'project_planed_phase_id' => $this->data['project_planed_phase_id']
					),
					'fields' => array('id', 'project_id')
				));
				if(!empty($result)){
					if(!empty($project_task)){
						// Update task
						$this->_checkRole(false, $project_id);
						$this->data['id'] = $project_task['ProjectTask']['id'];
						$result = $this->updateTaskJson($project_id, $this->data['id'], $this->data);
					}else{
						// Created task
						$this->_checkRole(false, $project_id);
						unset($this->data['id']);
						$result = $this->createTaskJson($project_id, $this->data);
					}
				}else{
	...
	function deleteTaskWhenImport(){
		$this->loadModels('HistoryFilter', 'Translation', 'Project', 'ProjectTask', 'ProjectPhasePlan', 'ProjectPart', 'ProjectPhase', 'ProjectMilestone', 'Menu','ActivityTask','ActivityRequest');
		$this->layout = 'ajax';
        $success = $result = false;
		if(!empty($this->data)){
			foreach($this->data as $key => $project_id){
				$this->_checkRole(false, $project_id);
				$listTasks = $this->ProjectTask->find('list', array(
					'recursive' => -1,
					'conditions' => array(
						'project_id' => $project_id
					),
				...
				if( !empty($tasksHasChild)){
					foreach( $tasksHasChild as $task_id){
						unset( $listTasks[$task_id]);
					}
				}
				$listActivityTasksLinked = $this->ActivityTask->find('list', array(
					'recursive' => -1,
					'conditions' => array(
						'project_task_id' => array_keys($listTasks)
					),
					'fields' => array('project_task_id', 'id')
				));
				//List task not have activity tasks.
				$listTaskNoActivityTasks = array_diff($listTasks, $listActivityTasksLinked);
				//cac task co timesheet(bao gom: moi nhap, request, reject,consumed)
				$listTaskInTimesheet = $this->ActivityRequest->find('list', array(
					'recursive' => -1,
					'conditions' => array(
						// 'status' => 2,
						// 'value !=' => 0,
				...
				//list task chua nhap timesheet.
				$listActivityTasksLinkedNoTimesheet = array_diff( $listActivityTasksLinked, $listTaskInTimesheet);
				/* Delete table linked to project task */
				if(!empty($listActivityTasksLinkedNoTimesheet)){
					foreach($listActivityTasksLinkedNoTimesheet as $projectTaskId => $val){
						$this->destroyTaskJson($projectTaskId, true);
					}
				}
				if(!empty($listTaskNoActivityTasks)){
					foreach($listTaskNoActivityTasks as $projectTaskId => $val){
						$this->destroyTaskJson($projectTaskId, true);
					}
				}
				
				//Phia tren xoa sub sub task(neu co). Code nay dung de xoa sub task
				if(!empty($tasksHasChild)){
					$tasksHasChildSubTask = $this->ProjectTask->find('list', array(
						'recursive' => -1,
						'conditions' => array(
							'parent_id' => $tasksHasChild
						),
						'fields' => array('parent_id', 'parent_id'),
						'group' => array('parent_id')
					));
					if( !empty($tasksHasChildSubTask)){
						foreach( $tasksHasChildSubTask as $task_id){
							unset( $tasksHasChild[$task_id]);
						}
					}
					$listActivityTasksLinked = $this->ActivityTask->find('list', array(
						'recursive' => -1,
						'conditions' => array(
							'project_task_id' => array_keys($tasksHasChild)
						),
						'fields' => array('project_task_id', 'id')
					));
					$listTaskNoActivityTasks = array_diff($tasksHasChild, $listActivityTasksLinked);
					$listTaskInTimesheet = $this->ActivityRequest->find('list', array(
						'recursive' => -1,
						'conditions' => array(
							'task_id' => array_values($listActivityTasksLinked)
				...
				//Phia tren xoa sub task(neu co). Code nay dung de xoa parent task
				if(!empty($tasksHasChild) && !empty($tasksHasChildSubTask)){
					$listActivityTasksLinked = $this->ActivityTask->find('list', array(
						'recursive' => -1,
						'conditions' => array(
							'project_task_id' => array_keys($tasksHasChildSubTask)
						),
						'fields' => array('project_task_id', 'id')
					));
					$listTaskNoActivityTasks = array_diff($tasksHasChildSubTask, $listActivityTasksLinked);
					$listTaskInTimesheet = $this->ActivityRequest->find('list', array(
						'recursive' => -1,
						'conditions' => array(
							'task_id' => array_values($listActivityTasksLinked)
			...
			$success = $result = true;
			$response = array("success" => $success, "message" => $result);
			die(json_encode($response));
		}else{
			$response = array("success" => $success, "message" => $result);
			die(json_encode($response));
