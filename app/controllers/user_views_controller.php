...
class UserViewsController extends AppController {
    ...
    function index_bak() {
        ...
        if ($this->is_sas)
            $userViews = $this->UserView->find('all');
        else {
            $userViews = array();
            $userViews = $this->UserView->find('all', array('conditions' => array('UserView.employee_id' => $this->employee_info['Employee']['id'])));
        }

        $empList = array();
        foreach ($userViews as $u) {
            if (!in_array($u['Employee'], $empList))
                $empList[] = $u['Employee'];
        }

        $this->set('userViews', $userViews);
        $this->set('search_manager', $empList);
        $this->set('default_view', $this->UserDefaultView->find('list', array(
                    'conditions' => array('UserDefaultView.employee_id' => $this->employee_info["Employee"]["id"]),
                    'fields' => array('UserDefaultView.user_view_id'))));
    }
    ...
    public function index() {
        ...
        if ($this->is_sas) {
            $userViews = $this->UserView->find('all', array(
                'recursive' => 0,
                'fields' => array('UserView.id', 'UserView.name', 'UserView.mobile', 'UserView.description', 'UserView.created_date', 'UserView.public', 'UserView.employee_id', 'Employee.first_name', 'Employee.last_name'),
		...
		$conpanyDefaultView = array();
        if($model=='project') {
            $statusView = $this->UserStatusView->find('all', array(
                'recursive' => -1,
                'conditions' => array('employee_id' => $this->employee_info["Employee"]["id"]),
                'fields' => array('employee_id', 'user_view_id', 'progress_view', 'oppor_view', 'archived_view', 'model_view', 'mobile')));
    ...
    public function toggle($model='project',$viewId = null, $switch = null) {
        ...
        if (!$switch && $defaultView) {
            $this->UserDefaultView->id = $defaultView['UserDefaultView']['id'];
            $this->UserDefaultView->save(array(
                        'user_view_id' => 0
                    ));
            //$this->UserDefaultView->delete($defaultView['UserDefaultView']['id']);
            $this->Session->setFlash(__('Saved', true));
            //$this->Session->setFlash(sprintf(__('The %s view has been set as not default', true), '<b>"' . $userView["UserView"]["name"] . '"</b>'), 'success');
        } elseif ($switch) {
            $this->UserDefaultView->create();
            if ($defaultView) {
                $this->UserDefaultView->id = $defaultView['UserDefaultView']['id'];
            }
            if ($this->UserDefaultView->save(array(
                        'employee_id' => $this->employee_info["Employee"]["id"],
                        'user_view_id' => $viewId,
                        'model' => $model
                    ))) {
                $this->Session->setFlash(__('Saved', true));
                //$this->Session->setFlash(sprintf(__('The %s view has been set as default', true), '<b>"' . $userView["UserView"]["name"] . '"</b>'), 'success');
            } else {
    ...
    public function toggle_mobile($id = null, $value = null){
        ...
        if( !$statusView ){
            ...
            $this->UserStatusView->save(array(
                'user_view_id' => $id,
                'employee_id' => $eid,
                'mobile' => $value
            ));
        } else {
            $this->UserStatusView->id = $statusView['UserStatusView']['id'];
            $this->UserStatusView->saveField('mobile', $value);
        }
        $this->Session->setFlash(__('Saved', true));
        $this->redirect(array('action' => 'index?model=' . $model));
    }
    ...
    public function toggle_public($model='project',$viewId = null, $switch = null) {
        ...
        $userView = $this->UserView->find('first', array('recursive' => -1, 'fields' => array('name', 'employee_id'), 'conditions' => $conditions));
        if (!$userView || $this->employee_info["Role"]["name"] !== 'admin') {
            $this->Session->setFlash(sprintf(__('The #%s view does not exist!', true), '<b>"' . $viewId . '"</b>'), 'error');
            $this->redirect(array('action' => 'index?model='.$model));
        }
        $this->UserView->id = $viewId;
        if ($this->UserView->saveField('public', !empty($switch))) {
            if (!empty($switch)) {
                $this->Session->setFlash(__('Saved', true));
                //$this->Session->setFlash(sprintf(__('The %s view has been set as public view', true), '<b>"' . $userView["UserView"]["name"] . '"</b>'), 'success');
            } else {
                ...
                $this->Session->setFlash(__('Saved', true));
                //$this->Session->setFlash(sprintf(__('The %s view has been set as not public view', true), '<b>"' . $userView["UserView"]["name"] . '"</b>'), 'success');
            }
        } else {
        ...
        if($model=='project') {
			...
			if( isset($enableWidgets['finance_plus']) && $canSeeBudget ){
				...
				foreach($finan_type as $key => $type){
					$othersField['ProjectWidget.FinancePlus'. $type] = $type;
				}
			}
			...
			if( isset($enableWidgets['internal_cost']) && $canSeeBudget ){
				$othersField['ProjectWidget.InternalBudgetMD'] = $enableWidgets['internal_cost'] . ' ' . __('M.D', true);
				$currency = $this->getCurrencyOfBudget();
				$othersField['ProjectWidget.InternalBudgetEuro'] = $enableWidgets['internal_cost'] . ' ' . $currency;
			}
		   ...
		   $datas = $this->Translation->find('all', array(
            ...
            $kpi = json_decode(!empty($this->companyConfigs['kpi_settings']) ? !empty($this->companyConfigs['kpi_settings']) : '');
            $new = array();
    ...
    public function toggle_status_views($fields = null, $viewId = null, $switch = null) {
        ...
        if (!$switch && $statusView) {
            ...
            if ($this->UserStatusView->save(array(
                        'employee_id' => $this->employee_info["Employee"]["id"],
                        'user_view_id' => $viewId,
                        $fields => 1
                    ))) {
    ...
    public function toggle_status_views_activity($fields = null, $viewId = null, $switch = null) {
        ...
        if (!$switch && $statusView) {
            $this->UserStatusViewActivity->id = $statusView['UserStatusViewActivity']['id'];
            $this->UserStatusViewActivity->save(array(
                        $fields => 0
                    ));
            //$this->UserDefaultView->delete($defaultView['UserDefaultView']['id']);
            $this->Session->setFlash(__('Saved', true), 'success');
            ...
            if ($this->UserStatusViewActivity->save(array(
                        'employee_id' => $this->employee_info["Employee"]["id"],
                        'user_view_id' => $viewId,
                        $fields => 1
                    ))) {
    ...
    public function toggle_status_views_sales($fields = null, $viewId = null, $switch = null) {
        ...
        if (!$switch && $statusView) {
            $this->UserStatusViewSale->id = $statusView['UserStatusViewSale']['id'];
            $this->UserStatusViewSale->save(array(
                        $fields => 0
                    ));
            $this->Session->setFlash(__('Saved', true), 'success');
        } elseif ($switch) {
            ...
            if ($this->UserStatusViewSale->save(array(
                        'employee_id' => $this->employee_info["Employee"]["id"],
                        'user_view_id' => $viewId,
                        $fields => 1
                    ))) {
    ...
    public function toggle_status_views_sale_deals($fields = null, $viewId = null, $switch = null) {
        ...
        if (!$switch && $statusView) {
            $this->UserStatusViewSaleDeal->id = $statusView['UserStatusViewSaleDeal']['id'];
            $this->UserStatusViewSaleDeal->save(array(
                        $fields => 0
                    ));
            $this->Session->setFlash(__('Saved', true), 'success');
        } elseif ($switch) {
            ...
            if ($this->UserStatusViewSaleDeal->save(array(
                        'employee_id' => $this->employee_info["Employee"]["id"],
                        'user_view_id' => $viewId,
                        $fields => 1
                    ))) {
     ...
     * setting default view for new user
     */
	
	public function company_default_view($model = 'project') {
        $company_id = $this->employee_info['Company']['id'];
		$publicViews = $this->UserView->find('all', array(
			'recursive' => 0,
			'fields' => array('UserView.id', 'UserView.name', 'UserView.mobile', 'UserView.description', 'UserView.created_date', 'UserView.public', 'UserView.employee_id', 'Employee.first_name', 'Employee.last_name'),
        ...
        // $isAdmin = (isset($this->employee_info['Role']['name']) && $this->employee_info['Role']['name'] === 'admin');
        $this->set(compact('publicViews', 'company_id','conpanyDefaultView', 'model'));
	}
	public function update_default_config(){
		$company_id = $this->employee_info['Company']['id'];
		$model = !empty( $this->data['model']) ? $this->data['model'] : 'project';
		
		$result = 'failed';
		$data = array();
		$defaultData = array();
		$message = '';
		// ktra view co thuoc company
		$hasView = $this->UserView->find('first', array(
			'recursive' => -1,
			'conditions' => array(
				'UserView.model'=>$model,
				'public' => '1',
		...
		$viewID = !empty($this->data['viewID']) ? $this->data['viewID'] : '';
		$field = !empty($this->data['field']) ? $this->data['field'] : '';
		if( $hasView && !empty($viewID) && !empty($field)){
			$this->loadModel('CompanyViewDefault');
			$last_config = $this->CompanyViewDefault->find('first', array(
				'recursive' => -1,
				'conditions' => array(
					'company_id' => $company_id,
					'user_view_id' => $viewID,
			...
			if( empty($last_config)){
				$this->CompanyViewDefault->create();
				$saveData = array(
					'company_id' => $company_id,
					'user_view_id' => $viewID,
					$field => 1,
				...
				$saveData = $last_config['CompanyViewDefault'];
				$saveData[$field] = empty($saveData[$field]) ? 1 : 0;
			}
			// debug($saveData); exit;
			$data = $this->CompanyViewDefault->save($saveData);
			if( !empty($data)){
				$result = 'success';			
				if( strpos($field, 'default') !== false && !empty( $data['CompanyViewDefault'][$field]) ){
					$last_defaults =  $this->CompanyViewDefault->find('all', array(
						'recursive' => -1,
						'conditions' => array(
							'company_id' => $company_id,
							$field => 1,
					...
					foreach( $last_defaults as $last_default){
						$last_default['CompanyViewDefault'][$field] = 0;
						$defaultData[] = $this->CompanyViewDefault->save($last_default);
					}
				}
			}else{
				$this->Session->setFlash(__('Save data failed', true), 'error');
				$message = __('Save data failed', true);
			}
		}else{
			$this->Session->setFlash(__('View not found', true), 'error');
			$message = __('View not found', true);
		}
