 ...
 */
class ProjectBudgetExternalsPreviewController extends AppController {
    /**
     *
     ...
     * @access public
     */
    var $name = 'ProjectBudgetExternalsPreview';
    //var $layout = 'administrators';
    var $uses = array('ProjectBudgetExternals');
    /**
    ...
    function index($project_id = null) {
        ...
        $this->loadModels('ProjectBudgetExternal','ProjectTask', 'Project', 'ProjectBudgetInternalDetail', 'BudgetProvider', 'BudgetType', 'BudgetFunder', 'ProfitCenter');
		...
		$valBudgetSynsExternal = $this->ProjectBudgetSyns->find('first', array(
			'recursive' => -1,
			'conditions' => array(
				'project_id' => $project_id
			),
			'fields' => array('external_costs_budget', 'external_costs_forecast', 'external_costs_var', 'external_costs_ordered')
		));
		$valBudgetSynsExternal = $valBudgetSynsExternal['ProjectBudgetSyns'];
		
		$this->progress_line($project_id);
        
        $employee_info = $this->employee_info;
        $this->set(compact('projectName', 'project_id', 'getDataProjectTasks', 'budgetProviders', 'budgetTypes', 'budgetExternals', 'capexTypes', 'activityLinked', 'totalconsumed', 'profits', 'funders', 'taskExternals', 'modifyBudget', 'employee_info','budget_settings', 'valBudgetSynsExternal'));
    }
    /**
     * update
     *
    ...
    public function update() {
        ...
        if (!empty($this->data)) {
            ...
            $data['capex_id'] = null;
			if( isset($this->data['capex_id']) && $this->data['capex_id'] != ''){
				$data['capex_id'] = @$capex_opex[$this->data['capex_id']];
			}
            $project_id = $this->data['project_id'];
            $activity_id = $this->data['activity_id'];
            $name = $this->data['name'];
			if( !$this->_checkRole(false, $project_id)){
				$this->_functionStop(false, $this->data, __('You have not permission to access this function', true), false, array('action' => 'index', $project_id));
			}
			if( !empty( $this->data['budget_type_id']) && !$this->_isBelongToCompany($this->data['budget_type_id'], 'BudgetType')){
				$this->_functionStop(false, $this->data, sprintf( __('The %s does not exist', true), 'Budget Type'), false, array('action' => 'index', $project_id));
			}
			if (!empty($this->data['id'])) {
                $this->ProjectBudgetExternal->id = $this->data['id'];
    ...
    function delete($id = null, $project_id = null) {
        $this->loadModels('ProjectBudgetExternal');
        if (!$id) {
		...
		$project_id = !empty( $last) ? $last['ProjectBudgetExternal']['project_id'] : 0;
