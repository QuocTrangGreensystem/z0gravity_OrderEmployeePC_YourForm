...
(function() {
    ...
    packagePath = path + '/ext5-packages/' + theme + '/build/';
    ...
    extPrefix = useDebug ? '/ext5.beautifier' : '/ext5';
    ...
    if (hasOverrides) {
		...
		if( Azuree.language == 'fr' ){
			locale = path + '/ext5-packages/ext-locale/ext-locale-fr.js';
		}
        if (repoDevMode && window.ActiveXObject) {
            Ext = {
                _beforereadyhandler: function() {
					...
					if( Azuree.language == 'fr' ){
						Ext.Loader.loadScript({ url: locale});
					}
                }
            };
        } else {
