(function ($) {
    ...
    function Overlays(options) {
        ...
        function moveSelectionOverlay(e, args) {
            ...
            var fillable = column.fillable;
            if( typeof options.beforeShowHandle == 'function' ){
                fillable = fillable && options.beforeShowHandle.call(grid, activeCell);
            }
            selectionOverlay.$handle.toggle(fillable);

            var position = grid.getCellNodeBox(activeCell.row, activeCell.cell);
            if( !position || !fillable ){
                selectionOverlay.toggle(false);
                return;
            }
        ...
        function handleOverlayDrag(e, dd) {
            ...
            dd.range.end = end;
            var endCell = end.cell, endRow = end.row;
            if( !options.horizontal ){
                endCell = dd.range.start.cell;
            }
            if( typeof options.vertical != 'undefined' && !options.vertical){
                endRow = dd.range.start.row;
            }
        ...
        function handleOverlayDragEnd(e, dd) {
            ...
            self.onFillUpDown.notify({ "grid": grid, "range": dragDecorator.getSelectedRange(), origin: dd.origin }, e, self);
