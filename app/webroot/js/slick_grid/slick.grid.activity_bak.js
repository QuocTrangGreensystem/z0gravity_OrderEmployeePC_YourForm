...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function finishInitialization() {
            if (!initialized) {
                ...
                if (jQuery.fn.mousewheel && ( options.frozenColumn > -1 || hasFrozenRows )) {
                    ...
                    $container.on('onmousewheel wheel onmousewheel mousewheel DOMMouseScroll', handleLeftViewportScroll);
                }
                $headerScroller
        ...
        function appendRowHtmlPlugin(stringArrayL, stringArrayR, row, range, dataLength) {
        ...
        function appendCellHtmlPlugin(stringArray, row, cell, colspan, item) {
            var m = columns[cell];
        ...
        function appendRowHtmlCustom(stringArrayL, stringArrayR, row, range, dataLength) {
            var d = getDataItem(row);
            var dataLoading = row < dataLength && !d;
            var rowCss = "slick-row" +
                (dataLoading ? " loading" : "") +
            ...
            for (var i = 0, ii = columns.length; i < ii; i++) {
                ...
                if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
                    ...
                    if (( options.frozenColumn > -1 ) && ( i > options.frozenColumn )) {
                        _getHeight = appendCellHtmlCalculation(stringArrayR, row, i, colspan, d);
                    } else {
                        _getHeight = appendCellHtmlCalculation(stringArrayL, row, i, colspan, d);
                    }
                } else if (( options.frozenColumn > -1 ) && ( i <= options.frozenColumn )) {
                    _getHeight = appendCellHtmlCalculation(stringArrayL, row, i, colspan);
                }
                if(_getHeight > _setHeight){
                    _setHeight = _getHeight;
                }
                if (colspan > 1) {
                    i += (colspan - 1);
                }
            ...
            var _top = getRowTop(row) - frozenRowOffset;
            var beforeRow = row - 1;
            var beforeTop = saveHeightRow[beforeRow] ? saveHeightRow[beforeRow] : 0;
            if(row == 0){
                heightIncrease = 0;
            }
            if(_setHeight != 0 && _setHeight > options.rowHeight){
                heightIncrease += _setHeight - options.rowHeight + 12;
                saveHeightRow[row] = heightIncrease;
            } else {
                saveHeightRow[row] = beforeTop;
            }
            _top += beforeTop;
            var rowHtml = "<div class='ui-widget-content " + rowCss + "' style='top:"
                + _top
            ...
            for (var i = 0, ii = columns.length; i < ii; i++) {
                ...
                if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
                    ...
                    if (( options.frozenColumn > -1 ) && ( i > options.frozenColumn )) {
                        appendCellHtmlCustom(stringArrayR, row, i, colspan, d, _setHeight);
                    } else {
                        appendCellHtmlCustom(stringArrayL, row, i, colspan, d, _setHeight);
                    }
                } else if (( options.frozenColumn > -1 ) && ( i <= options.frozenColumn )) {
                    appendCellHtmlCustom(stringArrayL, row, i, colspan, _setHeight);
                }
        ...
        function appendCellHtmlCalculation(stringArray, row, cell, colspan, item){
            var m = columns[cell];
            var _setHeight = 0;
            if (item) {
                var _height = $(getFormatter(row, m)(row, cell, '', m, item));
                if(_height.filter('.stone-detail').length != 0){
                    _height = _height.filter('.stone-detail').data('height');
                    _setHeight = _height;
                } 
            }
            return _setHeight;
        ...
        function appendCellHtmlCustom(stringArray, row, cell, colspan, item, setHeight) {
            var m = columns[cell];
            var cellCss = "slick-cell l" + cell + " r" + Math.min(columns.length - 1, cell + colspan - 1)
                + (m.cssClass ? " " + m.cssClass : "");
            if (row === activeRow && cell === activeCell) {
            ...
            var _style = (setHeight != 0) ? 'style="height: ' +setHeight+ 'px;"' : '';
            stringArray.push("<div class='" + cellCss + "' " +_style+ ">");
        ...
        function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
            if(options.rowCustom){
                appendRowHtmlCustom(stringArrayL, stringArrayR, row, range, dataLength);
            } else {
                appendRowHtmlPlugin(stringArrayL, stringArrayR, row, range, dataLength);
            }
        }

        function appendCellHtml(stringArray, row, cell, colspan, item) {
            if(options.rowCustom){
                appendCellHtmlCustom(stringArray, row, cell, colspan, item);
            } else {
                appendCellHtmlPlugin(stringArray, row, cell, colspan, item);
            }
        }
       ...
       return false;
        ...
        function handleLeftViewportScroll(event){

            var direction = event.originalEvent.deltaY > 0 ? 1 : -1, delta;
            if( $.browser.mozilla && direction == -1 ){
                delta = 0 - options.rowHeight;
            } else if( $.browser.safari ){
                delta = 0 - event.originalEvent.wheelDeltaY;
            } else {
                delta = direction * options.rowHeight * 4;
            }
            var xTop = $viewportScrollContainerY[0].scrollTop + delta;
            var maxScrollDistanceY = $viewportScrollContainerY[0].scrollHeight - $viewportScrollContainerY[0].clientHeight;

            if( xTop < 0 )xTop = 0;
            else if( xTop > maxScrollDistanceY )xTop = maxScrollDistanceY;

            var vScrollDist = Math.abs(xTop - prevScrollTop);
            if( vScrollDist ){
                if (options.frozenColumn > -1) {
                    $viewportTopR[0].scrollTop = xTop;
                    scrollTop = xTop;
                }
            }
        }
        ...
        function handleContextMenu(e) {
            ...
            trigger(self.onContextMenu, {record: e}, e);
        }
