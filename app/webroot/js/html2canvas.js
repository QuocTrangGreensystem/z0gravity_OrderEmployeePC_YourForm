     ...
     */
            ...
            switch(el.nodeName){
                ...
                case "INPUT":
                    ...
                    if (/^(text|url|email|submit|button|reset|radio|checkbox)$/.test(el.type)){
                ...
                if ( !ignoreElementsRegExp.test( el.nodeName ) ) {
                    ...
                    for (i = 0, childrenLen = elementChildren.length; i < childrenLen; i+=1) {
                        ...
                        if ( node.nodeType === 1 ) {
                            ...
                            stackRender.push([el, node, stack]);
                        }
                    }
        ...
        opts.queue = renderElement(element, null);

        // parse every child element
        for (i = 0, children = element.children, childrenLen = children.length; i < childrenLen; i+=1){
            parseElement(children[i], opts.queue);
        }

        opts.onRending = opts.onRending || function(current,total){

        };
        opts.onFinish = opts.onFinish || function(){};
        var sIndex = 0,sLength = stackRender.length,sTime = 0,mTime = 1000;
        ...
        sTime = getTime();
        var renderElementStack = function(){
            if(sIndex < sLength){
                renderText(stackRender[sIndex][0], stackRender[sIndex][1], stackRender[sIndex][2]);
                opts.onRending(sIndex+1,sLength);
                sIndex++;
                if(getTime() - sTime > mTime){
                    sTime = getTime();
                    setTimeout(renderElementStack,1);
                }else{
                    renderElementStack();
                }
            }else{
        ...
        renderElementStack();
        opts.queue.backgroundColor = getCSS( body, "backgroundColor" );
...
*/
    html2canvas.Renderer = function(parseQueue, opts){
        ...
        function canvasRenderer(zStack){
            ...
            ctx.fillStyle = zStack.backgroundColor || '#ffffff';
