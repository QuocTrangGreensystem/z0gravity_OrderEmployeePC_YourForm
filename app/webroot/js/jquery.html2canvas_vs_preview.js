...
(function( $ ){
    ...
    $(function(){
		...
		var run_laz = 0;
		if(GanttData.length && showType == 0 && $(GanttData[0][0]).hasClass('trFamily')) run_laz = 1; 
		var flyStaffing = function (){
			if( run_laz ){
				if( GanttData){
					$.each(GanttData, function ( i, _row){
						if($(_row[0]).hasClass('trFamily')){
							// console.log(_row);
							$side.append($(_row[0]).addClass('onload'));
							$chart.append($(_row[1]).addClass('onload'));
