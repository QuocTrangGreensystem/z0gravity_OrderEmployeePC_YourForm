...
(function( $ ){
    ...
    $(function(){
        ...
        var setPosition = function(){
            var top = $('.wd-panel').scrollTop();
            var offset = $gantt.parent().offset().top - $('.wd-panel').offset().top;
            var css ={};
            if(enabled && top >= 115){
                css = {
                    position:'absolute',
            ...
            if($chart.offset().top >= ( $('.wd-panel').offset().top+75) ){
                $first.show();
                $last.hide();
            }else{
                $first.hide();
	...
	$(window).resize(function(){
		if(!$('#mcs1_container').length) return;
    ...
    $(window).ready(function(){ 
        var _new_head_tds = $('.gantt-list.gantt-list-primary:first .gantt-head td div');
        var _old_head_tds = $('.gantt-list.gantt-list-primary:last .gantt-head td div' );
        var _index = 0;
        _new_head_tds.each(function(){
             _new_head_tds.eq(_index).width( _old_head_tds.eq(_index).width() ); 
            _index++;
        });
    });
})( jQuery );
