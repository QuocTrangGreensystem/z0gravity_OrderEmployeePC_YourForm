...
(function (a) {
    ...
    a.extend(a.jqx._jqxChart.prototype, {
        defineInstance: function () {
            ...
            this.crosshairsColor = "#e8e8e8";
            this.crosshairsDashStyle = "2,2";
            this.crosshairsLineWidth = 1;
			this.defaultUnit = 'J.H';
			this.isFormatTooltip = false;
        },
        ...
        _render: function (z) {
            ...
            b.y += T.top;
            if (!C) {
                var V = this.categoryAxis.tickMarksColor || "#e8e8e8";
                for (var Q = 0; Q < this.seriesGroups.length; Q++) {
                    var A = this.seriesGroups[Q].orientation == "horizontal";
                    var s = this._getCategoryAxis(Q) == this.categoryAxis ? -1 : Q;
                    var f = {
        ...
        _animateColumns: function (c, f) {
            ...
            for (var e = 0; e < g.length; e++) {
                ...
                if (o == 0) {
					...
					// console.log(n, l, g[e].itemIndex, h);
        ...
        _showToolTip: function (m, k, D, w, c) {
            ...
            if (g) {
                ...
                if (this._toolTipElement.vLine && this._toolTipElement.hLine) {
                    ...
                    var z = this.crosshairsColor || "#e8e8e8";
            ...
            if (!i && this.showToolTips != false) {
                ...
                var o = "<span class='" + C + "'>" + q + this.defaultUnit +"<span>";
				if(this.isFormatTooltip){
					o = q;
				}
                ...
                if (m + e.left + r.width > rect.x + rect.width - 5) {
                    ...
                    s.style.left = (e.left - r.width/2 - 6) + m + "px"
                }
                if (k + e.top + r.height > rect.y + rect.height - 5) {
                    k = rect.y + rect.height - r.height - 7;
                    s.style.top = (e.top - r.height + 5) + k + "px"
                }
                var p = this.host.coord();
                if (E) {
                    a(s).fadeOut(0, 0);
                    s.style.left = (e.left - r.width/2 - 6) + m + p.left + "px";
                    s.style.top = (e.top - r.height + 20) + k + p.top + "px"
                }
                a(s).html(o);
                a(s).clearQueue();
                a(s).fadeTo(400, 1);
                a(s).animate({
                    left: (e.left - r.width/2 - 6) + m + p.left,
                    top: (e.top - r.height + 20) + k + p.top,
                    opacity: 1
                }, 200, "easeInOutCirc")
            }
        },
        ...
        _showLabel: function (u, r, d, b, m, f, c) {
            ...
            var k = g.series[r];
			if( typeof k.customLabelFunction == 'function') return k.customLabelFunction(this, u, r, d, b, m, f, c);
            var p = {
                width: 0,
                height: 0
            };
        ...
        _getColors: function (s, p, d, e) {
			...
			// console.log( c);
            if (c) {
				var x = l.series[p].customGradient || l.customGradient;
				if (x != undefined) {
					h = this.renderer._toLinearOpaqueGradient(q, true, x);
					var hv = l.series[p].noHoverEff || l.noHoverEff;
					if( hv) n = h;
					else n = this.renderer._toLinearOpaqueGradient(f, true, x);
				}else{
    ...
    a.jqx.svgRenderer.prototype = {
        ...
        _gradients: {},
        _toLinearOpaqueGradient: function (e, g, h) {
			// console.log( e,g,h);
			/*
			e: color
			g: is Vertical
			h: custom color matrix,
				percent, lighter, opacity
			*/
            ...
            for (var f in h) {
                ...
                this.attr(j, {
                    offset: h[f][0] + "%",
                    'stop-color': a.jqx._adjustColor(e, h[f][1]),
					'stop-opacity': h[f][2],
                });
                d.appendChild(j)
            }
