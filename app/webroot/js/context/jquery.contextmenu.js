...
;(function($){
	$.contextMenu = {
		...
		show: function(t,e) {
			...
			if( !x && !y && e.target && e.isTrigger){
				var _t = $(e.target);
				x = _t.offset().left + _t.width();
				y = _t.offset().top + _t.height();
			}
			...
			if (cmenu.beforeShow()!==false) {
				...
				cmenu.shown=true;
				$(document).on('click', null, function(e){ if(!((e.target.id == 'open-menu') || ($(e.target).closest('#open-menu').length)) ){cmenu.hide();}}); // Handle a single click to the document to hide the menu
			}
		},
