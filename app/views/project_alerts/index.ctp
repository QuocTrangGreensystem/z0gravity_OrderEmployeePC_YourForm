...
<?php
	...
	$columns = array(
		...
		'number_of_day' => array(
			'id' => 'number_of_day',
			'field' => 'number_of_day',
			'name' => __('Number of day', true),
			'width' => 200,
			'sortable' => true,
			'resizable' => true,
			'editor' => 'Slick.Editors.textBox',
			'validator' => 'DateValidate.isNumeric',
		),
		'display' => array(
			'id' => 'display',
			'field' => 'display',
	...
	$dataView = array();
	$i = 1;
	foreach ($projectAlerts as $projectAlert){
		if ($projectAlert['ProjectAlert']['company_id'] != "") {
			$company_id_save = $projectAlert['ProjectAlert']['company_id'];
		...
		$row_data = array(
			'id' => $projectAlert['ProjectAlert']['id'],
			'no' => $i++,
			'alert_name' => $projectAlert['ProjectAlert']['alert_name'],
			'number_of_day' => $projectAlert['ProjectAlert']['number_of_day'],
			'display' => $projectAlert['ProjectAlert']['display'],			
	...
	$i18n = array(
		...
		'The %s must be number' => __('The %s must be number', true),
		'Number of day' => __('Number of day', true),
	);
?>
<script>
...
function default_item(){
	var _default = {
		id : {defaulValue : 0},
		alert_name : {defaulValue : '', allowEmpty : false},
		number_of_day : {defaulValue : '', allowEmpty : false},
...
(function($){
	$(function(){
		...
		DateValidate.isNumeric = function(value, args){
			var _valid = true;
            var _message = '';
            if(args && args.item){
            ...
            return {
                ...
                message : $.isNumeric(value) ? '' : $this.t('The %s must be number', $this.t('Number of day'))
            };
		}
