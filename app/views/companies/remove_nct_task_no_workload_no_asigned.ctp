<?php 
	...
	$dataView = array();
	foreach( $listProjects as $company_id => $projects){
		foreach( $projects as $project_id => $project_name){
			if( !empty($NCT_tasks_zero[$project_id])){
				$tasks = $NCT_tasks_zero[$project_id];
...
<script type="text/javascript">
	...
	(function($){
		$.extend($this,{
			...
			delete_link: "<?php echo $this->Html->url(array('action' => 'run_delete_task_noworkload_noasigned', '%ID%')); ?>",
		});
		...
		function count_selected(){
			...
			if( _selected.length) {
				if($('#btn-delete-project').is(':hidden')) $('#btn-delete-project').text($this.t('Delete') + ' ' + _selected.length + ' task(s)').fadeIn();
				else{
					$('#btn-delete-project').text($this.t('Delete') + ' ' + _selected.length + ' task(s)');
				}
			}
		...
		function delete_task(task_ids){
			if( typeof task_ids != 'object')
				task_ids = [task_ids];
			$.ajax({
				url : '/recycle_bins/delete_task_ntc_noworkload_noassigned/',
				type: 'POST',
				data: { data: {
					task_ids
				}},
				// async: false,
				beforeSend: function(){
					$.each(task_ids, function(i, task){
						var item = ControlGrid.getData().getItemById(task);
