...
<?php
	...
	$selectMaps = array(
		...
		'default' => array( 'yes' => __('Yes', true), 'no' => __('No', true)),
	);
	$dataView = array();
	$i = 1;
	foreach ($list as $val){
		$row_data = array(
			'id' => $val['ExpectationColor']['id'],
			'no' => $i++,
			'color' => $val['ExpectationColor']['color'],
			'default' => $val['ExpectationColor']['default'] ? 'yes' : 'no',
			'display' => $val['ExpectationColor']['display'] ? 'yes' : 'no',
			'company_id' => $current_company
		);
		$dataView[] = $row_data;
	}
	...
	(function($){
		$(function(){
			...
			$this.fields = {
				...
				default : {defaulValue : '', allowEmpty : false},
				display : {defaulValue : '', allowEmpty : false},
				key : {defaulValue : current_dataset, allowEmpty : false},
			};
