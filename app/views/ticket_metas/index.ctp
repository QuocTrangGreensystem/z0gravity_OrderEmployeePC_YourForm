...
<div id="action-template" style="display: none;">
	<div class="action-menu">
		<a onclick="return confirm('<?php echo h(sprintf(__('Delete?', true))); ?>');" class="action-menu-item" href="<?php echo $this->Html->url(array('action' => 'delete', $name, '%1$s/')); ?>">
...
<?php
	...
	$columns[] = array(
			'id' => 'enable_for_customer',
			'field' => 'enable_for_customer',
			'name' => __('Type_Enable_For_Customer', true),
			'width' => 120,
	...
	$selectMaps = array(
        ...
        'enable_for_customer' => array( 1 => __('Yes', true), 0 => __('No', true)),
        'category' => array( 0 => __('No', true), 1 => __('Support Ticket', true), 2 => __('Formation Ticket', true), 3 => __('Coaching Ticket', true)),
	);
	$dataView = array();
	$i = 1;
	// ob_clean();
	...
	foreach ($data as $val){
		$row_data = array(
			'id' => $val['TicketMeta']['id'],
			'no' => $i++,
			'name' => $val['TicketMeta']['meta_value'],
			'meta_name' => $val['TicketMeta']['meta_name'],
            'enable_for_customer' => $val['TicketMeta']['enable_for_customer'] ? $val['TicketMeta']['enable_for_customer'] : '0',
            'category' => $val['TicketMeta']['category'] ? $val['TicketMeta']['category'] : '0',
			'company_id' => $company_id
		);
		$dataView[] = $row_data;
	}
...
function default_item(){
	var _default = {
		...
		enable_for_customer : {defaulValue : "0", allowEmpty : false},
		category : {defaulValue : "0", allowEmpty : false},
		meta_name : {defaulValue : name, allowEmpty : false},
	};
