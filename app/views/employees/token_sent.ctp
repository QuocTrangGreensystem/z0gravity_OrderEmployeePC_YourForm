<style type="text/css">
.message{position:relative;color:#363636;text-align:center; font-size:  20px; margin-bottom: 110px}.message .close{display:none;}fieldset div.wd-input div.wd-error-message{background-color:#114B80;clear:both;display:block;width:auto;color:red;text-align:left;padding:0;}
.page-login__panel{
    min-height: 25em;
	text-align: center;
}
...
<div class="page-login__panel new">
    ...
    <div class="wd-input">
		...
		<div class="tokent-sent-content">
			<div class="tokent-text-send"><?php echo __('Message sent to your email.', true); ?></div>
			<div class="tokent-text-spam"><?php echo __('Also check your spam.', true); ?></div>
		</div>
		<a href="<?php echo $this->Html->url('/login'); ?>" class="wd-input-link page-login__login-btn btn btn--fancy"><?php __("To login") ?></a>
		 <div class="page-login__copy-right">
			<?php 
				$date = getdate();
				$currentYear = $date['year'];
				$langCode = Configure::read('Config.langCode');
				$url_version =  $html->url(array('controller' => 'versions', 'action' => 'version', '?' => array('hl' => $langCode)));
			?>
			<?php echo '' . sprintf(__('Copyright © 2012-%s. <a href="https://www.z0gravity.com/">Version %s.</a> All rights reserved.', true),$currentYear,  $version) . ''; ?>
		</div>
    </div>
