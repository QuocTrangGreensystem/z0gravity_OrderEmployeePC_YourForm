...
<?php
	...
	foreach ($projectTypes as $val){
		if ($val['ProjectType']['company_id'] != "") {
			$company_id_save = $val['ProjectType']['company_id'];
		...
		$row_data = array(
			'id' => $val['ProjectType']['id'],
			'no' => $i++,
			'project_type' => $val['ProjectType']['project_type'],
			'display' => $val['ProjectType']['display'] ? 1 : '0',
...
function default_item(){
	var _default = {
		id : {defaulValue : 0},
		project_type : {defaulValue : '', allowEmpty : false},
