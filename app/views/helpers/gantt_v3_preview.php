 ...
 *
 */
class GanttV3PreviewHelper extends AppHelper {
	...
	public function setTextDomain($_domain){
		$this->_domain = !empty( $_domain) ? $_domain : 'default';
	}
    ...
    public function create($type, $start, $end, $stones = array(), $displayList = true, $format = 'd-m-Y', $noPhasename = false) {
        ...
        foreach ($stones as $_stones) {
            ...
            if(!empty($valid)){
                $stone .= "<div class=\"gantt-msi gantt-msi-green gantt-ms\" style=\"left:$left%;top:0px\" data-index='" . $_index ."' title=\"" . h($desc) . ' : ' . date($format, $k) . "\"><i></i></div>\n";
            } else {
                $currentDate = strtotime(date('d-m-Y', time()));
                $k = strtotime(date('d-m-Y', $k));
                if($currentDate > $k){
                    $stone .= "<div class=\"gantt-msi gantt-ms\" data-index='" . $_index ."' style=\"left:$left%;top:0px\" title=\"" . h($desc) . ' : ' . date($format, $k) . "\"><i></i></div>\n";
                } elseif($currentDate < $k){
                    $stone .= "<div class=\"gantt-msi gantt-msi-blue gantt-ms\" data-index='" . $_index ."' style=\"left:$left%;top:0px\" title=\"" . h($desc) . ' : ' . date($format, $k) . "\"><i></i></div>\n";
                } else {
                    $stone .= "<div class=\"gantt-msi gantt-msi-orange gantt-ms\" data-index='" . $_index ."' style=\"left:$left%;top:0px\" title=\"" . h($desc) . ' : ' . date($format, $k) . "\"><i></i></div>\n";
                }
            }
            $_index++;
        }
                ...
                if ($_d > $_days) {
                    ...
                    if ($type == 'month') {
                        if($_m == 12 || $i >= $total-$step){
							// chỉ hiển thị số năm khi có ít nhất 2 tháng, do font chữ lớn 
                            $head.="<td colspan=\"$_m_of_y\" class=\"gantt-year\"><div class='abs-title'>" . ($_m_of_y > 2 ? $_y : '') ."</div></td>";
                            $_m_of_y = 1;
                        }else{
                            $_m_of_y++;
                        }
                        $num.= "<td class=\"gantt-d$_days day_of_month\" id=\"" . Inflector::slug("month $_m $_y") . "\"><div>{$month[$_m]}</div></td>";
        ...
        if ($displayList) {
            $list .= "<table class=\"gantt-list gantt-head-content gantt-list-primary " . ($noPhasename ? 'team-plus' : '') . "\">
                            <tr class=\"gantt-head\">
                                        <td><div>" . ( $noPhasename ? '&nbsp;' : __('Phase name', true) ) . "</div><span class='category-view abs-title'></span></td>
        ...
        $chart .= "<table class=\"gantt gantt-primary gantt-chart-content\">
            <tr>
     ...
     * @return void
     */
    public function draw($_id, $_name, $_psor, $_start, $_end, $_rstart, $_rend, $color, $class = 'parent', $comp = null, $assign = null, $viewPhase = true) {
            ...
            $chart.= "<tr class=\"gantt-$class wd-$_id hideGantt\">
                        <td class=\"gantt-node\">
                                <div class=\"gantt-line\">
                                    ...
                                    {$this->_draw($_id, $_name, $_psor, 'n', $start, $total, $_start, $_end, 'd/m', $color, $class == 'parent', $comp, $assign)}";
        } else {
     ...
     */
    protected function _draw($id, $name, $psor, $pos, $time, $total, $start, $end, $format, $color = '#000', $addSpan = false, $comp = null, $assign = null) {
		...
		if($psor){
			...
			$predecessor = $psor_task[1] . ' - ';
		}
        if ($pos == 'n') {
            $out.= "<div class=\"gantt-line-desc gantt-line-$pos gantt-line-tip\" style=\"margin-top: -3px; left:". ($left) ."%\">". h($predecessor) . h($name) . "</div>";
        }
        $_task = explode('-', $id);
        $_isTask = 'block';
        if (!empty($_task)) {
        ...
        return $out . "<div id=\"line-$pos-$id\" class=\"gantt-line-$pos hover-tooltip\" title=\""
                ...
                <em style=\"background-color: $color; position: relative; top: -2px; height: 8px; display: block; width:$comp%;\"></em>"
    ...
    public function parseData(&$data, $strict = false) {
		...
		/*
			...
			ticket 606: Rules if ( workload > 0 and capacity < Total workload)  then RED 
		*/
    ...
    public function drawStaffing2($staffings, $employee_info, $budgetMdTeams, $freezeTeams, $displaySummary = true, $showType = false , $displayTeamPlus, $profile = false, $showAllPicture = false, $isPlus = false) {
        ...
        foreach ($staffings as $key => $staffing) {
			...
			$row_not_affected_class = $is_na ? 'wd_row_not_affected' : '';
			$is_total = ($key === 'summary');
			$row_total_class = $is_total ? 'wd_row_total' : '';
			$rowClass = $row_not_affected_class . $row_total_class;
            if($displayCapacity){
                if($showType == false){
                    $staffGantt = "<tr class=\"gantt-staff $rowClass\">
                        <td class=\"gantt-node gantt-node-head\">
                          <table rel=\"{$staffing['id']}\" class=\"$summary gantt-right\" check=\"{$is_check}\">
                               $titles
                               <tr class=\"gantt-num wd-total-workload\">{$validated[$key]}</tr>";
					if( $is_total ){
						$staffGantt .= "<tr class=\"gantt-num\">{$consumed[$key]}</tr>";
					}elseif( $is_na ){
						$staffGantt .= "<tr class=\"gantt-num\">{$assignEm[$key]}</tr>";
					}else{
						$staffGantt .= "<tr class=\"gantt-num\">{$consumed[$key]}</tr>
                               <tr class=\"gantt-num gantt-capacity\">{$capacity[$key]}</tr>
                               <tr class=\"gantt-num gantt-capacity\">{$absence[$key]}</tr>
                               <tr class=\"gantt-num gantt-total-workload\">{$totalWorkload[$key]}</tr>
                               <tr class=\"gantt-num\">{$assignEm[$key]}</tr>";
					}
					$staffGantt .= "<tr class=\"fixedHeightStaffing gantt-num\"><td class=\"gantt-space\" colspan=\"{$count[1]}\"><div>&nbsp;</div></td></tr>
                          </table>
                        </td>
                    </tr>";
					...
					if( $is_total ){
						...
						$staffGantt .= "<tr class=\"gantt-num\">{$assignPc[$key]}</tr>";
					}else{
						$staffGantt .= "<tr class=\"gantt-num\">{$consumed[$key]}</tr>
                               <tr class=\"gantt-num gantt-capacity\">{$capacity[$key]}</tr>
                               <tr class=\"gantt-num gantt-capacity\">{$absence[$key]}</tr>
					...
					$staffGantt = "<tr class=\"gantt-staff $rowClass\">
						<td class=\"gantt-node gantt-node-head\">
						  <table rel=\"{$staffing['id']}\" class=\"$summary gantt-right\" check=\"{$is_check}\">
							   ...
							   <tr class=\"gantt-num wd-total-remains gantt-total-workload\">{$totalWorkload[$key]}</tr>
				...
				else
				{
					...
					if( !($is_total || $is_na)){
						$staffGantt .= "<tr class=\"gantt-num wd-total-remains\">{$remains[$key]}</tr>";
					}
					$staffGantt .= "<tr class=\"fixedHeightStaffing gantt-num\"><td class=\"gantt-space\" colspan=\"{$count[1]}\"><div>&nbsp;</div></td></tr>
						  </table>
						</td>
			...
			$estimation[$key].= "<td rowspan=\"$rowspan\" class=\"gantt-name\">$_name</td>";
            foreach ($_title as $k => $v) {
				if( $is_na && ($k == 'assignEm' || $k == 'assignPc')) $v = __('% not affected', true);
            ...
            if ($titles) {
                switch ($showType) {
                    case 1: {
                            $titles = "<td class=\"gantt-name\"><div>" . __('Profit centers', true) . "</div><span class='category-view abs-title'></span></td><td class=\"gantt-func\"><div></div></td>";
                            break;
                        }
                    case 3: {
                            ...
                            $titles .="<td class=\"gantt-budget\"><div id=\"gantt-budget-id\">".__d(sprintf($_domain, 'Internal_Cost'), 'Budget M.D', true)."</div></td>";
                            break;
                        }
                    case 2: {
							$module = 'Skills';
                            ...
                            $titles = "<td class=\"gantt-name\"><div>" . __($module, true) . "</div><span class='category-view abs-title'></span></td><td class=\"gantt-func\"><div></div></td>";
                            break;
                        }
                    default: {
                            $titles = "<td class=\"gantt-name\"><div>" . __('Employees', true) . "</div><span class='category-view abs-title'></span></td><td class=\"gantt-func\"><div></div></td>";
                        }
                }
            }
            ...
            if($displayCapacity){
                ...
                if($showType == false){
                    ...
                    $staffList = "<tr class=\"gantt-staff $rowClass\">
                        <td class=\"gantt-node gantt-child\" colspan=\"5\">
                          <table rel=\"list-{$staffing['id']}\" class=\"$summary\">
                               $titles
                               <tr class='wd-estimation'>{$estimation[$key]}</tr>
                               <tr class='wd-workload'>{$validated[$key]}</tr>";
					if( $is_total ){
						$staffList .= "<tr class='wd-consumed'>{$consumed[$key]}</tr>";
					}elseif( $is_na ){
						$staffList .= "<tr class=\"wd-assignEm\">{$assignEm[$key]}</tr>";
					}else{
						$staffList .= "<tr class='wd-consumed'>{$consumed[$key]}</tr>
                               <tr class=\"gantt-capacity\">{$capacity[$key]}</tr>
							   <tr class=\"gantt-capacity wd-absence\">{$absence[$key]}</tr>
                               <tr class=\"gantt-total-workload\">{$totalWorkload[$key]}</tr>
                               <tr class=\"wd-assignEm\">{$assignEm[$key]}</tr>";
					}
					$staffList .= "<tr class=\"fixedHeightStaffing\"><td class=\"gantt-space\" colspan=\"{$count[0]}\"><div>&nbsp;</div></td></tr>
                          </table>
                        </td>
                    </tr>";
                    ...
                    $staffList = "<tr class=\"gantt-staff $rowClass\">
                        <td class=\"gantt-node gantt-child\" colspan=\"5\">
                          <table rel=\"list-{$staffing['id']}\" class=\"$summary gantt-left\">
                               ...
                               <tr class=wd-workload>{$validated[$key]}</tr>";
					if( $is_total ){
						$staffList .= "<tr>{$consumed[$key]}</tr>";
					}elseif( $is_na ){
						$staffList .= "<tr>{$assignPc[$key]}</tr>";
					}else{
						$staffList .= "<tr>{$consumed[$key]}</tr>
                               <tr class=\"gantt-capacity\">{$capacity[$key]}</tr>
				...
				else
				{
					...
					if( !($is_total || $is_na)){
						$staffList .= "<tr class=wd-remains>{$remains[$key]}</tr>";
					}
					$staffList .= "<tr class=\"fixedHeightStaffing\"><td class=\"gantt-space\" colspan=\"{$count[0]}\"><div>&nbsp;</div></td></tr>
						  </table>
						</td>
                ...
                if($showType == 3){
					...
					$staffList .= "<tr class=\"fixedHeightStaffing\"><td class=\"gantt-space\" colspan=\"{$_count}\"><div>&nbsp;</div></td></tr>
						  </table>
						</td>
					</tr>";
                }
