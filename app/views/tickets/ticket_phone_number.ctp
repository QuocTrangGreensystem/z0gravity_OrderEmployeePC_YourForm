<?php
	...
	$dataView = array();
	foreach ($phone_data as $value){
		$class = null;
		if ($value['TicketPhoneNumber']['company_id'] != "") {
			$company_id_save = $value['TicketPhoneNumber']['company_id'];
		...
		$accfecteds = '';
		if(!empty($value['TicketPhoneNumber']['acffected_cus'])){
			$accfecteds = 'customer';
		}
		if(!empty($value['TicketPhoneNumber']['acffected_dep'])){
			$accfecteds = 'developer';
		}
		$row_data = array(
			'id' => $value['TicketPhoneNumber']['id'],
			'no.' => $i++,
			'phone_number' => $value['TicketPhoneNumber']['phone_number'],
			'phone_name' => $value['TicketPhoneNumber']['phone_name'],
			'acffected_to' => $accfecteds,
...
$.extend($this,{
	...
	canModified: true,
	delete_link: "<?php echo $this->Html->url(array('controller' => 'tickets', 'action' => 'delete_phone_number', '%ID%')); ?>",
	onCellChange: function(args){
		return true;
	},
	url: "<?php echo $html->url(array('controller' => 'tickets','action' => 'update_phone_number')); ?>",
});
$this.fields = {
	id : {defaulValue : 0},
	phone_number : {defaulValue : '', allowEmpty : false},
	phone_name : {defaulValue : '', allowEmpty : true},
	acffected_to : {defaulValue : 'customer' , allowEmpty : false},
