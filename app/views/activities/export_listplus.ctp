...
$activeSheet->setTitle('Activity Management');
...
$fieldset = array(
    ...
    array(
        ...
        'path' => 'Activity.budget_customer_id',
        'width' => 30,
    ),
    array(
    ...
    foreach ($fieldset as $_fieldset) {
        ...
        switch ($_fieldset['path']) {
            ...
            case 'project_manager_id' : {
                    $list = array();
                    foreach ($_output as $_out) {
                        $manager = !empty($projectManagers[$_out['project_manager_id']]) ? $projectManagers[$_out['project_manager_id']] : '';
                        $list[] = sprintf('%s%s', $manager, $_out['is_backup'] ? '(B)' : '');
                    }
            ...
            case 'Activity.project' : {
                    $_output = isset($projects[$_output]) ? $projects[$_output] : '';
                    break;
                }
            ...
            case 'AccessibleProfit' : {
                    ...
                    if ($_output) {
                        foreach ($_output as $k => $val) {
                            $profit = !empty($profitCenters[$val['profit_center_id']]) ? $profitCenters[$val['profit_center_id']] : '';
                            $outs .= $profit . ";";
                        }
                    }
                    $_output = $outs;
            ...
            case 'LinkedProfit' : {
                    $outs = "";
                    if ($_output) {
                        $profit = !empty($profitCenters[$_output['profit_center_id']]) ? $profitCenters[$_output['profit_center_id']] : '';
                        $outs = $profit;
                    }
                    $_output = $outs;
                    break;
