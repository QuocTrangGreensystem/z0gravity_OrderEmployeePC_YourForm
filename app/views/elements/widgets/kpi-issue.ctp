<div class="group-content">
    <h3><span><?php echo __d(sprintf($_domain, 'KPI'), 'Issue', true);?></span></h3>
        ...
        <div style="float: left; line-height: -40px; width:30%">
            <div class="wd-input wd-weather-list-dd">
                <ul style="float: left; display: inline;">
                    <li><input class="input_weather" checked="true" <?php echo !(($canModified && !$_isProfile )|| ($_isProfile && $_canWrite)) ? 'disabled' : '' ?> style="width: 25px; margin-top: 8px;" <?php echo @$this->data["ProjectAmr"]["issue_control_weather"] == 'sun' ? 'checked' : 'checked'; ?> value="sun" name="data[ProjectAmr][issue_control_weather][]" type="radio" /> <img title="Sun"  src="<?php echo $html->url('/img/sun.png') ?>"  /></li>
                    <li><input class="input_weather" <?php echo !(($canModified && !$_isProfile )|| ($_isProfile && $_canWrite)) ? 'disabled' : '' ?> type="radio" <?php echo @$this->data["ProjectAmr"]["issue_control_weather"] == 'cloud' ? 'checked' : ''; ?> value="cloud" name="data[ProjectAmr][issue_control_weather][]" style="width: 25px;margin-top: 8px;" /> <img title="Cloud" src="<?php echo $html->url('/img/cloud.png') ?>"  /></li>
                    <li><input class="input_weather" <?php echo !(($canModified && !$_isProfile )|| ($_isProfile && $_canWrite)) ? 'disabled' : '' ?> type="radio" <?php echo @$this->data["ProjectAmr"]["issue_control_weather"] == 'rain' ? 'checked' : ''; ?> value="rain" name="data[ProjectAmr][issue_control_weather][]" style="width: 25px;margin-top: 8px;"   /> <img title="Rain"  src="<?php echo $html->url('/img/rain.png') ?>"  /></li>
                </ul>
        ...
        <ul>
            <?php
                ...
                if(!empty($commentIssues)){
                    foreach($commentIssues as $logSystem){
            ...
            <li id="sale-log-<?php echo $logSystem['id'] ?>" data-log-id="<?php echo $logSystem['id'] ?>">
                ...
                <div class="log-body">
                    ...
                    <textarea class="log-content" rowspan="2" onfocus="autosize(this)" onblur="autosize.destroy(this)" <?php echo !(($canModified && !$_isProfile )|| ($_isProfile && $_canWrite)) ? 'disabled' : '' ?> onchange="updateLog.call(this, 'ProjectIssue')"><?php echo $logSystem['description'] ?></textarea>
                </div>
            </li>
            <?php
                    }
