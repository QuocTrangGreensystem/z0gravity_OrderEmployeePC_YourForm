							...
							<div class="wd-content">
								<h2 class="wd-t3"><?php __('Import projects', true) ?></h2>
									...
									echo $this->Form->create('Import', array('id' => 'uploadForm', 'type' => 'file',
										'url' => array('controller' => 'project_importers', 'action' => 'import_excel')));
									?>
									<div class="wd-input">
...
$columns = array();
$arrayDate = array('Date 1','Date 2','Date 3','Date 4','Date 5','Date 6','Date 7','Date 8','Date 9','Date 10','Date 11','Date 12','Date 13','Date 14','Start Date','End Date');
	...
	$(function(){
		...
		function run_import(){
			...
			if( list_submit.length ){
				...
				$.ajax({
					url : '/project_importers/save_projects',
					type: 'POST',
					dataType: 'json',
					data: {
		...
		$("#matching-field").click(function(){
			...
			$.each(columns, function(index, value){
				...
				// field = $(ControlGrid.getHeaderRowColumn(value['id'])).find('i.selected-item').data('id');
				if(field != 0 && value['id'] != 'no.'){
			...
			$.each(data, function(index, values){
				...
				$.each(list_matching, function(key, field_name){
					if(field_name && typeof field_name !== 'undefined'){
						val = values[key];
						if(field_name.substr(0, 6) == 'price_' && isNaN(val)){
							val = 0;
						}
						if(field_name == 'category'){
							var cate_text = $.trim(val).replace(/ /g,'').toLowerCase();
							// console.log(cate_text);
							if(!(cate_text == 'inprogress' || cate_text == 'model' || cate_text == 'archived')){
								val = 'Opportunity';
							}
						}
						if(field_name.substr(0, 5) == 'bool_' && isNaN(val) && $.inArray(val, [0,1]) == -1){
							val = '';
						}
						if(field_name == 'activated' && isNaN(val) && $.inArray(val, [0,1]) == -1){
							val = '';
						}
						if(field_name.substr(0, 3) == 'yn_'){
							val = (val.toString()).toLowerCase();
							if($.inArray(val, yn_matching) == -1){
								val = '';
							}
						}
