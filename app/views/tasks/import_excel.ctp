							...
							<div class="wd-content">
								<h2 class="wd-t3">
									<div id="delete-task-switch" class="hidden-print">
										...
										echo $this->Form->input('delete_task_exist', array(
											...
											'div' => array(
												...
												'title' => __('Detele existing tasks before the import',true),
												),
											'type' => 'checkbox', 
										));
										?>
									...
									echo $this->Form->create('Import', array('id' => 'uploadForm', 'type' => 'file',
										'url' => array('controller' => 'tasks', 'action' => 'import_excel/')));
									?>
									<div class="wd-input">
	...
	$(function(){
				...
				$.ajax({
					...
					complete: function(){
						if(length == curr && is_success){
							$('#export-submit').show();
							$.each(project_id, function(index,value){
								$.ajax({
									url : '/project_tasks/staffingWhenUpdateTask/' + value,
									type: 'POST',
									dataType: 'json',
								});
							});
		...
		$("#matching-field").click(function(){
			...
			if(list_matching.length > 0){
				...
				$.ajax({
					type: 'POST',
					url: '/tasks/save_setting_matching_fields',
					data: {
			...
			$.each(data, function(index, values){
				...
				if(has_value){
					...
					matching_item['project_id'] = values['project_id'];
					if(can_save){
			...
			if(matching_datas.length > 0){
				...
				var exporter = new Slick.DataExporter('/tasks/export_excel_index', function beforeSubmit(_form){
