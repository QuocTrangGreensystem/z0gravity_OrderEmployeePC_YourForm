...
if(!empty($months)){
    ...
    if(!empty($staffings)){
        $wlSummary = $csSummary = $emSummary = $caSummary = array();
        $wlYearSums = $csYearSums = $emYearSums = $caYearSums = array();
        $wlTotalSums = $csTotalSums = $emTotalSums = $caTotalSums = 0;
        foreach($staffings as $keys => $staffing){
            $bgcolor = 'FBFCCA';
            $workloadYear = $cosumnedYear = $employeeYear = $capacityYear = array();
            if(!empty($staffing['data'])){
                foreach($staffing['data'] as $time => $value){
                    if($minDate <= $time && $time <= $maxDate){
                        ...
                        // employee cua 1 nam
                        if(!isset($employeeYear[$year])){
                            $employeeYear[$year] = 0;
                        }
                        $employeeYear[$year] += !empty($value['employee']) ? $value['employee'] : 0;
                        // capacity cua 1 nam
                        if(!isset($capacityYear[$year])){
                            $capacityYear[$year] = 0;
                        }
                        $capacityYear[$year] += !empty($value['capacity']) ? $value['capacity'] : 0;
                        // workload summary
                        ...
                        // employee summary
                        if(!isset($emSummary[$time])){
                            $emSummary[$time] = 0;
                        }
                        $emSummary[$time] += !empty($value['employee']) ? $value['employee'] : 0;
                        // capacity summary
                        if(!isset($caSummary[$time])){
                            $caSummary[$time] = 0;
                        }
                        $caSummary[$time] += !empty($value['capacity']) ? $value['capacity'] : 0;   
                    }
                }
            } 
            $cols = $_colDetail;
            foreach($months as $month){
                ...
                $employee = !empty($staffing['data'][$_day]['employee']) ? $staffing['data'][$_day]['employee'] : 0;
                $capacity = !empty($staffing['data'][$_day]['capacity']) ? $staffing['data'][$_day]['capacity'] : 0;
                $_wlSums = !empty($wlSummary[$_day]) ? $wlSummary[$_day] : 0;
                $_csSums = !empty($csSummary[$_day]) ? $csSummary[$_day] : 0;
                $_emSums = !empty($emSummary[$_day]) ? $emSummary[$_day] : 0;
                $_caSums = !empty($caSummary[$_day]) ? $caSummary[$_day] : 0;
                ...
                if($keys === 'summary'){
                    ...
                    $PhpExcel->value($_colNameDetail . ($_row+2), $_emSums);
                    $PhpExcel->value($_colNameDetail . ($_row+3), $_caSums);
                } else {
                    $PhpExcel->value($_colNameDetail . ($_row), $workload);
                    $PhpExcel->value($_colNameDetail . ($_row+1), $consumed);
                    $PhpExcel->value($_colNameDetail . ($_row+2), $employee);
                    $PhpExcel->value($_colNameDetail . ($_row+3), $capacity);
                }
                
            }
            $colYear = 2;
            $colTotal = count($_yearList) + $colYear;
            $wlTotal = $csTotal = $emTotal = $caTotal = 0;
            foreach($_yearList as $_years){
                ...
                $csTotalSums += $csYear;
                // tinh tong employee theo tung nam theo tung project
                $emYear = !empty($employeeYear[$_years]) ? $employeeYear[$_years] : 0;
                // tinh summary employee theo tung nam cua tat ca project
                if(!isset($emYearSums[$_years])){
                    $emYearSums[$_years] = 0;
                }
                $emYearSums[$_years] += $emYear;
                // tinh tong employee cua 1 project
                $emTotal += $emYear;
                // tinh tong employee cua tat ca project
                $emTotalSums += $emYear;
                // tinh tong capacity theo tung nam theo tung project
                $caYear = !empty($capacityYear[$_years]) ? $capacityYear[$_years] : 0;
                // tinh summary capacity theo tung nam cua tat ca project
                if(!isset($caYearSums[$_years])){
                    $caYearSums[$_years] = 0;
                }
                $caYearSums[$_years] += $caYear;
                // tinh tong capacity cua 1 project
                $caTotal += $caYear;
                // tinh tong capacity cua tat ca project
                $caTotalSums += $caYear;
                ...
                if($keys === 'summary'){
                    ...
                    $PhpExcel->value($_colNameYear . ($_row + 2), $emYearSums[$_years]);
                    $PhpExcel->value($_colNameYear . ($_row + 3), $caYearSums[$_years]);
                } else {
                    $PhpExcel->value($_colNameYear . ($_row), $wlYear);
                    $PhpExcel->value($_colNameYear . ($_row + 1), $csYear);
                    $PhpExcel->value($_colNameYear . ($_row + 2), $emYear);
                    $PhpExcel->value($_colNameYear . ($_row + 3), $caYear);
                }
            ...
            if($keys === 'summary'){
                ...
                $PhpExcel->value($_colNameTotal . ($_row + 2), $emTotalSums);
                $PhpExcel->value($_colNameTotal . ($_row + 3), $caTotalSums);
            } else {
                $PhpExcel->value($_colNameTotal . ($_row), $wlTotal);
                $PhpExcel->value($_colNameTotal . ($_row + 1), $csTotal);
                $PhpExcel->value($_colNameTotal . ($_row + 2), $emTotal);
                $PhpExcel->value($_colNameTotal . ($_row + 3), $caTotal);
            }
            ...
            $activeSheet->mergeCells('A' . ($_row) . ':A' . ($_row + 3));
            $_row += 5;
        }
    }
}
