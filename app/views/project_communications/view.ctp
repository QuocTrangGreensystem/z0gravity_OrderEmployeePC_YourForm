...
// echo $html->script(array(
	...
	// 'jscolor',
// ));
if( empty($article['ProjectCommunication']['custom_color'])) $article['ProjectCommunication']['custom_color'] = '217FC2';
?>
...
for ($m=1; $m<=12; $m++) {
	...
	$link = $this->Html->url(array(
		...
		'action' => 'view',
		$article['ProjectCommunication']['public_key']
	));
	$link = urlencode($link);
$socials = array(
	'mail' => array(
...
<style id="article-custom-color">
	.article-container .custom-color{
		color: #<?php echo $article['ProjectCommunication']['custom_color'];?>;
	}
	.article-container .custom-background-color{
		background-color: #<?php echo $article['ProjectCommunication']['custom_color'];?>;
	}
...
<div class="wd-article-container"> 
	<div class="wd-blog-info block custom-background-color">
		<div class="text-left">
			...
			<span class="public-info">
				<?php 
					if( !empty($article['ProjectCommunication'])){
					$public_date = strtotime( $article['ProjectCommunication']['public_date'] );
	...
	<div class="wd-article-title block">
		<h2><?php echo $article['ProjectCommunication']['communication_title'];?></h2>
	</div>
	<div class="article_attachment block40">
		<?php 
		$url = $this->Html->url(array(
			'controller' => $this->params['controller'],
			'action' => 'attachment',
			$project_id,
			$article['ProjectCommunication']['id']
			));
		?>
		<img src="<?php echo $url;?>" title="<?php echo $article['ProjectCommunication']['image'];?>">
	</div> 
	<div class="article-content-info block">
		<div class="wd-text-block wd-date start-date">
			...
			<span class="wd-text-block-content">
				<?php echo $article['ProjectCommunication']['start_date'];?>
			</span>						
		</div>
		<div class="wd-text-block wd-date end-date">
			...
			<span class="wd-text-block-content">
				<?php echo $article['ProjectCommunication']['end_date'];?>
			</span>						
		</div>
		<div class="wd-text-block status-text-group">
			...
			<span class="wd-text-block-content">
				<span class="status status<?php echo $article['ProjectCommunication']['status'];?>"><?php echo $article['ProjectCommunication']['status'] ? __('In Progress', true) : __('Closed', true);?> </span>
				<span class="status-text"> <?php echo $article['ProjectCommunication']['status_text'];?> </span>
			</span>
		</div>
	</div>
	<div class="article-content block">
		<?php echo $article['ProjectCommunication']['content'];?>
	</div>
	<div class="wd-article-sub-title block">
		<h3><?php echo $article['ProjectCommunication']['sub_title'];?></h3>
	</div>
	<?php if( !empty($article['ProjectCommunicationUrl'])){ ?>
		<div class="article-ex-url block">
			
			<ul class="article-urls" id="article-urls">
			<?php foreach($article['ProjectCommunicationUrl'] as $k=>$url ){?>
