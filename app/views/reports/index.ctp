...
<!-- dialog_add_html -->
<div id="dialog_add_html" style="display:none" title="<?php __('Add Email') ?>" class="buttons">
    <?php
    ...
    <div class="wd-input">
        <center>
            ...
            echo $this->Form->input('data', array('type' => 'text',
                ...
                'value' => !empty($reports) && !empty($reports['Report']['data']) ? $reports['Report']['data'] : ''
                )); ?>
        </center>
    </div>
            ...
            <div class="wd-list-project">
                <div class="wd-title" style="margin-bottom: 3px !important; padding-top: 5px; margin-top: -12px;">
                   ...
                   if($isAdmin):
                        if(!empty($idHtml)):
                   ?>
                        <a onclick="return confirm('<?php echo __('Delete?', true); ?>');" href="<?php echo $this->Html->url(array('action' => 'delete', $company_id, $idHtml));?>" class="ch-button-delete" style="margin-left: 6px;" ><span><?php __('Delete Email') ?></span></a>
                   <?php endif;?>
                   <a href="javascript:void(0);" id="add_html" class="btn-text">
                        <img src="<?php echo $this->Html->url('/img/ui/blank-plus.png') ?>" />
                        <span><?php __('Add Email') ?></span>
                    </a>
                   <?php endif;?>
                </div>
                ...
                <div class="wd-table" id="project_container" style="width:100%;height:100%; border: 1px solid rgb(228, 228, 228)">
                    <?php
                        if(!empty($reports) && !empty($reports['Report']['data'])){
							$url = $this->Tableau->get_trusted_url($reports['Report']['data'],'tableauingenia.ovh','views/TableaudebordCEA-Azure/TableaudebordCEA');
							if (empty($url)) {
								echo '<p style="width:400px;height:400px;font-size: 16px; color: red; padding: 6px;">Report not exist!</p>';
							} else {
					?>
							<iframe src="<?php echo $url;?>" width="100%" height="563"></iframe>
					<?php
							}
                        } else {
                            echo '<p style="width:400px;height:400px;font-size: 16px; color: red; padding: 6px;">Report not exist!</p>';
