...
<?php
	...
	$selectMaps = array(
		'company_id' => $company_prioritys,
		'display' => array( 'yes' => __('Yes', true), 'no' => __('No', true)),
	);
	$dataView = array();
	$i = 1;
	// ob_clean();
	// debug($company_prioritys);
	// debug($projectPriorities);
	// exit;
	foreach ($projectPriorities as $projectPrioritie){
		if ($projectPrioritie['ProjectPriority']['company_id'] != "") {
			$company_id_save = $projectPrioritie['ProjectPriority']['company_id'];
			$company_name = $company_prioritys[$company_id_save];
			if ($parent_companies[$company_id_save] != "") {
				$company_name = $company_prioritys[$parent_companies[$company_id_save]] . " --> " . $company_name;
			}
		}
		else { $company_name = " ";}
		$row_data = array(
			'id' => $projectPrioritie['ProjectPriority']['id'],
			'no' => $i++,
			'priority' => $projectPrioritie['ProjectPriority']['priority'],		
...
(function($){
	$(function(){
        ...
        $this.fields = {
            id : {defaulValue : 0},
            priority : {defaulValue : '', allowEmpty : false},
