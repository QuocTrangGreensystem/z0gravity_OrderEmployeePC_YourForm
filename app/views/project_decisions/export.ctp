...
$activeSheet->setTitle('Project Decision List');
...
$fieldset = array(
    array(
        ...
        'path' => 'ProjectDecision.project_decision',
        'width' => 30,
    ),
    array(
        'name' => __('Decision Explanation', true),
        'path' => 'ProjectDecision.project_decision_explanation',
        'width' => 30,
    ),
    array(
    ...
    array(
        ...
        'path' => 'ProjectDecision.project_decision_date',
        'width' => 30,
    )
);
...
foreach ($projectDecisions as $projectDecision) {
    ...
    foreach ($fieldset as $_fieldset) {
        $colName = PHPExcel_Cell::stringFromColumnIndex($cIndex++);
        $_output = Set::classicExtract($projectDecision, $_fieldset['path']);
        switch ($_fieldset['path']) {
            case 'ProjectDecision.project_decision_date' : {
...
header('Content-Disposition: attachment;filename="projects_decision_' . date('H_i_s_d_m_Y') . '.xls"');
