...
<?php
	...
	$dataView = array();
	$i = 1;
	foreach ($projectAmrStatuses as $projectAmrStatus){
		if ($projectAmrStatus['ProjectAmrStatus']['company_id'] != "") {
			$company_id_save = $projectAmrStatus['ProjectAmrStatus']['company_id'];
		...
		$row_data = array(
			'id' => $projectAmrStatus['ProjectAmrStatus']['id'],
			'no' => $i++,
			'amr_status' => $projectAmrStatus['ProjectAmrStatus']['amr_status'],
...
<script>
	...
	(function($){
		$(function(){
			...
			$this.fields = {
				id : {defaulValue : 0},
				amr_status : {defaulValue : '', allowEmpty : false},
