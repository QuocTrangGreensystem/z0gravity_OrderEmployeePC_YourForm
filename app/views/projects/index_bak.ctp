...
<?php
    ...
    echo $html->script(array('jquery-ui.min', 'jquery.easing.1.3', 'jquery.mCustomScrollbarClone'));
endif;
?>
...
$columnAlignRightAndManDay = array(
    'ProjectAmr.md_validated', 'ProjectAmr.md_engaged', 'ProjectAmr.md_forecasted','ProjectAmr.md_variance',
...
//    $text = strlen(str_replace(' ', '', $project['Project']['project_name']))*8;
//    $textArmProject = strlen(str_replace(' ', '', $project['ProjectAmrProgram']['amr_program']))*8;
//    if($length < $text){ $length = $text; }
//    if($lengArmProject < $textArmProject){ $lengArmProject = $textArmProject; }
//}
...
foreach ($selects as &$select) {
    asort($select);
    $select = $this->Form->select('test', $select, null, array('empty' => __('-- Any -- ', true), 'secure' => false));
}
    ...
    <div class="wd-layout">
        <div class="wd-main-content">
            <div class="wd-list-project">
                <div class="wd-title" style="position: relative;">
                    <div class="check-box-stones">
                        <?php
                    ...
                    <a href="javascript:void(0);" id="add_portfolio" class="wd-add-project" style="margin-right:5px; "><span><?php __('Portfolio') ?></span></a>
                    <a href="javascript:void(0);" id="add_vision_portfolio" class="wd-add-project" style="margin-right:5px; "><span><?php __('Vision portfolio') ?></span></a>
                    <a href="javascript:void(0);" id="add_vision_staffing_news" class="wd-add-project" style="margin-right:5px; "><span><?php __('Vision staffing+') ?></span></a>
                    ...
                    ?>
                        <select style="margin-right:11px; width:18.8%% !important; padding: 6px;" class="wd-customs" id="CategoryStatus">
                            <option value="0"><?php echo  __("--Select--", true);?></option>
                        </select>
                        <select style="margin-right:11px; width:8.8%% !important; padding: 6px;" class="wd-customs" id="CategoryCategory">
                            <!--<option value="0" <?php //echo isset($sx) ? $sx : '';?>><?php //echo  __("--Select--", true)?></option>-->
                ...
                ?>
                <div class="wd-table" id="project_container" style="width: <?php echo ($viewGantt == true) ? '50%' : '100%'?>; float: left" rels="<?php echo ($viewGantt && !empty($confirmGantt) && $confirmGantt['stones']) ? 'yes' : 'no';?>">
...
<script type="text/javascript">
    ...
    (function($){
        ...
        $('#CategoryCategory').change(function(){
            ...
            //     if($(this).is(':selected')){
            //         id = $('#CategoryCategory').val();
            //         cate_id = id;
            //     }
            // });
            // listProjectStautus(id, 0);
        });
        ...
        heightViewPort = (heightViewPort < 400) ? 380 : heightViewPort;
        $('.twoTable').css({
            height: heightViewPort
            });
        $(window).resize(function(){
            var _heightViewPort = $(window).height() - 390;
            _heightViewPort = (_heightViewPort < 400) ? 380 : _heightViewPort;
            $('.twoTable').css({
                height: _heightViewPort
            });
        });
        //$('.gantt tr:first').css('position', 'absolute');
        //$('.gantt tr:first td').css('width', $('.gantt tr:first').width() * 10000);
        //position: absolute;
    })(jQuery);
    //center current date
    ...
    function showPhaseDetail(project)
	{
        ...
        if(stoneDetail.hasClass('stone-detail-hide')){
            ...
            if(stoneDetail.html() != ''){
                ...
                $('.slick-viewport .grid-canvas .slick-row').children('div').find('a').each(function(){
                    ...
                    _listTopRow[$row.attr('id')] = parseFloat($row.css('top'));
                    checkRow++;
                });
