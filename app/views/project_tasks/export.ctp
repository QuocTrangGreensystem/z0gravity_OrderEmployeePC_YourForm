...
$activeSheet->setTitle('Project Task List');
...
$fieldset = array(
    array(
        'name' => __('Phase', true),
        'path' => 'ProjectTask.project_planed_phase_id',
        'width' => 30,
    ),
    array(
        'name' => __('Task', true),
        'path' => 'ProjectTask.task_title',
        'width' => 30,
    ),
    array(
        'name' => __('Priority', true),
        'path' => 'ProjectPriority.priority',
        'width' => 30,
    ),
    array(
        'name' => __('Status', true),
    ...
    array(
        ...
        'path' => 'ProjectTask.task_completed',
        'width' => 15,
    ),
    array(
    ...
    array(
        ...
        'path' => 'ProjectTask.task_start_date',
        'width' => 20,
    ),
    array(
        'name' => __('End date', true),
        'path' => 'ProjectTask.task_end_date',
        'width' => 20,
    ),
    array(
        'name' => __('Real End date', true),
        'path' => 'ProjectTask.task_real_end_date',
        'width' => 20,
    )
);
...
foreach ($projectTasks as $projectTask) {
    ...
    foreach ($fieldset as $_fieldset) {
        $colName = PHPExcel_Cell::stringFromColumnIndex($cIndex++);
        $_output = Set::classicExtract($projectTask, $_fieldset['path']);
        switch ($_fieldset['path']) {
            case 'ProjectTask.task_start_date' :
            case 'ProjectTask.task_end_date' :
            case 'ProjectTask.task_real_end_date' : {
                    $_output = (empty($_output) || $_output == '0000-00-00') ? '' : $str_utility->convertToVNDate($_output);
                    break;
                }
                case 'ProjectTask.project_planed_phase_id' : {
                    if(!empty($projectPhases[$_output])){
                       $_output = $projectPhases[$_output];
                    }
                    break;
                }
            case 'Employee' : {
                ...
                case 'ProjectTask.task_completed':{
                       $_output .= ' %';
                }
        }
...
header('Content-Disposition: attachment;filename="projects_task_' . date('H_i_s_d_m_Y') . '.xls"');
