...
#wd-container-main .wd-table .slick-viewport .slick-cell{
	height: 38px;
}
.wd-table .slick-row .slick-cell .ui-combobox{
	height: 38px;
}
.slick-cell .btn.btn-plus-green{
	width: 38px;
	height: 38px;
	line-height: 38px;
}
#wd-container-main .wd-table .slick-row .slick-cell{
	border-right: 0;
	border-left: 0;
...
.wd-table .grid-canvas .editor-text{
	width: calc( 100% - 20px);
	padding-left: 20px;
}
.wd-table-container .wd-table{
	min-height: inherit;
}

.wd-table .download-attachment{
    background: url(/img/new-icon/download.png) center center no-repeat;
    padding: 0;
    margin: 0;
}
.wd-table .delete-attachment {
    background: url(/img/new-icon/delete.png) center center no-repeat;
    padding: 0;
    margin: 0;
    position: relative;
    top: 1px;
}
.wd-table #action-attach-url{
    background: url(/img/new-icon/folder.png) center center no-repeat;
    float: none;
    width: 20px;
    background-position: unset;
}
.wd-table .url-attachment{
    background: url(/img/new-icon/link.png) center center no-repeat;
    padding: 0;
    margin: 0;
    position: relative;
    top: 5px;
...
.wd-table #action-attach-url:hover{
    background-position: unset;
}

.wd-table .slick-cell.editable{
    text-align: center;
}
</style>
<?php
...
$menu = $this->requestAction('/menus/getMenu/project_budget_sales/index');
...
<div id="dialog_attachement_or_url" class="buttons" style="display: none;">
    <div class="wd-popup-container">
		<div class="wd-popup">
			<?php 
			echo $this->Form->create('popupInvoiceUpload', array(
				// 'id' => 'Upload'
				'type' => 'POST',
				'url' => array('controller' => $this->params['controller'], 'action' => 'upload', $projectName['Project']['id'])));
				?>
			<div id="popup_template_attach" >
				...
				<div class="trigger-upload">
					<div id="upload-popup" method="post" action="/project_livrables_preview/add_new_document/<?php echo $projectName['Project']['id']; ?>" class="dropzonex" value="" >
			...
			echo $this->Form->input('url', array(
				...
				'label' => __('URL link',true),
				'type' => 'text',
				'id' => 'url', 
				'placeholder' => __('https://', true) ,  
				));
			echo '<div class="button-group">';
			echo $this->Form->button( __('btnCancel', true), array('type' => 'button', 'class' => 'btn-form-action btn-cancel', 'id'=>'cancelBtn', 'onclick' => 'closeForm()' ));
...
<!-- popup Invoice Upload -->
<div id="InvoiceUpload" class="popup-fullscreen-container" style="display: none;">
	<div class="popup-fullscreen-container-inner" >
		<div class="wd-popup-container">
			<div class="popup-title">
				<h3 class="title"> <?php __('Attachement or URL'); ?></h3>
				<a href="javascript:closeFullscreenPopup();" class="popup-close-btn"></a>
			</div>
			<div class="wd-popup-content">
				...
				<div id="popup_template_attach" >
					...
					<div class="trigger-upload">
						<div id="upload-popup" method="post" action="<?php echo $this->Html->url(array('controller' => $this->params['controller'], 'action' => 'upload', $projectName['Project']['id']));?>" class="dropzone" value="" >
						</div>
					</div>
				</div>
				<?php 
				...
				echo $this->Form->input('id_invoice', array(
					'type' => 'hidden',
					'id' => 'uploadIdInvoice', 
				));
				?>
				...
				echo '<div class="button-group">';
				echo $this->Form->button( __('btnCancel', true), array('type' => 'button', 'class' => 'btn-form-action btn-cancel', 'id'=>'cancelBtn', 'onclick' => 'closeFullscreenPopup()' ));
				echo $this->Form->button(__('btnSave', true), array('type' => 'submit', 'class' =>'btn-form-action btn-ok btn-right' ));
				echo '</div>';
				echo $this->Form->end();
				?>
...
<!-- popupInvoiceUpload end -->
<!-- export excel  -->
<fieldset style="display: none;">
    <?php
    echo $this->Form->create('Export', array(
        'type' => 'POST',
        'url' => array('controller' => 'project_budget_sales', 'action' => 'export', $projectName['Project']['id'])));
...
$columns = array(
    ...
    array(
        'id' => 'man_day',
        'field' => 'man_day',
        'name' => __d(sprintf($_domain, 'Sales'), $md, true),
        'width' => 150,
        'sortable' => false,
        'resizable' => true,
        'noFilter' => 1,
        ...
        <div class="wd-main-content">
            ...
            <div class="wd-list-project">
				...
				<div class="wd-table-container" style="width:100%;">
                    <?php if($canModified){ ?>
                        <a href="javascript:void(0);" class="btn add-field" id="add-new-sales" style="margin-right:5px;" onclick="addNewSalesButton();" title="<?php __('Add an item') ?>"></a>
...
<div id="attachment-template" style="display: none;">
    <div style="overflow: hidden; display: inline-block;" class="img-to-right">
        ...
        $(function(){
            ...
            $.extend(Slick.Editors,{
                Attachement : function(args){
					...
					$('#action-attach-url').on('click', function(){
						$('#uploadID').val(args.item.id);
						$('#uploadIdInvoice').val( args.item.id_invoice ? args.item.id_invoice : '');
						$('#InvoiceUpload').show();
						console.log(args);
            ...
            $this.onCellChange = function(args){
				...
				$('.row-parent').closest('.slick-row').addClass('row-parent-custom');
            ...
            */
			...
			// $('body').on('click', function(e){
				// if( !$('#InvoiceUpload').find(e.target).length ) $('#InvoiceUpload').fadeOut();
			// });
...
<script>
    ...
    function setupScroll(){
        ...
        $("#scrollTopAbsence").css('margin-left', $(".slick-viewport-right:first").offset().left - $('#project_container').offset().left );
    }
    setTimeout(function(){
        setupScroll();
    }, 2500);
...
<script>
     ...
     */ 
	var popupDropzone;
	var _form = $('#popupInvoiceUploadIndexForm');
	...
	popupDropzone.on("queuecomplete", function(file) {
		...
		location.reload();
		// tham khảo update lại table trong milestone preview
		_form.closest('.popup-fullscreen-container').removeClass('loading');
	});
	
	_form.on('submit', function(e){
		_form.closest('.popup-fullscreen-container').addClass('loading');
		// _form.find('#dataUploadID').val($('#newDocName').val());
	...
	function closeFullscreenPopup(){
		$('.popup-fullscreen-container').fadeOut();
	}	
</script>
<style type="text/css">
#wd-header-custom{height:30px;border:1px solid #E0E0E0;border-bottom:none;border-right:none!important;}
.slick-headerrow-columns div{margin-right:-7px;}.slick-headerrow-columns div p{padding-top:5px;font-weight:700;text-align: center;}#project_container{overflow:visible!important;}
.slick-headerrow-columns div{background-color:#b5d2e2;}
.gs-custom-cell-md-header{background:#75923C!important;}.gs-custom-cell-md{background-color:#C2D69A!important;}.cl-average-daily-rate{margin-left:-12px;width:135px;padding:7px;}.color-rate-loading{color:#ccc;}.color-rate-success{color:#3BBD43;}.color-rate-error{color:#F71230;}/* .slick-row.odd{background:#FFF!important;} */ .wd-table .slick-row.row-parent-custom{background-color:#EAF1FA !important;} .row-disabled-custom{background-color:#FAFAFA!important;}.row-number-custom{text-align:right;}.row-current-edit{border-top:1px solid #004482!important;border-bottom:1px solid #004482!important;box-shadow:0 0 3px #004482;}
</style>
