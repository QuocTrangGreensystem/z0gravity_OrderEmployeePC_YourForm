                        ...
                        <div class="wd-section" id="wd-fragment-1">
                            <?php
                                ...
                                echo $this->Form->create('Employee', array(
                                    'type' => 'POST',
                                    'url' => array('controller' => 'employees_preview', 'action' => 'my_profile', $employee_id)));
                                ...
                                <div class="wd-content">
									<div class="wd-content-container">
										<div class="wd-top-content">
											<div id="ch_avatar">
												<?php
												//Quan update ticket #431. Thay doi dieu kien, cach hien thi avatar. 22/08/2019
												if( !empty( $employee_info['Employee']['avatar_resize']) ) {
													//linkAvatar link hinh anh.
													$linkAvatar = $this->UserFile->avatar($employee_id, 'avatar');
													...
													$text_avatar_non = '<p class="circle-name" style="background-color: ' . (!empty( $employee_info['Employee']['avatar_color']) ? $employee_info['Employee']['avatar_color'] : '#72ADD2') .'">' . $avt_name . '</p>';
													//
												}else{
													$avt_name = substr( trim($employee_info['Employee']['first_name']),  0, 1) .''.substr( trim($employee_info['Employee']['last_name']), 0, 1);
													$text_avatar_non = '<p class="circle-name" style="background-color: ' . (!empty( $employee_info['Employee']['avatar_color']) ? $employee_info['Employee']['avatar_color'] : '#72ADD2') .'">' . $avt_name . '</p>';
												...
												<?php 
													if( !empty( $employee_info['Employee']['avatar_resize']) ){
														?><div class="avatar_show"><?php
															echo $text_avatar;
														?></div>
														<div class="avatar_non hidden"><?php
															echo $text_avatar_non;
														?></div><?php
													} else{
														echo $text_avatar_non;
													}
														...
														<div class="wd-color-avat">
															...
															<ul>
																<?php function is_avatar_color_checked($employee_info, $color){
																	...
																	if( ($employee_info['Employee']['avatar_color'] == $color) && (empty($employee_info['Employee']['avatar_resize'])) ){
																		return "selected";
																	}
																	return '';
																} ?>
					...
					<div class="wd-row">
						<div class="wd-col wd-col-md-6">
							...
							echo $this->Form->input('Role', array(
								...
								'type' => 'text',
								'disabled' => true,
								'value' => !empty($profileName) ? $profileName :  __($role_id,true),
			...
			<div class=" wd-col wd-col-md-6 wd-my-content-right">
				...
				<div class="div-row">
					...
					echo $this->Form->input('activate_copy', array(
						'label' => __("Fill the timesheet automatically",true),
						'between' => '<span class="checkmark"></span>',
						'type' => 'checkbox',
						'div' => array(
				...
				<!-- ***Quan update Not display 2 field Validation when login by Consultant*** - 14/01/2019 -->
					<?php
...
<?php
	...
	$('.wd-color-avat ul li a').on('click', function(){
		...
		$('.avatar_show').addClass('hidden');
		$('.avatar_non').removeClass('hidden');
