<?php
    ...
    if(!empty($statusClosedWon) && $statusClosedWon == 'true'){
        $disabled = 'disabled';
        $disabledStatus = 'disabled';
    } else{
        if($modifySaleLead == 'false'){
            $disabled = 'disabled';
            $disabledStatus = 'disabled';
        }
    }
    if($roles === 'admin' || (!empty($saleRoles) && $saleRoles == 1)){
        $disabledStatus = '';
    }
?>
<style>
            ...
            <div class="wd-tab">
                <div class="wd-panel">
                    <div class="wd-section" id="wd-fragment-1">
                        <h2 class="wd-t2"><?php __("Lead Details") ?></h2>
                        <?php echo $this->Session->flash(); ?>
                        <?php
                        echo $this->Form->create('SaleLead', array(
                            'enctype' => 'multipart/form-data',
                            'url' => array('controller' => 'sale_leads', 'action' => 'update', $company_id, $id)
                        ));
                        ...
                        <fieldset>
                            <div class="wd-scroll-form" style="height:auto;">
                                <div class="wd-left-content">
                                    ...
                                    <div class="wd-input">
                                        <label for="name" style="color: #F00808;"><?php __("Lead Name");?></label>
                                        <?php
                                    ...
                                    <div class="wd-input" style="position: relative;">
                                        <label for="goal"><?php __("Goal");?></label>
                                        <?php
                                        echo $this->Form->input('goal', array('div' => false, 'label' => false,
                                            'type' => 'text',
                                            'disabled' => $disabled,
                                            "style" => "padding: 6px 2px; width: 62%;"));
                                        ?>
                                ...
                                <div class="wd-right-content">
                                    ...
                                    <div class="wd-input">
                                        <label for="sale_setting_lead_maturite"><?php __("Maturity") ?></label>
                                        <?php
                                            echo $this->Form->input('sale_setting_lead_maturite', array('div' => false, 'label' => false,
                                                'disabled' => $disabled,
                                                "options" => !empty($maturites) ? $maturites : array(), 'empty' => __("--Select--", true))); ?>
                                    </div>
                                    <div class="wd-input">
                                        <label for="sale_setting_lead_phase"><?php __("Phase") ?></label>
                                        <?php
                                            echo $this->Form->input('sale_setting_lead_phase', array('div' => false, 'label' => false,
                                                'disabled' => $disabled,
                                                "options" => !empty($saleSettings[6]) ? $saleSettings[6] : array(), 'empty' => __("--Select--", true))); ?>
                                    </div>
                                    <div class="wd-input">
                        ...
                        <hr style="clear: both;" />
                        <div class="wd-title">
                            <?php if($roles === 'admin' || (!empty($saleRoles) && ($saleRoles == 1))):?>
...
<script>
    ...
    var statusClosedWon = <?php echo json_encode(($disabled == 'disabled') ? 'true' : 'false');?>;
    (function($){
        $(function(){
            /**
             * Khai bao bien
            ...
            $('html').click(function(e){
                ...
                ($(e.target).attr('class').split(' ')[0] && ($(e.target).attr('class').split(' ')[0] == 'salesman')) ||
             ...
             * DatePicker
             */
            $("#SaleLeadDealStartDate, #SaleLeadDealEndDate, #textStartOfBilling, #txtAchievementStartDate, #txtAchievementEndDate, #txtDateGoLive").datepicker({
                //showOn          : 'button',
                        ...
                        if(statusClosedWon == 'true'){
          		        ...
          		        if(saleLeadFileOfEstimates && Object.keys(saleLeadFileOfEstimates).length > 0){
                            ...
                            $.each(saleLeadFileOfEstimates, function(ind, val){
                                var hrefDownload = '/sale_leads/attachment/estimate/'+company_id+'/'+idOfLead+'/'+val.id+'/download/';
                                var hrefDelete = '/sale_leads/attachment/estimate/'+company_id+'/'+idOfLead+'/'+val.id+'/delete/';
                                tmpHtml +=
            		...
            		PostInit: function(up) {
                        ...
                        if(saleLeadFileOfOrders && Object.keys(saleLeadFileOfOrders).length > 0){
                            ...
                            $.each(saleLeadFileOfOrders, function(ind, val){
                                var hrefDownload = '/sale_leads/attachment/order/'+company_id+'/'+idOfLead+'/'+val.id+'/download/';
                                var hrefDelete = '/sale_leads/attachment/order/'+company_id+'/'+idOfLead+'/'+val.id+'/delete/';
                                tmpHtml +=
                                '<li id="' + val.id + '" class="plupload_done">' +
            						'<div class="plupload_file_name"><span>' + val.file_attachment + '</span></div>' +
                                    '<div class="plupload_file_action_modify">' +
            ...
            addLogSaleLead = function(){
                ...
                var link = <?php echo $this->UserFile->avatarjs('small') ?>.replace('{id}', employeeLoginId);
                var logSystemHtml =
                    '<div class="pch_log_system" rels="' + rels + '">' +
                        '<div class="pch_log_avatar pch_log_avatar_content">' +
                            '<img id="logAvatar_' + rels + '" src="' + link + '" ' + _onclick + ' />' +
                        '</div>' +
                        '<div class="pch_log_body">' +
                            '<div class="pch_log_name">' +
