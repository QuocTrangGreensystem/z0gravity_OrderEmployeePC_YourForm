...
<style>
	...
	<!-- **
	****Quan update button back 28-01-2019
	**-->
	.heading-back{
	display: inline-block;
    vertical-align: middle;
                            ...
                            <!-- start list -->
                                ...
                                <?php
                                    foreach($data as $field):
                                        ...
                                        $name = __d(sprintf($_domain, 'KPI'), Inflector::humanize($name[0]), true);
                                        $items = array();
                                        if(!empty($showMenu)){
                                            foreach ($showMenu as $item) {
                                                $items[] = $item['Menu']['widget_id'];
                                            }
                                        }
...
jQuery(document).ready(function($){
    ...
    $('#save').click(function(){
        ...
        //ajax
        $.ajax({
            url: '<?php echo $this->Html->url('/kpi_settings/save') ?>',
            type: 'POST',
            data: serialize(),
            complete: function(){
