                    ...
                    <div class="wd-container-block-list">
                        <form id="createdValue" action="#">
                            <div class="wd-block-created-value">
                                <p class="total-sum"><span id="value_sum">0</span>/<span id="value_sum_total"><?php echo $totalValue['ProjectCreatedValue']['total'];?></span><canvas id="createdvalueCanvas" width="90px" height="90px"></canvas></p>
                            </div>
...
<script>
	...
	function updateText() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data) {
                    _html += '<div class="comment"><textarea data-project_id = ' + project_id + ' data-type_value = ' + type + ' cols="30" rows="6" id="update-comment"></textarea></div>';
	...
	function setPositionTotalSum(){
		var total_ele = $('.total-sum');
		if(total_ele.length > 0 && $('.wd-block-created-value').css('position') == 'absolute'){
			var box_1 = $('.wd-block-list-01');
			_top = box_1.offset().top + box_1.height();
			total_ele.offset({ top: _top});
			$('.wd-block-created-value').css('background-position-y', _top - 220);
		}else{
			total_ele.css({ top: ''});
			$('.wd-block-created-value').css('background-position-y', '');
		}
	}
	setPositionTotalSum();
