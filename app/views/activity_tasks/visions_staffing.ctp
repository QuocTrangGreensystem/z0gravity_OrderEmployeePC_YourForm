            ...
            <div class="wd-list-project">
                ...
                <div id="GanttChartDIV" class="ganttCustom">
                    ...
                    if (empty($start) || empty($end)) {
                        ...
                        echo $this->Html->scriptBlock('GanttData = ' . $this->GanttVs->drawStaffing($staffings, $md, $summary, $showType, $isCheck, $newDataStaffings, $activityType));
                        $this->GanttVs->end();
                    }
                    ?>
	...
	function setScrollBarWhenScroll(target)
	{
		$('.box-left:visible').each(function(index, element) {
			...
			else
			{
                ...
                if( target != null && $(this).has(target) ){
    ...
    (function($){
        ...
        GanttCallback = function($list, $gantt){
			...
			if(showType == 5 && isCheck == 0){
				$('.gantt-capacity').removeClass('displayNone');
				$('.check-name').removeAttr('rowspan');
                $('.check-name').attr('rowspan', <?php echo $colCapacityByYears;?>);
				$('.gantt-summary').find('.check-name').attr('rowspan', <?php echo $colCapacityByYears;?>);
				$('.gantt-summary').find('.gantt-employee').addClass('displayNone');
				$('.trFamily, .trSubFamily, .trActivity ').find('.check-name').attr('rowspan', <?php echo $colCapacityByYears - 1;?>);
			}
