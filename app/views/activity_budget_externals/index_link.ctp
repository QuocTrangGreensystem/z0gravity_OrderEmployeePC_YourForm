...
<div id="dialog_auto_task" class="buttons" style="display: none;">
    <?php 
        echo $this->Form->create('ActivityBudgetExternal', array('type' => 'file','url' => array('controller' => 'project_budget_externals', 'action' => 'add_auto',$activityName['Activity']['project'])
    )); ?>
...
foreach ($budgetExternals as $key=>$budgetExternal) {
    ...
    $data['capex_id'] = '';
    $data['capex_id'] = ($budgetExternal['ProjectBudgetExternal']['capex_id'] === null) ? '' : ($budgetExternal['ProjectBudgetExternal']['capex_id'] == 1 ? 'capex' : 'opex');
...
<script type="text/javascript">
    ...
    function loadTemplateAddTask(id){
        ...
        var href = "/activity_budget_externals/add_auto_task/"+parseInt(_id[1])+'/'+parseInt(_id[2]);
    ...
    (function($){
        ...
        $(function(){
            ...
            var _budgetErros = _orderedErros = _remainErros = _forecastErros = _varErros = _progressErros = _opexCalcul = _capexCalcul = _manDay = _progressMd = SpecialConsumed = special_consumed = 0;
            var totalRecord = 0;
            ...
            SpecialConsumed = number_format(special_consumed, 2, ',', ' ')+ ' ' + viewManDay;
            ...
            $this.onCellChange = function(args){
                ...
                if(args.item){
                    ...
                    _budgetErros = _orderedErros = _remainErros = _forecastErros = _progressErros = _opexCalcul = _capexCalcul = _manDay = _progressMd = special_consumed =  0;
            ...
            $.each(headers, function(index, val){
                if( headers[index].id.indexOf('actionaddtask') != -1 || headers[index].id.indexOf('man_day') != -1 || headers[index].id.indexOf('special_consumed') != -1 ){
            ...
            var hd = {
                ...
                forecast_erro : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-forecast-erro"><p>' +_forecastErros+ '</p></div>',
                var_erro : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-var-erro"><p>' +_varErros+ '</p></div>',
                ordered_erro : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-ordered-erro"><p>' +_orderedErros+ '</p></div>',
                remain_erro : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-remain-erro"><p>' +_remainErros+ '</p></div>',
                man_day : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-md" id="gs-man-day"><p>' +_manDay+ '</p></div>',
                special_consumed : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-md" id="gs-special-consumed"><p>' +SpecialConsumed+ '</p></div>',
                progress_md : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-progress-manDay"><p> '+_progressMd+' </p></div>',
                progress_erro : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-progress-erro"><p>' +_progressErros+ '</p></div>',
                opex_calculated : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-opex-calcul"><p>' +_opexCalcul+ '</p></div>',
                capex_calculated : '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro" id="gs-capex-calcul"><p>' +_capexCalcul+ '</p></div>'
            };
            var header = '<div id="wd-header-custom" class="slick-headerrow-columns" style="width: '+gridW+'px">'
                        + '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-erro"></div>';
            var i = 1;
            $.each(settings, function(sadasd, field){
                var classes = 'l' + i + ' r' + i;
                if( hd[field] ){
                    if( field == 'man_day' ){
                        header += '<div class="ui-state-default slick-headerrow-column %s wd-row-custom wd-custom-cell gs-custom-cell-md" id="gs-man-day-task"><p></p></div>'.replace('%s', classes);
                        classes = 'l' + ++i + ' r' + i;
                    }
