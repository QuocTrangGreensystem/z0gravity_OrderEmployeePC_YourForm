...
$status[] = array(
    ...
    'validator' => 'DataValidator.onLyFinish'
);
$colDescription[] = array(
    'id' => 'description',
    'field' => 'description',
    'name' => __('Description', true),
    'width' => ($type == 'mission_status') ? 320 : 450,
    'sortable' => true,
    'resizable' => true,
...
//if($type == 'mission_status' || $type == 'mission_manager' || $type == 'recom_manager'){
if($type == 'mission_status'){
    $columns = array_merge($columns, $status, $colDescription, $order, $actions);
} else {
...
if(!empty($auditSettings)){
    foreach ($auditSettings as $auditSetting) {
        $data = array(
            'id' => $auditSetting['AuditSetting']['id'],
            'company_id' => $company_id,
            'no.' => $i++,
            'MetaData' => array()
        );
        $data['name'] = (string) $auditSetting['AuditSetting']['name'];
        $data['status'] = $auditSetting['AuditSetting']['status'];
        $data['description'] = (string) $auditSetting['AuditSetting']['description'];
        $data['weight'] = (int) $auditSetting['AuditSetting']['weight'];
        $data['action.'] = '';
        $dataView[] = $data;
    }
}
...
<script type="text/javascript">
    ...
    (function($){
        ...
        $(function(){
            ...
            DataValidator.onLyFinish = function(value,args){
                var result = true;
                $.each(args.grid.getData().getItems() , function(undefined,dx){
                    if(args.item.id && args.item.id == dx.id){
                        return true;
                    }
                    if(value.toLowerCase() === 'fin' && dx.status.toLowerCase() == 'fin'){
                        return result = false;
                    }
                });
                return {
                    valid : result,
                    message : $this.t('Only have one Finish status.')
                };
            }
