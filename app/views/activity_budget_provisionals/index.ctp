    ...
    <div class="wd-layout">
        <div class="wd-main-content">
            ...
            <div class="wd-list-project">
                <div class="wd-title">
                    <h2 class="wd-t1"><?php echo __("Provisionals", true) . ': ' . $activities['Activity']['name']; ?></h2>
                    <select style="margin-right:3px; padding: 6px;" class="wd-customs" id="viewFollow">
                        <option value="man-day" <?php echo ($viewFollow == 'man-day') ? 'selected="selected"' : '';?>><?php echo  __("M.D", true)?></option>
                        <option value="euro" <?php echo ($viewFollow == 'euro') ? 'selected="selected"' : '';?>><?php echo  $viewEuro; ?></option>
                    </select>
                </div>
...
$columnDefaults = array(
	...
	'rerenderOnResize' => true,
		'id' => 'budget',
		'field' => 'budget',
		'name' => ($viewFollow == 'euro') ? __('Budget &euro;', true) : __('Budget M.D', true),
		'width' => 130,
		'sortable' => true,
		'resizable' => true,
...
$startDates = !empty($projects) && !empty($projects['Project']['start_date']) && $projects['Project']['start_date'] != '0000-00-00' ? strtotime($projects['Project']['start_date']) : 0;
$endDates = !empty($projects) && !empty($projects['Project']['end_date']) && $projects['Project']['end_date'] != '0000-00-00' ? strtotime($projects['Project']['end_date']) : 0;
...
if(!empty($internals)){
    ...
    foreach($internals as $internal){
        ...
        $data['project_id'] = !empty($activities['Activity']['project']) ? $activities['Activity']['project'] : 0;
        $data['activity_id'] = !empty($activities['Activity']['id']) ? $activities['Activity']['id'] : 0;
...
<script type="text/javascript">
    ...
    (function($){
        ...
        $(function(){
            ...
            // For validate date
            var activities = <?php echo json_encode($activities['Activity']); ?>;
...
//                },
            ...
            DateValidate.valueMonth = function(value, args){
                ...
                if(args && args.item){
                    ...
                    if(parseFloat(total) <= parseFloat(args.item.budget)){
                        ...
                        _valid = false;
                        _message = $this.t('The total Man Day less than or equal %1$s', parseFloat(args.item.budget));
                    }
                }
                return {
                    valid : _valid,
            ...
            $this.fields = {
                id : {defaulValue : 0},
                project_id    : {defaulValue : activities['project'], allowEmpty : false},
                activity_id    : {defaulValue : activities['id'], allowEmpty : false},
                view : {defaulValue : ''}
            };
            if(listYears){
            ...
            var options = {
                //showHeaderRow: false,
                enableAddRow : false,
                frozenColumn: 2 + parseInt(Object.keys(listYears).length)
            };
            ...
            $('#viewFollow').change(function(){
                location.href = '<?php echo $this->Html->url('/activity_budget_provisionals/index/' . $activity_id) ?>/' + $(this).val();
            });
        });
