...
<?php
	...
	$dataView = array();
	$i = 1;
	foreach ($projectAmrPlans as $projectAmrPlan){
		if ($projectAmrPlan['ProjectAmrPlan']['company_id'] != "") {
			$company_id_save = $projectAmrPlan['ProjectAmrPlan']['company_id'];
		...
		$row_data = array(
			'id' => $projectAmrPlan['ProjectAmrPlan']['id'],
			'no' => $i++,
			'amr_plan' => $projectAmrPlan['ProjectAmrPlan']['amr_plan'],
...
<script>
	...
	(function($){
		$(function(){
			...
			$this.fields = {
				id : {defaulValue : 0},
				amr_plan: {defaulValue : '', allowEmpty : false},
