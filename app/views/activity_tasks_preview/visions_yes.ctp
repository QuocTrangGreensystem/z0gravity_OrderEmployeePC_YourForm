                ...
                <div class="wd-panel">
                    <div class="wd-title">
                        ...
                        <div style="float: right; margin-right: 5px; margin-top: 2px;">
                            ...
                            if(!empty($staffingCate)){
                                    ...
                                    $func = ($staffingCate == 'function') ? 'selected="selected"' : '';
                    ...
                    <div class="wd-section" id="wd-fragment-1">
                        ...
                        <div class="wd-content">
                            <h2 class="vision-project"><?php echo sprintf(__('Vision Project of %s', true), $projectName['Project']['project_name']); ?></h2>
                            ...
                            <div id="GanttChartDIV">
                                ...
                                // HuuPc: 12/02/2014. Khong dung doan code nay.
                                // Chi hien thi Real Time.
                                //$convtFields = array(
...
//                                );
                                ...
                                foreach ($phasePlans as $phasePlan) {
                                    ...
                                    foreach ($convtFields as $key => $field) {
                                        $_phaseDate = $this->GanttV2->toTime($phasePlan['ProjectPhasePlan'][$field]);
                                ...
                                //$tmpStart = mktime(0, 0, 0, date("m", $start)-1, date("d", $start), date("Y", $start));
//                                $tmpEnd = mktime(0, 0, 0, date("m", $end)+1, date("d", $end), date("Y", $end));
//                                if($tmpStart < strtotime($startEmployees[0].'-'.$startEmployees[1].'-'.$startEmployees[2])){
//                                    $startEmployees = !empty($tmpStart) ? explode('-', date('t-m-Y', $tmpStart)) : array();
//                                }
//                                if($tmpEnd > strtotime($endEmployees[0].'-'.$endEmployees[1].'-'.$endEmployees[2])){
//                                    $endEmployees = !empty($tmpEnd) ? explode('-', date('t-m-Y', $tmpEnd)) : array();
//                                }
                                //pr(date('Y-m-d', $start));
                                //pr(date('Y-m-d', $end));
                                //pr($nodes);
                                ...
                                if (empty($start) || empty($end)) {
                                    ...
                                    $this->GanttV2->create($type, $start, $end, $stones, true);
                                    $line = array();
                                    ...
                                    echo $this->Html->scriptBlock('GanttData = ' . $this->GanttV2->drawStaffing2($staffingss, true, $showType, $profile));
...
echo $this->Form->hidden('months', array('value' => serialize($this->GanttV2->getMonths())));
...
?>
<style type="text/css">
    .ui-dialog,.type_buttons a{color:#000;font-size:100%;}.gantt-list-primary tr td,.gantt-list-primary td div{white-space:nowrap;}.gantt-d31 div{width:62px;}.gantt-d30 div{width:60px;}.gantt-d29 div{width:58px;}.gantt-d28 div{width:56px;}.gantt-image{padding-right:10px;vertical-align:middle;height:16px;width:16px;}.gantt-unzero{background-color:#68A8CA !important;}.gantt-unzero input, .gantt-unzero div{color:#fff !important;}.gantt-summary .gantt-image{display:none;}
</style>
<!-- Dialog Export -->
...
<?php  echo $html->css(array('visions')); ?>