...
<style>
    ...
    if (isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident') !== false)){
        echo '';//.ganttCustom-month .gantt-list.gantt-list-primary .gantt-head td { padding: 4px 0 3px;}';
    }
    ?>
</style>

<div id="loading_staffing" class="loading_w"><div class="loading_w_cont"><img class="loading_image" src="<?php echo $this->Html->url('/img/business/wait-1.gif');?>" alt="loading"/><span></span><div id="progressBarStaffing" style="height: 20px;" class = 'progressBar1'></div></div></div>
    ...
    <div class="wd-layout">
        <div class="wd-main-content">
            ...
            <div class="wd-list-project">
                ...
                <div id="GanttChartDIV" class="ganttCustom <?php if($dateType == 'month') echo 'ganttCustom-month'; ?> ">
                    <?php
                    $rows = 0;
                    $start = $end = 0;
                    ...
                    if (empty($start) || empty($end)) {
                        ...
                        $this->GanttVsPreview->create($type, $start, $end, array(), false); // thay strtotime('01-01-2015') = start date, strtotime('31-01-2015') = end date.
                        ...
                        echo $this->Html->scriptBlock('GanttData = ' . $this->GanttVsPreview->drawStaffing($staffings, $md, $summary, $showType, $isCheck, $newDataStaffings, $activityType));
                        $this->GanttVsPreview->end();
                    }
                    ?>
...
echo $this->Form->create('ExportProject', array('url' => array('controller' => 'project_staffings', 'action' => 'export_project'), 'type' => 'file'));
...
echo $this->Form->hidden('months', array('value' => serialize($this->GanttVsPreview->getMonths())));
...
<style>
	...
	if(isset($arrGetUrl['target']) && $arrGetUrl['target'] == 1)
	{
        ...
        ..trPC:first-child > td.gantt-node {
    ...
    .box-left .gantt-node:first-child {
        border-top: 0 !important;
    }
    .box-left .percent div {
        text-align: center;
    }
</style>
<!-- Dialog Export -->
...
<script type="text/javascript">
    ...
    var start = <?php echo json_encode($st) ?>,
        ...
        pc = <?php echo json_encode(isset($arrGetUrl['aPC']) ? implode(',', $arrGetUrl['aPC']) : '') ?>,
        ...
        show_na = <?php echo json_encode(intval(isset($_GET['show_na']) ? $_GET['show_na'] : 1)) ?>;
	...
	function showGraph(emp,idFamily,type){
		...
		htmlBtn+='<input type="radio" '+_check2+' name="pie-chart" onclick=\'showGraph('+emp+',"'+idFamily+'","mdconsumed");\'  /> <span class="title-chart"><?php echo __($md,true); echo ' '; echo __('Consumed',true);?> </span>';
		htmlBtn+='<input type="radio" '+_check3+' name="pie-chart" onclick=\'showGraph('+emp+',"'+idFamily+'","mdworkload");\'  /> <span class="title-chart"><?php echo __($md,true); echo ' '; echo __('Workload',true);?> </span>';
		...
		if(type=='consumed'||type=='workload')
		{
			...
			{ ?>
				var dataGraph='<?php echo isset($_SESSION['graph'])?json_encode($_SESSION['graph'], JSON_HEX_APOS):array(); ?>';
		...
		else
		{
			...
			{ ?>
				var dataGraph='<?php echo isset($_SESSION['graphMonth'])?json_encode($_SESSION['graphMonth'], JSON_HEX_APOS):array(); ?>';
				dataGraph=JSON.parse(dataGraph);
			<?php } ?>
			...
			$('#staffing-chart').highcharts({
				...
				tooltip: {
					valueSuffix: ' <?php echo __($md,true); ?>'
				},
				legend: {
					layout: 'vertical',
					align: 'right',
    ...
    var xURL='<?php echo $currentUrlResource ?>',
        redirector = '<?php echo $this->Html->url('/activity_tasks/redirect/') ?>',
        fullUrl = '<?php echo $_SERVER['REQUEST_URI'] ?>';
    var PCUrl, resourceUrl, isLoading = false;

    function buildPcUrl(pc, id){
        uri = new URI(fullUrl);
        uri.removeQuery('aProfitCenter[]');
        uri.removeQuery('aPC[]');
        uri.setQuery({
            'aPC[]': pc,
            summary: 1,
        ...
        if( !pc.length )uri.addQuery('aPC[]', id);
        return uri.toString();
    }
    function ajaxShowResourceX(idTr,id){
		// console.log( "ajaxShowResourceX");
		if( isLoading )return;
        if($("#"+idTr).hasClass('onload'))
        {
            ...
            var currentClass = '';
            yURL = new URI(xURL);
            yURL.removeQuery('getResource').addQuery({
                ItMe: id,
                classParent: currentClass
            });
            $('.p-'+idTr).remove();
            showLoader(idTr);
                // console.log(yURL.toString());
            jQuery.ajax({
                url: yURL.toString(),
                type: "GET",
                cache: false,
                success: function (html) {
                    ...
                    $.each(arrHtml, function(key,val){
                        ...
                        {
                            leftM += val.leftManager;
                            rightM += val.rightManager;
                        }
                    });
                    ...
                    $('<tr class="p-' + idTr + '"><td class=' +_colspanOne+ ' colspan=' +_colspanOne+ '><div class=\'box-left box-left-ajax\' rel="'+idTr+'" id="div-box-left-'+idTr+'"><table id="box-left-'+idTr+'">'+leftM + left+'</table></div></td></tr>').insertAfter( "#"+idTr );
                    $('<tr class="p-' + idTr + '"><td class=' +_classTwo+ ' colspan=' +_colspanTwo+ '><div class=\'box-right box-right-ajax\' rel="'+idTr+'" id="div-box-right-'+idTr+'"><table id="box-right-'+idTr+'">'+rightM + right+'</table></div></td></tr>').insertAfter( "#r-"+idTr );
                    $('.showLoader').remove();
	...
	function ajaxShowPC(idTr, id){
		// console.log( "ajaxShowPC");
        //quyet did here
        // if( id == 999999999 ){
        //     return;
        // }
        var children = $('.pp-' + idTr),
            me = $('#' + idTr);
        // if( !children.length || me.hasClass('children-loaded') ){
        //     //ajax load details
        //     //ajaxShowResourceX(idTr, id);
        //     if( me.hasClass('children-loaded') ){
        //         hideShowBox(idTr, _willHide);
        //         _willHide = !_willHide;
        //     } else {
        //         ajaxShowActivity('', '', idTr, id, function(){
        //             me.addClass('children-loaded');
        //             _willHide = true;
        //         }, function(){
        //             $('#absence-prev').removeProp('onclick').off('click').on('click', function(){
        //                 window.location.reload(1);
        //                 return false;
        //             });
        //             $('.box-left[rel!="' + idTr + '"], .box-right[rel!="' + idTr + '"]').remove();
        //         });
        //     }
        // } else {
        var list = [id];
        me.siblings('.p-' + idTr).each(function(){
            list.push($(this).data('pc-id'));
        });
        if( id == 999999999 ){
            if( me.hasClass('children-loaded') ){
                hideShowBox(idTr, _willHide);
    ...
    function ajaxShowPCInfo(idTr, id){
		// console.log( "ajaxShowPCInfo");
		// $('#loading_staffing .loading_w_cont span').html('<?php __('Loading Profit Center');?>');
        var temp;
        if($("#"+idTr).hasClass('onload'))
        {
            ajaxShowActivity('', '', idTr, id);
        }
        else
        {
            if($("#"+idTr).hasClass('acti'))
	...
	function fixedScrollBar(idTr,hideFamily){
		...
		flag=setInterval(function(){
			...
			if($('#box-left-'+idTr).length)
			{
				$('.scroll-bar .content-scroll').height($('#box-left-'+idTr).height());
				var arrPosition = $('#div-box-left-'+idTr).offset();
				// jQuery(window).scrollTop(arrPosition.top - 50);
				$('.settingBox').css({
                    top: arrPosition.top - 27,
                    left:arrPosition.left + 19
                });
				$('.settingBox').fadeIn();
				$('.scroll-bar').css({
                    'top':arrPosition.top,
                    'left': arrPosition.left - 27
                });;
	...
	function resizeBox(type){
		$('.box-left').each(function(index, element) {
			...
			else
			{
				...
				if(type=='in')
				{
					if($this.height()>800 || ($this.height() > $this.children('table').height()))
					return false;
	...
	function setScrollBarWhenScroll(target)
	{
		$('.box-left:visible').each(function(index, element) {
            if($(this).hasClass('acti'))
			{
				//do nothing
			}
			else
			{
				...
				$('.scroll-bar .content-scroll').height(tableHeight);
                if( target != null && target.length && $(this).has(target) ){
                    pos = target.position().top - table.position().top;
                    pos = pos / tableHeight * $(this).prop('scrollHeight');
                    $('.scroll-bar').animate({
                        scrollTop: pos
                    }, 200);
                }
	...
	function hideShowBoxResource(id, hideIt){
		...
		if(hideIt === true){
			...
			$('#'+id).removeClass('acti');
			$('#div-box-left-'+id).show();
			$('#div-box-right-'+id).show();
			$('.scroll-bar').show();
			$('.settingBox').show();
	...
	function ajaxShowActivity(leftPC,rightPC,idTr,id, callback, before){
		// console.log( "ajaxShowActivity");
		// $('#loading_staffing .loading_w_cont span').html('<?php __('Loading Activity');?>');
		var currentClass = $("#"+idTr).attr('class').replace('displayNone', '');
		var currentUrl='<?php echo $currentUrlActivity; ?>';
		currentUrl=currentUrl+'&ItMe='+id+'&classParent='+currentClass + '&aPC[]=' + itMe;
        var curl = new URI(currentUrl);
        curl.setQuery({
            selectPCAll: 'false'
        });
        currentUrl = curl.toString();
		...
		jQuery.ajax({
			...
			success: function (html) {
				...
				ajaxShowEmployee(leftPC,rightPC,left,right,idTr,id, callback);
				$('.box-left').addClass('acti');
				$('.box-right').addClass('acti');
				$( "#"+idTr ).removeClass('onload');
				$( "#"+idTr ).removeClass('acti');
	...
	function ajaxShowResource(idTr,id){
		// console.log( "ajaxShowResource");
		var _this = $("#"+idTr);
		if( _this.hasClass('loading')) return;
		if(_this.hasClass('onload'))
		{
			...
			jQuery.ajax({
				...
				success: function (html) {
					...
					_this.removeClass('acti');
					_this.removeClass('loading');
					$('tr.trPCEmployee').hide();
				},
				complete: function(){
					_this.removeClass('loading');
				}
		...
		if($("#"+idTr).hasClass('onload'))
		{
			...
			jQuery.ajax({
				...
				success: function (html) {
					...
					$('<div class=\'box-left\' rel="'+idTr+'" id="div-box-left-'+idTr+'"><table id="box-left-'+idTr+'">'+leftActi+leftManager+left+'</table></div>').insertAfter( "#"+idTr );
					$('<div class=\'box-right\' rel="'+idTr+'" id="div-box-right-'+idTr+'"><table id="box-right-'+idTr+'">'+rightActi+rightManager+right+'</table></div>').insertAfter( "#r-"+idTr );

                    if( typeof callback != 'undefined' ){
                        callback.call(idTr, id, leftActi, leftManager, leftPC, left);
                    }
					$('.showLoader').remove();
                    //hide not affected
                    if( !show_na ){
                        // $('#r-tr-employee-999999999, #tr-employee-999999999').hide();
                    }
					$( "#"+idTr ).removeClass('onload');
					$( "#"+idTr ).removeClass('acti');
                    //fill pc
					...
					// $('#loading_staffing .loading_w_cont span').html('<?php __('Loading Resource');?>');
                    // $('#loading_staffing').show();
                    ...
                    getStaffing(idList, function(){
                        ...
                        getStaffing(resource, function(){
							// console.log('ajaxShowEmployee finish');
                            $('#loading').hide();
							$('#loading_staffing').fadeOut();
                        });
                    });
                    //fill employee
    ...
    (function($){
        ...
        var na = '<?php echo $this->GanttVsPreview->na; ?>';
        /**
         * Convert to float
         *
         * @param val
	...
	{
		// if( showType == 1 ){
  //           var curl = <?php echo json_encode($currentUrl) ?>;
  //           //open new window
  //           var url = new URI(curl);
  //           url.setQuery({
  //               summary: 1,
  //               type: 0,
  //               'aEmployee[]': id.replace(/[^\d]+/g, '')
  //           });
  //           url.removeQuery('ItMe')
  //               .removeQuery('target')
  //               .removeQuery('ajax');
  ...
  //       }
	...
	function toggleSub(id)
	{
		...
		me.addClass('loading');
		setTimeout( function(){
			if( me.hasClass('onload') && GanttData){
				var left = $('#tr-'+id);
				var left_next = left.next();
				var right = $('#r-tr-'+id);
				var right_next = right.next();
				var left_parent = left.parent();
				var right_parent = right.parent();
				$.each(GanttData, function(i, _row){
					if( $(_row[0]).hasClass(id)){
						if( left_next.length){
							left_next.before($(_row[0]));
							right_next.before($(_row[1]));	
						}else{
							left_parent.append($(_row[0]));
							right_parent.append($(_row[1]));	
						}	
						GanttCallback( $(_row[0]), $(_row[1]) );
						GanttData[i] = ['',''];
					}
				});
				me.removeClass('onload');
			}
			if($('#tr-'+id).hasClass('acti'))		{
				$('#tr-'+id).removeClass('acti');
				$('.trSubFamily.'+id).slideDown('150', toggleActivity('sub' + id + '-'));			
			}
			else {
				$('#tr-'+id).addClass('acti');
				//$('.trSubFamily.'+id).fadeOut();
	...
	function togglePCEmployee(id){
		var me = $('#tr-' + id);
		if($('.trEmployee.'+ id).is(":visible")){
			$('#tr-'+id).addClass('acti');
			$('.trEmployee.'+id).fadeOut();
		}else{
			
			$('#tr-'+id).removeClass('acti');
			$('.trEmployee.'+id).fadeIn();
		}
		setScrollBarWhenScroll(me);
	}
    ...
    $(document).ready(function(){
        ...
        $(this)
            .on('mousewheel', '.box-left, .box-right', function(event){
                var amount = -event.deltaY * event.deltaFactor;
                $(".scroll-bar")[0].scrollTop += amount;
            });
        $(".scroll-bar").scroll(function () {
            $('.box-left, .box-right').scrollTop($(this).scrollTop());
        });
    });
    ...
    function GanttDone(){
        ...
        if( type == 1 ){
            if(targetCurrent == 'true' && itMe != 0){
                //hide summary
                // $('.gantt-summary tr:not(.gantt-title, .fixedHeightStaffing)').hide();

                // if( pc == itMe ){
                //     //hide it me
                //     // $('#tr-' + itMe).removeProp('onclick');
                //     // $('#r-tr-' + itMe).removeProp('onclick');
                // } else {
                //     //prepare hidden tr-ids
                //     $('<tr id="tr-' + itMe + '" class="x-left-pc onload acti displayNone"><td></td></tr>').insertBefore('#mcs_container .trPC:first');
                //     $('<tr id="r-tr-' + itMe + '" class="x-right-pc onload acti displayNone"><td></td></tr>').insertBefore('#mcs1_container .trPC:first');
                // }
                //
                $('#tr-' + itMe).addClass('displayNone');
                $('#r-tr-' + itMe).addClass('displayNone');
                $('.pp-tr-' + itMe).removeClass('is-child');
                ajaxShowPCInfo('tr-' + itMe, itMe);
            }
        }
        $('#loading').css('display', 'inline');
		// $('#loading_staffing').fadeIn();
...
<script type="text/javascript">
    ...
    Exporter.prototype.init = function(){
        ...
        //header: month
        var header = [];
        $('.gantt-head-scroll .gantt-staff .gantt-title td').each(function(){
            header.push($(this).text());
        });
        var gantt_head_index = 0;
        var index = 0;
        $('#export-header:first .gantt-num td').each(function(i){
            var _year = $('#export-header:first .gantt-head td').eq(gantt_head_index);
            var _month_of_year = _year.prop('colspan');
            _month_of_year = _month_of_year ? _month_of_year : 0;
            if(index == _month_of_year){
                gantt_head_index++;
                index = 0;
                var _year = $('#export-header:first .gantt-head td').eq(gantt_head_index);
            }
            // console.log('year', _year);
            var text = $(this).text() + '-' + _year.text();
            index++;
            header.push(text);
        });
        //extract data
        $('.export-left:visible').each(function(index){
            var left = $(this),
                right = $('.export-table:visible').eq(index),
                rows = [],
                title = '';
            //build left content
            left.find('tr:not(.gantt-head, .fixedHeightStaffing, :empty, .displayNone, .gantt-capacity:hidden)').each(function(){
                var tr = $(this),
                    cols = [];
            ...
            //build right content
            right.find('tr:not(.gantt-head, .fixedHeightStaffing, :empty, .displayNone, .gantt-capacity:hidden)').each(function(i){
                //traverse tds
                var row = rows[i];
    ...
    var pcName = <?php echo json_encode(isset($pcName) ? $pcName : '') ?>;
    if( pcName ){
        $('.gantt-list-primary div').text(pcName);
    }
...
<script>
    // (function($) {
        ...
        // $(window).ready(function(){
            // $('#mcs1_container .gantt-chart-wrapper >.gantt .gantt-node.gantt-node-head .gantt-head td').each(function(){
               // var _this = $(this);
               // var size = $(this).prop('colspan');
               // if( size < 3) _this.children().hide();
            // });  
            // $('td.multi-row').closest('tr').addClass('has-td-multirow');
        // });
    // })(jQuery);
	function re_align_table(){
		var right_tables = $('.gantt-chart-wrapper .export-table');
		right_tables.css('height', '');
		right_tables.each(function(){
			var _right = $(this);
			var rel = _right.attr('rel');
			if( rel) {
				rel = 'list-' + rel;
				var _left = $('table[rel="'+ rel + '"]');
				var _hei = Math.max( _left.height(), _right.height() );
				if( _hei != 0) {
					_left.height(_hei);
					_right.height(_hei);
				}
