...
foreach ($listPcs as $listPc) {
    ...
    $data['project_function_id'] = '';
    //$data['profit_center_id'] = $projectTeam['ProjectTeam']['profit_center_id'];
    $data['price_by_date'] = '';
    $data['work_expected'] = '';
    $data['profit_center_id'] = $listPc;
    if(!empty($resetRefers[$listPc])){
        foreach($resetRefers[$listPc] as $values){
            $data['employee_id'][] = $values;
            $data['backup'][$values] = "0";
            $data['profit'][$values] = '';
        }
    } else {
