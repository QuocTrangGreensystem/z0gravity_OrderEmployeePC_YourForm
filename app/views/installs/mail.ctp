<style>
    ...
    .wd-error-1, .wd-error-3, .wd-error-2, .wd-error-4, .wd-error-5{
        padding-left: 140px;
        padding-top: 5px;
        color: red;
        font-size: 12px;
    ...
    <div class="install">
        <div class="wd-left">
            <ul>
                ...
                <li class="wd-current-li"><?php echo __('Step 6: Mail Settings', true);?></li>
                <li style="border-bottom: none;"><?php echo __('Installation successful', true);?></li>
            </ul>
        </div>
       	<h2><?php echo $title_for_layout; ?></h2>
        <div class="wd-form-dt">
            <?php echo $this->Form->create('Mail', array('url' => array('controller' => 'installs', 'action' => 'mail')));?>
                <div class="wd-dt-input">
                    <label>SMTP Host <span>(*)</span>:</label>
                    <?php
                        echo  $this->Form->input('host', array('label' => false, 'div' => false, 'class' => 'wd-input', 'placeholder' => 'ssl://auth.smtp.1and1.fr'));
                    ?>
                    <p class="wd-error-1"></p>
                </div>
                <div class="wd-dt-input">
                    <label>SMTP Post <span>(*)</span>:</label>
                    <?php
                        echo  $this->Form->input('port', array('label' => false, 'div' => false, 'class' => 'wd-input placeholder', 'placeholder' => 465));
                    ?>
                ...
                <div class="wd-dt-input">
                    ...
                    <?php
                        echo  $this->Form->input('username', array('label' => false, 'div' => false, 'class' => 'wd-input placeholder', 'placeholder' => 'customer@gmail.com'));
                    ?>
                ...
                <div class="wd-dt-input">
                    ...
                    ?>
                    <p class="wd-error-4"></p>
                </div>
                <div class="wd-dt-input">
                    <label>SMTP Transport <span>(*)</span>:</label>
                    <?php
                        echo  $this->Form->input('transport', array('label' => false, 'div' => false, 'class' => 'wd-input placeholder', 'placeholder' => 'Smtp'));
                    ?>
                    <p class="wd-error-5"></p>
                </div>
...
<script>
    $('#MailMailForm').submit(function(){
        var host = $('#MailHost').val();
        var post = $('#MailPort').val();
        var username = $('#MailUsername').val();
        var password = $('#MailPassword').val();
        var transport = $('#MailTransport').val();
        ...
        if(password == ''){
            $('.wd-error-4').html('The input is not blank!');
            return false;
        } else {
            $('.wd-error-4').html('');
        }
        ...
        if(transport == ''){
            $('.wd-error-5').html('The input is not blank!');
            return false;
        } else {
            $('.wd-error-5').html('');
        }
