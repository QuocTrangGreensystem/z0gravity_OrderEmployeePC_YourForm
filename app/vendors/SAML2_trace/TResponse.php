...
class OneLogin_Saml2_TResponse extends OneLogin_Saml2_Response
{
	...
	public function add_error_log($msg){
    ...
    public function __construct(OneLogin_Saml2_Settings $settings, $response)
    {
        ...
        if (!$this->document) {
            $this->add_error_log(OneLogin_Saml2_TValidationError::msg_error(
    ...
    public function isValid($requestId = null)
    {
        ...
        try {
            // Check SAML version
			$this->add_log('SAML Version: ' . $this->document->documentElement->getAttribute('Version'));
			...
			$this->add_log('Checking ID: ' . ($this->document->documentElement->hasAttribute('ID') ? 'true' : 'false'));
			if( $this->document->documentElement->hasAttribute('ID')){
				$this->add_log('ID: ' .$this->document->documentElement->getAttribute('ID'));
			}
            ...
            if (!$singleAssertion) {
				...
				$this->add_log('Checking Assertion: SUCCESS');
			}
            ...
            if ($this->_settings->isStrict()) {
                ...
                if ($security['wantXMLValidation']) {
                    ...
                    if (!$res instanceof DOMDocument) {
						...
						$this->add_log('Checking SAML Response. Match the saml-schema-protocol-2.0.xsd');
					}
                    ...
                    if ($this->encrypted) {
                        ...
                        if (!$res instanceof DOMDocument) {
							...
							$this->add_log('Checking SAML Response with encrypted. Match the saml-schema-protocol-2.0.xsd');
						}
                    }
                ...
                if (!isset($requestId) && isset($responseInResponseTo) && $security['rejectUnsolicitedResponsesWithInResponseTo']) {
					...
					$this->add_log('Checking InResponseTo attribute: SUCCESS');
				}
                ...
                if (isset($requestId) && $requestId != $responseInResponseTo) {
					...
					$this->add_log('Check if the InResponseTo of the Response matchs the ID of the AuthNRequest (requestId) if provided: SUCCESS');
				}
                ...
                if (!$this->checkOneCondition()) {
					...
					$this->add_log('Check Validate Conditions element is exists: SUCCESS');
				}

                // Validate Asserion timestamps
                if( !$this->validateTimestamps()) return false;
                ...
                if (!$this->checkOneAuthnStatement()) {
					...
					$this->add_log('Check Validate AuthnStatement element exists and is unique: SUCCESS');
				}
                ...
                foreach ($issuers as $issuer) {
                    ...
                    if (empty($trimmedIssuer) || $trimmedIssuer !== $idPEntityId) {
                        $this->Z(OneLogin_Saml2_TValidationError::msg_error(
    ...
    public function getAssertionId()
    {
        if (!$this->validateNumAssertions()) {
            $this->add_error_log(OneLogin_Saml2_TError::msg_error("SAML Response must contain 1 Assertion."));
			return null;
        }
    ...
    public function checkStatus()
    {
        ...
        if (isset($status['code']) && $status['code'] !== OneLogin_Saml2_Constants::STATUS_SUCCESS) {
			...
			$this->add_log('Checking Status: SUCCESS');
		}
		return true;
    ...
    */
    ...
    protected function _decryptAssertion($dom)
    {
        ...
        if (empty($pem)) {
            $this->add_error_log(OneLogin_Saml2_TError::msg_error(
        ...
        if ($check === false) {
            $this->add_error_log(OneLogin_Saml2_TError::msg_error('Error: string from decrypted assertion could not be loaded into a XML document'));
        }
