    ...
    public function staffing4ncTasks($project_id = null, $checkData = false){
     ...
     public function staffingSystem($project_id = null, $checkData = false){
        ...
        if(!empty($project_id)){
			...
			$normalTaskId = !empty($projectTasks) ? Set::classicExtract($projectTasks, '{n}.ProjectTask.id') : array();
			...
			$listNCTtaskids = !empty($nctasks) ? array_keys($nctasks) : array();
			// Update by Viet Nguyen
			// Get all task id of project
			$listProjectTaskIds = array_merge($normalTaskId, $listNCTtaskids);
            ...
            $str = new str_utility();
			$sum_nct_workload = array();
			$sum_nct_workload_refer = array();
			$nct_task_id = array();
            foreach($workloads as $workload){
				...
				if(!isset($sum_nct_workload[$wl['project_task_id']])) $sum_nct_workload[$wl['project_task_id']] = 0;
				$sum_nct_workload[$wl['project_task_id']] += $wl['estimated'];
				
				if(!isset($sum_nct_workload_refer[$wl['project_task_id']])) $sum_nct_workload_refer[$wl['project_task_id']] = array();
				if(!isset($sum_nct_workload_refer[$wl['project_task_id']][$wl['reference_id']])) $sum_nct_workload_refer[$wl['project_task_id']][$wl['reference_id']] = 0;
				$sum_nct_workload_refer[$wl['project_task_id']][$wl['reference_id']] += $wl['estimated'];
				
				if(!empty($wl['estimated'])) $nct_task_id[$wl['project_task_id']] = $wl['project_task_id'];
                $fakeTasks[] = array(
                    'ProjectTask' => array(
			...
			// Sync the estimated nct_workload into project task and project task employee refers
			if(!empty($nct_task_id)){
				 $projectNCTTask = $ProjectTask->find('list', array(
					'recursive' => -1,
					'conditions' => array('ProjectTask.project_id' => $project_id,'ProjectTask.id' => $nct_task_id ),
					'fields' => array('id', 'estimated')
				));
				if(!empty($projectNCTTask)){
					foreach($projectNCTTask as $nct_task_id => $nct_value){
						if(!empty($sum_nct_workload[$nct_task_id]) && floatval($sum_nct_workload[$nct_task_id]) != floatval($nct_value) && !empty($sum_nct_workload_refer[$nct_task_id])){
							$ProjectTask->id = $nct_task_id;
							if($ProjectTask->saveField('estimated', $sum_nct_workload[$nct_task_id])){
								foreach($sum_nct_workload_refer[$nct_task_id] as $employee_id => $e_nct_value){
									$ProjectTaskEmployeeRefer->updateAll(
										array(
											'ProjectTaskEmployeeRefer.estimated' => $e_nct_value
										),
										array(
											'ProjectTaskEmployeeRefer.reference_id' => $employee_id,
											'ProjectTaskEmployeeRefer.project_task_id' => $nct_task_id
										)
									);
								}
							}
			...
			$endDataEmployees = array();
			$datasSaveOfPCStepOne = array();
			$datasSaveOfPCNotAffected = array();
			$dataSaveProfile = array();
            ...
            if(!empty($projectTasks)){
                ...
                */
                ...
                foreach($projectTasks as $projectTask){
                    ...
                    if(in_array($_taskId, $parentIds) || empty($projectTask['ProjectTask']['task_start_date']) || empty($projectTask['ProjectTask']['task_end_date'])){
			...
			$ProjectBudgetSyn->updateBudgetSyn($project_id);
			if($idActivityLinked != 0)
			{
